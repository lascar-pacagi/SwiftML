...
let position_of_offset (ls : int array) (off : int) : Token.pos =
  ...
  ignore (ls, off);
  failwith "TODO(01-v1b): implement Lexer_v1_fast.position_of_offset (lazy line/col)"
...
let lex (source : string) (diagnostics : Diagnostics.sink) : soup =
  ...
  ignore (push, char_at, skip_trivia, is_digit, is_identifier_start, is_identifier_continue);
