...
scan: from the parser never needing a token at every position, so the tokens it skips are never
built at all. Deferring only pays for the work that is never demanded.
