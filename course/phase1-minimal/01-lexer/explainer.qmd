...
- **the three ways input can run out** — a file whose last line has no `\n` (`5` on its own is
  `int(5) eof`, *not* `int(5) newline eof`), a comment that reaches end-of-input, and an empty
  file (just `eof`). Each is a place where a munch loop without a bounds check walks off the end.

The goldens were produced by running the answer key; they are the specification, not a
recording of your output — if `dune promote` looks tempting, the test is telling you something.
...
```
   lexer.ml:22 current_position            22.0%  ########
   lexer.ml:37 make_token                  19.3%  #######
```

That is as close as it gets to "this line is the bottleneck" — `current_position` builds a `Token.pos`, `make_token`
...
```
   lexer_v1_fast.ml:84  Lexer_v1_fast.position_of_offset           29.1%
...
The records and the positions are back — `position_of_offset` is now the single biggest allocator. The fast rung
