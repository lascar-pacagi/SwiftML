...
4. **Dispatch** on `peek_char lx`:
   - `'0'..'9'`: remember `lx.pos`, `bump` while the next char is a digit, take the substring
     between the two positions, `int_of_string` it, `make lo lx (Token.Int n)`.
...
let take_while lx pred =
  let start = lx.pos in
  while (not (at_end lx)) && pred (peek_char lx) do
    ignore (bump lx)
  done;
...
- **`TODO(01-v1a)` — `lex`**, the scanning loop. The soup type, the growable arrays and their
  ...
  push a one-byte token, and anything else is `error s (s + 1) "invalid character in source file"`
...
- **`TODO(01-v1b)` — `pos_of`**, the search that turns a byte offset into `line:col` against the
...
while !continue do
  ...
  if !pos >= len then (push t_eof len len; continue := false)
    ...
    if is_digit c then (
      ...
      incr pos;
      while !pos < len && is_ident_cont (g !pos) do
        incr pos
      done;
      push t_ident s !pos)
    else (
...
if is_digit c then (
  let start = lx.pos in
  while (not (at_end lx)) && (let d = peek_char lx in is_digit d || d = '_') do
    ignore (bump lx)
  done;
