...
a soup. Time the three things a parser could do with it instead (2.6 MB, three runs, stable):

```
   v0  tokenize -> Token.t list           27.8 MB/s    1.00x
   v1  lex (soup only)                   199.1 MB/s    7.2x     <- what the bench reports
   v1  lex + to_tokens -> Token.t list    20.6 MB/s    0.74x    <- SLOWER than v0
   v1  lex + walk every kind, no spans   153.2 MB/s    5.5x
```
...
keeps 5.5× of the 7.2×; one that calls `to_tokens` first keeps none of it. swiftc makes exactly
