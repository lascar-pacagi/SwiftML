...
- **Out of scope (Phase 1):** string/float literals, interpolation (`"area is \(area)"` — an
  expression embedded in a literal, so the scanner would have to lex code inside a string),
  most keywords/operators, Unicode identifiers — added in later phases.
...
> **Swift notes** — what is non-obvious about this subset coming from C, Rust or Go. The lexer only
> *tokenizes*, and every rule below is enforced later, in `03-sema` or `04-codegen`. They are here
> because each one is a place where a reasonable guess gives a different answer from Swift's, and
> Phase 1 is done only when `swiftml` and `swiftc` agree on every program in the corpus.
>
...
let width  = 12
var height = 5
print(width * height - 3)   /* prints 57 */
```
...
```
token         ::= Int | Ident | keyword | operator | punct | Newline | Eof

Int           ::= digit+                          # decimal only in Phase 1  -> Token.Int
Ident         ::= id-head id-tail*                # -> Token.Ident (unless a keyword, below)
keyword       ::= "let" | "var"                   # an Ident whose spelling is in the table
operator      ::= "+" | "-" | "*" | "/" | "%" | "="
punct         ::= "(" | ")" | ","
Newline       ::= "\n"                            # SIGNIFICANT: terminates a statement
Eof           ::= <end of input>                  # the one token matched by NO characters:
                                                  # emitted once, when the input runs out
...
nl         ::= Newline+                        # ONE OR MORE, so the `nl?` above is any run
...
Watch three things: the comment-only first line still yields a `newline`; the two characters `12`
become one `int(12)` rather than two `int`s (maximal munch); and the trailing `/* prints 57 */` is
dropped, but its line's `\n` is not.

```
newline          # line 1 was only a comment: skipped, but its line break is a token
...
Each branch consumes characters with `advance_char` (which also advances line/column) and builds a token
spanning `[start_position, current_position)` so later stages can point diagnostics at exact source ranges.
...
@fig-dfa is the whole of `next()` on one page: skip trivia, test for end of input, dispatch on
one character, and every path ends at a token.

![The lexer DFA: skip trivia, then dispatch on the first character.](figs/dfa.png){#fig-dfa}
...
type sink = { mutable diagnostics : t list             (* threaded through the front end *)
            ; source : string option }            (* the text, for rendering snippets *)

val create     : ?source:string -> unit -> sink
val error      : sink -> Token.span -> string -> unit
val warning    : sink -> Token.span -> string -> unit
val note       : sink -> Token.span -> string -> unit  (* explains the one above it *)
val has_errors : sink -> bool
val print      : sink -> unit              (* swiftc-style `line:col: error: message` *)
```

`print` renders what `swiftc -diagnostic-style=llvm` renders: the message, then the source line it
points at, then a caret under the offending column.

```
2:8: error: expected expression
let y =
       ^
```

That third line is why the sink carries the source at all. A span can point at a token with no
glyph — the `Newline` that ends a line — and then `2:8` on a seven-character line looks like it
points at nothing; the caret sitting just past the text says *end of this line* instead. A sink
built without a source (a unit test that only inspects `all`) still works and prints the message
alone, which is why `source` is optional.
...
```ocaml
Lexer.create  : string -> Diagnostics.sink -> t           (* concept 01 *)
...
The helper is given, right under `make_token`:
...
Open `lexer.ml` in this directory. The cursor type, `create`, and the helpers `current_position`, `at_end`, `peek_char`,
`advance_char`, `make_token`, plus the `tokenize` driver are **already written**. You implement **`next`**.

1. **Skip trivia.** Loop: while `peek_char lexer` is a space or tab, `advance_char`. Then handle comments:
   if you see `//`, `advance_char` to end of line (but *don't* consume the `\n` — it's a token). If you see
   `/*`, consume a **nesting** block comment (track depth). After skipping, you may need to loop
   again (comment followed by spaces).
2. **Capture the start** with `let lo = current_position lexer in`.
3. **End of input:** `if at_end lexer then make_token lo lexer Token.Eof`.
4. **Dispatch** on `peek_char lexer`:
   - `'0'..'9'`: remember `lexer.pos`, `advance_char` while the next char is a digit, take the substring
     between the two positions, `int_of_string` it, `make_token lo lexer (Token.Int n)`.
   - letter or `'_'`: scan an identifier the same way (first char letter/`_`, rest
     letters/digits/`_`), then `make_token lo lexer (Token.keyword_or_ident text)`.
   - `'+'` → `Plus`, `'-'` → `Minus`, `'*'` → `Star`, `'/'` → `Slash` (only if not a comment, which
     you already handled), `'%'` → `Percent`, `'='` → `Eq`, `'('` → `LParen`, `')'` → `RParen`,
     `','` → `Comma`. Each: `ignore (advance_char lexer); make_token lo lexer <kind>`.
   - `'\n'`: `ignore (advance_char lexer); make_token lo lexer Token.Newline`.
   - anything else: `report_error lexer lo "invalid character in source file"`, then **consume the byte and
     carry on** (`ignore (advance_char lexer); next lexer`) — reporting without recovering would still stop at the
...
let take_while lexer pred =
  ...
  while (not (at_end lexer)) && pred (peek_char lexer) do
...
Three suites run. `tests/lexer.t` diffs the token stream from `./lab.exe --emit-tokens` — a
binary built from *this* directory's lexer, so it is your `next` under test and not the phase
binary. `tests/test_lexer.ml` (alcotest) asserts kinds **and spans** in-process. And
`tests/oracle.t` asks `swiftc` itself, every run, whether each program in `oracle-corpus.txt` is
lexable — the goldens record what swiftc said once; the oracle keeps asking. Between them they
pin the corners that are easy to get subtly wrong:
...
- **running out of input** — an empty file, a file that ends inside a comment, and a file whose
  last line has no `\n` (`5` alone lexes as `int(5) eof`, with no `newline` invented before it):
  the three places a munch loop can read past the last byte.

The goldens were produced by running the answer key; they are the specification, not a
recording of your output.
Expected for `print(1 + 2)`:
...
```
  ...
  v0_naive      32.9 MB/s   (750001 tokens/lex, 1.897s)
  v1_fast      397.1 MB/s   (750001 tokens/lex, 0.157s)
  => v1_fast is 12.1x v0_naive
```

So we go from tens of MB/s to **hundreds — squarely in swiftc's band**. (The ratio moves with the
machine and the corpus; what does not move is where the time goes.) Here's the whole story, because
*how* you get there is the lesson.
...
**The diagnosis is one table** — view 2, the same input at three levels of ambition:

```
   byte scan floor (no tokens)          1025.6 MB/s      1.95 ms
   v1_fast (offsets, columnar)           233.6 MB/s      8.56 ms
   v0 (Token.t list + line/col)           40.3 MB/s     49.68 ms

   => scanning itself is 4% of v0's time; the other 96% is what v0 BUILDS
```

Touching every byte costs 2 ms; the lexer costs 50 ms. **Optimising the scan loop cannot win more
than 4%.** The other three views are that sentence in detail.

**View 1, cost per construct** — the same 2 MB nine times, each run dominated by one token shape.
Read `ns/token`, not MB/s, which flatters comments because they produce no tokens at all:
...
Comments are the cheapest bytes in the file: they build nothing. Identifiers are the dearest
tokens: `String.sub`, then `Token.keyword_or_ident` compares the result.

**View 3, allocation accounting** — `Gc.quick_stat` deltas around one pass. "Allocation-bound"
stops being a claim and becomes a number: **~25 words/token for v0, ~7 for v1_fast**.

**View 4, allocation sites** — `Gc.Memprof` samples one allocation in a thousand *with a
callstack*, and attributes the words to the innermost frame that has a source location. Three runs,
three pictures:

```
   v0                          v1_fast (scan only)           v1_fast + to_tokens
   22.0% current_position      66.9% lex.push.grow           29.1% position_of_offset
   19.3% make_token            11.5% Array.make tags         17.7% lex.push.grow
                               11.5% Array.make ends         11.1% to_tokens.build
                               10.1% Array.make starts       10.9% to_tokens.build
```

Left: a `Token.pos` and a span per token. Middle: four *arrays* — the initial three and their
doublings — and nothing per token, because the scan never builds one. Right is the honest one: ask
for every token's span and the records come back, `position_of_offset` now the biggest allocator of
all. The fast rung **deferred** those costs, it did not remove them — which is why the parser is
meant to consume the soup and ask for a position only when it has a diagnostic to place, exactly as
swiftc does with `SourceLoc` and `SourceManager`.

**The CPU view** says the same thing from outside. `make profile-cpu` attaches macOS `sample` to a
steady workload (`profile.exe --spin`, no Memprof overhead) and sorts frames by top-of-stack:
...
Three quarters of the CPU is the collector — `caml_shared_try_alloc`, `do_some_marking`,
`pool_sweep`, `oldify_one`. `Lexer.next` itself is 6.6%. Two notes on reading the full report: the frame table
it prints is a top-15 excerpt while these shares cover *every* frame, so the rows do not add up to
them; and "yours + stdlib" is one bucket because both are ordinary OCaml frames — `List.rev_append`
(from `tokenize`'s `List.rev`) sits in it, so *your* lexer is nearer 16% than 20%.
...
|---|---|---|---|---|
| **v0** | `Lexer.next` 5.2%, `advance_char` 3.2%, `peek_char` 1.7% | 18.2% | **78.1%** | 3.7% |
...
### How v1_fast gets its 12× — the exact moves swiftc makes

`lexer_v1_fast.ml` attacks the representation, not the loop. Three changes, all of which mirror
...
1. **Offset-only positions (Swift's `SourceLoc`).** A token stores just two `int` byte offsets —
   *start* and *end*. It does **not** carry line/col. Line and column are derived **lazily** from a
   one-time *line-start table* (`position_of_offset` binary-searches the offsets of line beginnings) only when
...
3. **Defer the data work.** `Int` literals aren't `int_of_string`'d and identifiers aren't
   ...
   `to_tokens` / `token_kind_at`. Even the keyword check (`let`/`var`) compares bytes in place rather than
   allocating a substring.

@fig-memory is the same tokens under both layouts, measured with `Obj.reachable_words` on a real
2801-token program rather than counted by hand.

![A `Token.t list` is five blocks per token at five addresses; the soup is three arrays where
column `i` is token `i`.](figs/memory.png){#fig-memory}

A `Token.t list` costs **18.1 words per token**, spread over five separate blocks — the cons cell,
the `Token.t`, its `span`, and two `pos` records. Reading one token's *kind* means following
pointers to addresses the allocator picked independently, and each load must complete before the
next address is even known: a pointer chase is the one access pattern a hardware prefetcher cannot
run ahead of.

The soup costs **4.0 words per token**, and column `i` *is* token `i`. Reading every tag is a
stride-1 walk, so a 64-byte cache line carries eight of them: one line per eight tokens, against two
or more lines *per token* for the list. The allocation saving is about 4×; the cache saving is much
larger, which is why the measured speedup is bigger than the word count alone predicts.

Net effect: the scanning pass allocates *essentially nothing per token*. That's the 12×.

**But what does a *parser* get?** `make bench` times `lex` against `tokenize` — a soup against a
finished `Token.t list`. A parser needs tokens, so measure whole front ends instead: same grammar,
same corpus, ASTs checked identical.

```
   A  v0 lexer -> Token.t list -> parser   121.8 ms  =  92.9 lex  +  28.9 parse   1.00x
   B  soup -> parser, AST keeps line/col   101.1 ms  =   7.8 lex  +  93.3 parse   1.20x
   C  soup -> parser, AST keeps offsets     58.5 ms  =   7.8 lex  +  50.7 parse   2.08x
```

**12× on the lexer is 1.2× on the front end** — and would be 0.78× if the parser called
`to_tokens` first, which is *slower than never deferring at all*. The cost does not vanish, it
moves: B's parse is 93 ms against A's 29, because every span it materialises costs two binary
searches over a 50,000-entry line table where v0 kept line and column with one increment per
character.

Row C is the design that actually pays, and it is swiftc's: the AST stores **offsets**, so
`line:col` is a `SourceManager` query priced per *diagnostic* rather than per node. Even then the
win is 2×, not 12× — and it comes from a different place than you would guess. Not from the fast
scan: from the parser never needing a whole token at most positions, so the ones it skips are never
built. Take one line of the corpus and ask which of its tokens end up in the tree:

```
   let  value_0  =  1000  +  0  *  3  -  40  /  5  %  6  nl      15 tokens
    ●      ●         ●       ●     ●     ●      ●     ●          8 reach the tree
```

The eight are the `let` keyword (the `Let` node spans from it), the identifier, and the six integer
literals. The other seven are read for their **kind alone**. The operators are the surprise: look at
`parse_expr_bp` and the `Binary` node's span is `span_between (expr_span left) (expr_span right)` —
computed from its *operands*, so the `+` token's own span is never asked for. Same for the `=` of an
assignment, the `(` of a call, and every `Newline` that `skip_newlines` steps over. And
`peek_kind_at parser 1`, the lookahead that tells `c = …` from `c + …`, reads a kind at a position
the parser may not consume at all.

v0 built all fifteen. Deferring pays for exactly the seven nobody asks about — which is also why
the win shrinks as the consumer gets greedier, and vanishes at `to_tokens`, which asks for all of
them.
...
- **`TODO(01-v1a)` — `lex`**, the scanning loop. The soup type, the growable arrays and their
  `push`, and the trivia `skip` are given; you write the token loop. Shape: until you have pushed
  `t_eof`, call `skip ()`, stop at end of input, else read `s = !pos` and dispatch — digits munch
  into `t_int`, an identifier head munches into `t_ident`, the single-character operators and `'\n'`
  push a one-byte token, and anything else is `report_error s (s + 1) "invalid character in source file"`
  followed by carrying on (no token pushed), exactly the recovery v0 does. The rules are the three
  moves above: offsets only, no `Token.t`/`span`/`pos`, and no `String.sub` or `int_of_string` in
  the loop. Mind the end of the buffer — `g` is `String.unsafe_get`, so an off-by-one reads past the
  string instead of raising; test `!pos < len` after `skip ()` and in every munch loop.
- **`TODO(01-v1b)` — `position_of_offset`**, the search that turns a byte offset into `line:col` against the
  line-start table `ls`: find the **last index `i` with `ls.(i) <= off`**, then `line = i + 1` and
  `col = off - ls.(i) + 1` (both 1-based) and `offset = off`. Binary-search it — this is the piece
  that *lets* the hot loop ignore positions, and it is the only caller of the table in the rung.

The two holes are **independent**, and the tests know it — each has its own check that stays
skipped until you start that piece:

```
[OK]  v1_fast piece: position_of_offset   checked.
[OK]  v1_fast piece: lex      skipped — TODO(01-v1a) lex not written yet
[OK]  v1_fast                 skipped — lexer_v1_fast.ml is still a skeleton
```

`position_of_offset` needs only the line table, and the scanner touches it only on its error path, so you can
write and green-light either one first and debug it alone. `position_of_offset`'s test also cross-checks the
whole corpus against **v0**: v0 maintained `line`/`col` as it scanned, `position_of_offset` reconstructs them
...
### Why the reference lexer stays naive

`lexer.ml` keeps the plain `Token.t list` on purpose: it is the file you learn the DFA from, and
Phase 1 is not perf-critical. The fast rung sits beside it as `lexer_v1_fast.ml` — a second module
in this same directory, with its own answer key in `solution/` — so both are built, both are
tested, and `to_tokens` proves they agree token-for-token.

Which one a real compiler ships is the table above, not a matter of taste: the soup, *and* an AST
that stores offsets. Half the design buys 1.2×; both halves buy 2×. §6 has further perf exercises.
...
and nothing here depends on it: `lexer.t` runs its own `lab.exe`, which prints kinds only.
...
`Diagnostics.print` also echoes the offending source line with a caret under it after each of these
(§2's *The third file*); the transcripts below show only the message lines, which are what the wording
exercise is about. One level open:
...
so the message is `insert '*/' to close this comment` at depth 1, and
`insert '*/*/' to close these 2 nested comments` at depth 2 — singular and plural
...
type and `create`/`current_position`/`at_end`/`peek_char`/`advance_char`/`make_token`/`tokenize` are the given scaffolding; the
...
let next (lexer : t) : Token.t =
  ...
  if at_end lexer then make_token lo lexer Token.Eof
    ...
    if is_digit c then (
      ...
      | '=' -> ignore (advance_char lexer); make_token lo lexer Token.Eq
...
- **Maximal munch** is the `while … is_digit …` / `while … is_identifier_continue …` loops: keep consuming
...
- Errors go into the `Diagnostics.sink` the cursor carries, and the lexer **recovers** — the two
  ...
  lexer pred` helper (see §3) is an equally good style — the solution inlines them for directness.
...
while !continue do
  ...
  if !pos >= len then (push t_eof len len; continue := false)
    ...
    if is_digit c then (
      incr pos;
      while !pos < len && is_digit (g !pos) do
        incr pos
      done;
      push t_int s !pos)
    else if is_identifier_start c then (
      incr pos;
      while !pos < len && is_identifier_continue (g !pos) do
        incr pos
      done;
      push t_ident s !pos)
    else (
...
`int_of_string`, and no line/col bookkeeping. Everything the loop defers, `token_kind_at`/`to_tokens`
reconstruct later — and only for tokens somebody actually looks at.

## 9. Exercise solutions

The fragments below are quoted from `solution/exercises/lexer.ml` and `solution/exercises/token.ml` — the stock answer key with these
exercises applied, which `make check-exercises C=phase1-minimal/01-lexer` builds and runs.
...
if is_digit c then (
  let start = lexer.pos in
  while (not (at_end lexer)) && (let d = peek_char lexer in is_digit d || d = '_') do
    ignore (advance_char lexer)
  done;
  let lexeme = String.sub lexer.source start (lexer.pos - start) in
  make_token lo lexer (Token.Int (int_of_string lexeme)))
```
...
The opener has to be captured *before* the two `advance_char`s that consume `/*`, and the depth counter is
already there — it just has to survive the loop:

```ocaml
else if c = '/' && peek_next_char lexer = '*' then (
  let opener = current_position lexer in          (* <- before consuming the opener *)
  ignore (advance_char lexer); ignore (advance_char lexer);
  let depth = ref 1 in
  while !depth > 0 && not (at_end lexer) do
    ...
  ...
  if !depth > 0 then (
    ...
    let note span msg =
      Diagnostics.emit lexer.diagnostics
        { Diagnostics.severity = Note; span; message = msg }
    in
...
the inner ones only advance_char `depth` — which is exactly why `depth` is still the count of *unclosed*
...
the newline token in `"a\nb"` prints `newline @ 1:2-2:1`, because `advance_char` moved the cursor to line 2
