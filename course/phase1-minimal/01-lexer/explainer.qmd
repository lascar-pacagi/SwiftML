...
val note       : sink -> Token.span -> string -> unit   (* explains the diagnostic above it *)
...
let is_identifier_start (c : char) : bool = (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c = '_'
...
```ocaml
  ...
  if !depth > 0 then (
    ...
    let note span msg =
      Diagnostics.emit lexer.diagnostics { Diagnostics.severity = Note; span; message = msg }
    in
