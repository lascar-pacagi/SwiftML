...
let shapes : (string * string) list =
  [
    ...
    ( "mixed (real code)",
      repeat_to "let value_9 = 1000 + 9 * 3 - 40 / 5 % 6   // row\n" );
  ]
...
let have_v1 =
  lazy (match count_v1 "1 + 2" with _ -> true | exception Failure _ -> false)

let with_v1 (f : unit -> 'a) : 'a option =
  if Lazy.force have_v1 then Some (f ()) else None
...
let section_shapes () =
  ...
  print_endline "";
  print_endline
    "   ns/token is the honest cross-shape comparator (MB/s also reflects how \
     many";
  print_endline
    "   tokens a shape packs per byte). Comments produce no tokens — read \
     their MB/s.";
  print_endline "";
  Printf.printf "   %-18s %10s %10s %12s %10s\n" "construct" "MB/s v0" "MB/s v1"
    "ns/token v0" "tokens";
  Printf.printf "   %s\n" (String.make 64 '-');
  let rows =
    List.map
      (fun (name, src) ->
        ...
        let ns_col =
          if n0 < 1000 then "           -" else Printf.sprintf "%12.1f" ns
        in
        let v1_col =
          if Float.is_nan mbps1 then "         -"
          else Printf.sprintf "%10.1f" mbps1
        in
  ...
  let worst =
    List.fold_left
      (fun a r ->
        if (fun (_, _, ns, _) -> ns) r > (fun (_, _, ns, _) -> ns) a then r
        else a)
  ...
  Printf.printf
    "   => per token: %s is dearest (%.0f ns), %s cheapest (%.0f ns) — a %.1fx \
     spread.\n"
    (name_of worst) (ns_of worst) (name_of best) (ns_of best)
    (ns_of worst /. ns_of best);
  print_endline
    "      A construct costs either because it RE-READS bytes or because it \
     ALLOCATES.";
...
let section_ladder (src : string) =
  print_endline
    "2. THE COST LADDER — what each layer of the design costs (same input)";
  print_endline "";
  let mb = bytes_mb src in
  ...
  (match with_v1 (fun () -> best_of_3 count_v1 src) with
  | Some (t1, n1) ->
      row "v1_fast (offsets, columnar)" t1 (Printf.sprintf "%d tokens" n1)
  | None ->
      print_endline
        "   v1_fast (offsets, columnar)          -            -       not \
         implemented yet");
  row "v0 (Token.t list + line/col)" t0 (Printf.sprintf "%d tokens" n0);
  print_endline "";
  Printf.printf
    "   => scanning itself is %.0f%% of v0's time; the other %.0f%% is what v0 \
     BUILDS\n"
    (tf /. t0 *. 100.)
    ((t0 -. tf) /. t0 *. 100.);
  (match with_v1 (fun () -> best_of_3 count_v1 src) with
  | Some (t1, _) ->
      Printf.printf
        "      (v1_fast keeps the same scan and pays %.1fx less than v0 for \
         the building)\n"
        ((t0 -. tf) /. (t1 -. tf))
  | None ->
      print_endline
        "      (implement TODO(01-v1a) to see what the fast rung pays instead)");
  print_endline ""
...
let section_alloc (src : string) =
  ...
  | Some w1 ->
      Printf.printf
        "   => v0 allocates %.1f words per token, v1_fast %.1f (%.0fx less).\n"
        w0 w1
        (w0 /. max 0.01 w1)
  | None ->
      Printf.printf
        "   => v0 allocates %.1f words per token (v1_fast not implemented yet).\n"
        w0);
  print_endline
    "      One OCaml word = 8 bytes. A Token.t is a record holding a span, \
     which holds";
  print_endline
    "      two positions — plus one list cons per token. That is the \
     4-records-per-token";
  print_endline
    "      shape §5 of the explainer talks about; the number above is the \
     receipt.";
  print_endline ""
...
let section_memprof ?(rung = "v0") ~(run : string -> int) (src : string) =
  ...
  let attribute (cs : Printexc.raw_backtrace) (n_samples : int) =
           ...
           | Some loc when Filename.check_suffix loc.Printexc.filename ".ml" ->
               ...
               key :=
                 Printf.sprintf "%s:%d %s"
                   (Filename.basename loc.Printexc.filename)
    ...
    Hashtbl.replace tbl !key
      (n_samples + Option.value ~default:0 (Hashtbl.find_opt tbl !key))
  in
  let tracker =
    {
  ...
  if !total = 0 then
    print_endline
      "   (no samples — build with debug info, or raise the sampling rate)"
  else
    List.iteri
      (fun i (site, n) ->
        if i < 8 then
          Printf.printf "   %-40s %6.1f%%  %s\n" site
            ...
            (String.make
               (int_of_float (float_of_int n /. float_of_int !total *. 40.))
               '#'))
      rows;
  print_endline "";
  print_endline
    "      Percentages are of sampled allocated words, attributed to the \
     innermost";
  print_endline
    "      frame with a source location. Read it as \"this line is where the \
     garbage";
  print_endline
    "      comes from\" — then ask whether that value needs to be \
     heap-allocated.";
  print_endline ""
...
let main () =
  ...
  Printf.printf
    "Lexer profile — %.2f MB of generated Swift per measurement, best of 3.\n\n"
    (bytes_mb mixed);
  section_shapes ();
  ...
  | Some () ->
      ...
      section_memprof ~rung:"v1_fast + to_tokens"
        ~run:(fun src ->
          List.length
            (Lexer_v1_fast.to_tokens
               (Lexer_v1_fast.lex src (Diagnostics.create ()))))
        mixed
  | None -> ());
  print_endline
    "Next: `make profile-cpu C=phase1-minimal/01-lexer` for a sampled call \
     tree,";
