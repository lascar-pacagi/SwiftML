...
let check_equiv () =
  let v0 = Lexer.tokenize (Lexer.create tricky) in
  let v1 = Lexer_v1_fast.(to_tokens (lex tricky)) in
...
let () =
  ...
  check_equiv ();
  let v0 = time "v0_naive" (fun () -> List.length (Lexer.tokenize (Lexer.create src))) iters mb in
  let v1 = time "v1_fast" (fun () -> (Lexer_v1_fast.lex src).Lexer_v1_fast.n) iters mb in
