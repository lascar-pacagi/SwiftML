      ...
      (fun i (a : Token.t) ->
        ...
        | Some b ->
            Printf.printf "    [%d] v0=%s@%d:%d  v1=%s@%d:%d\n" i
              (Token.string_of_kind a.Token.kind)
              a.Token.span.Token.lo.Token.line a.Token.span.Token.lo.Token.col
              (Token.string_of_kind b.Token.kind)
              b.Token.span.Token.lo.Token.line b.Token.span.Token.lo.Token.col
...
let () =
  ...
  Printf.printf
    "Lexer throughput — %.2f MB source (%d lines) x %d iterations:\n" mb lines
    iters;
  if not (v1_ready ()) then (
    let v0 =
      time "v0_naive"
        (fun () ->
          List.length
            (Lexer.tokenize (Lexer.create src (Diagnostics.create ()))))
        iters mb
    in
    ignore v0;
    print_endline "";
    print_endline
      "  v1_fast is not implemented yet (TODO(01-v1a) in lexer_v1_fast.ml), so \
       there is";
    print_endline
      "  nothing to compare against and the perf gate is skipped. `make \
       profile` works";
    print_endline
      "  on v0 alone and shows you why the fast rung is worth writing.";
    exit 0);
  check_equiv ();
  let v0 =
    time "v0_naive"
  ...
  if v1 /. v0 < min_speedup then (
    Printf.printf
      "\n\
      \  PERF GATE FAILED: v1_fast must be at least %.1fx v0_naive, measured \
       %.1fx.\n\
      \  A fast rung that allocates per token is not a fast rung — run\n\
      \  `make profile C=phase1-minimal/01-lexer` (section 3) and drive \
       words/token down.\n"
      min_speedup (v1 /. v0);
    exit 1)
