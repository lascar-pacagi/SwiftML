...
let test_ex1_redeclaration () =
  ...
  Alcotest.(check (list string))
    ...
    [ "invalid redeclaration of 'y'" ]
    (errors "let y = 1\nvar y = 2\nprint(y)\n");
  Alcotest.(check (list string))
    "...and so is `let` after `var`"
    [ "invalid redeclaration of 'z'" ]
    (errors "var z = 1\nlet z = 2\nprint(z)\n");
  ...
  Alcotest.(check bool)
    "the redeclaration is reported" true
    (List.mem "invalid redeclaration of 'w'" both);
  Alcotest.(check bool)
    "and its initializer is still checked" true
...
let test_ex2_note () =
  ...
  if List.mem "invalid redeclaration of 'x'" (errors redecl) then (
    ...
    in
    Alcotest.(check int)
      "...on the FIRST declaration's line" 1
      note.Diagnostics.span.Token.lo.Token.line)
...
let () =
  Alcotest.run "sema exercises"
    [
      group "ex1 redeclaration" ex1_started "the redeclaration check"
        test_ex1_redeclaration;
