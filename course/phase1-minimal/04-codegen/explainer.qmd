...
LLVM IR is a typed, SSA assembly. For Phase 1 you need a sliver of it:
...
Key facts: every computed value is a fresh `%N` register assigned exactly once (SSA);
`i64` is our `Int`; `alloca` gives a stack slot you `store`/`load`; arithmetic is `add/sub/mul/
sdiv/srem i64`; `printf` with `%lld` prints a 64-bit int. (Phase 4's mem2reg will *remove* the
allocas by promoting them to SSA registers — but writing them now is the simple, correct start.)
