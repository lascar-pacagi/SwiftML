...
Read that as pseudo-code: `%k` is whatever `fresh ()` just returned, and `v`, `a`, `b` inside the
strings are the operand strings the recursive calls handed back — in real code the line is built
with `Printf.sprintf`, e.g.

```ocaml
| Ast.Unary (Ast.Neg, e, _) ->
    let v = gen_expr e in
    let r = fresh () in
    emit (Printf.sprintf "%s = sub i64 0, %s" r v);
    r
```

The shape to notice is the last two lines of every case: *emit the instruction, then return the name
of the register that now holds the value*. That returned string is the only thing a caller needs —
which is why a literal costs no instruction at all (it returns `"42"`, and the parent drops it
straight into its own operand slot). Statements: `let`/`var` ⇒ allocate a slot (a map from source
