...
And `swiftml` is *your* compiler, not a stashed reference build. `phase1-minimal/bin/dune` declares
it as an executable over `swiftml_codegen`, the library built from **this** directory
(`irgen.ml` + `driver.ml`), which depends on `swiftml_sema` (03) → `swiftml_parser` (02) →
`swiftml_lexer` (01). So the binary is exactly the chain of concept directories you have been
filling in, and `oracle.sh` runs `dune build` before using it — your latest edit is always what
gets compared against `swiftc`.

That is also the difference from `make lab`, which builds a `./lab.exe` from *one* concept's
library so a concept's tests exercise that directory alone. `make oracle` always goes through the
whole phase binary, because it needs to produce a running program.
