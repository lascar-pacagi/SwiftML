...
let dump_of_expr (src : string) : string =
  Ast.dump_expr (Parser.parse_expr (mk src))

let check_expr name expected src =
  Alcotest.(check string) name expected (dump_of_expr src)
...
let test_expr_precedence () =
  ...
  check_expr "/ and % share a level, left assoc, above +" "(+ 1 (% (/ 20 4) 3))"
    "1 + 20 / 4 % 3";
  check_expr "nested parens collapse" "7" "((7))";
  (* every operator at once: both levels, both associativities, one derivation *)
  check_expr "all five operators in one expression"
    "(- (+ 1 2) (% (/ (* 3 4) 5) 6))" "1 + 2 - 3 * 4 / 5 % 6";
  check_expr "a long chain stays left-leaning" "(+ (+ (+ 1 2) 3) 4)"
    "1 + 2 + 3 + 4"
...
let test_expr_calls () =
  ...
  check_expr "an argument may start with '('" "(print (* (+ 1 2) 3))"
    "print((1 + 2) * 3)";
...
let test_expr_errors () =
  ...
  Alcotest.(check bool)
    "trailing operator is reported" true
    (ndiags_expr "1 +" >= 1);
  Alcotest.(check bool)
    "missing ')' is reported" true
    (ndiags_expr "print(1" >= 1)
...
let test_expr_spans () =
  ...
  | Ast.Binary (_, l, r, sp) ->
      Alcotest.(check int)
        "a binary starts where its LEFT operand starts" (lo_col l)
        sp.Token.lo.Token.col;
      Alcotest.(check int)
        "...and ends where its RIGHT operand ends" (hi_col r)
  ...
  | Ast.Binary (_, Ast.Unary (_, _, us), _, _) ->
      Alcotest.(check int)
        "unary minus starts at the '-'" 1 us.Token.lo.Token.col
  ...
  | Ast.Call (_, _, sp) ->
      ...
      Alcotest.(check int)
        "...and covers the closing paren" 13 sp.Token.hi.Token.col
...
let test_stmt_kinds () =
  ...
  | _ ->
      Alcotest.fail
        "expected an assignment (one token of lookahead past the ident)");
  ...
  | Ast.Expr_stmt (e, _) ->
      Alcotest.(check string)
        "bare expression statement" "(+ 1 2)" (Ast.dump_expr e)
...
let test_stmt_errors () =
  ...
  Alcotest.(check bool)
    "missing name is reported" true
    (ndiags_stmt "let = 1" >= 1);
  Alcotest.(check bool)
    "missing '=' is reported" true
    (ndiags_stmt "let a 1" >= 1)
...
let test_program_blank_lines () =
  Alcotest.(check int)
    "blank and comment-only lines are not statements" 2
    (List.length
       (prog "\n// just a comment\n\nlet a = 1\n\nprint(a)\n").Ast.stmts);
  Alcotest.(check int)
    "a last line without a newline still counts" 1
  ...
  Alcotest.(check int)
    "a last line without a newline is CLEAN" 0 (ndiags "print(1)");
  Alcotest.(check int)
    "...also after several statements" 0
    (ndiags "let a = 1\nvar b = 2\nprint(a + b)");
  ...
  Alcotest.(check (list string))
    ...
    (dumps "\n\n\nlet a = 1\n\n\n\nprint(a)\n\n");
  Alcotest.(check int)
    "an empty file is an empty program" 0
    (List.length (prog "").Ast.stmts);
...
let rejects name src =
  Alcotest.(check bool)
    (Printf.sprintf "%s: %S is rejected" name src)
    true
    (ndiags src >= 1)
...
let test_program_rejects () =
  ...
  Alcotest.(check int)
    "a valid program is still clean" 0
    (ndiags "let a = 1\nvar b = a * 2\nb = b - 1\nprint(b)\n")
...
let test_program_recovery () =
  ...
  Alcotest.(check bool)
    "the bad line is reported" true
    (List.length (Diagnostics.all d) >= 1);
  Alcotest.(check bool)
    "parsing continued to the end" true
    (List.length prog.Ast.stmts >= 2)

let test_program_errors () =
  Alcotest.(check int)
    "a valid program is clean" 0
    (ndiags "let a = 1\nprint(a)\n");
  (* two statements on one line: the program layer is what notices *)
  Alcotest.(check bool)
    "missing statement separator is reported" true
    (ndiags "1 2\n" >= 1)
...
let () =
  Alcotest.run "parser"
    [
      ...
      ( "expr: precedence",
        [
          Alcotest.test_case "precedence, associativity" `Quick
      ...
      ( "expr: nesting",
        [
          Alcotest.test_case "calls and parens nest" `Quick
            test_expr_nested_calls;
        ] );
      ( "expr: errors",
        [ Alcotest.test_case "a diagnostic is raised" `Quick test_expr_errors ]
      );
      ( "expr: spans",
        [ Alcotest.test_case "spans cover operands" `Quick test_expr_spans ] );
      (* layer 2 — TODO(02b), needs parse_stmt (and your expression parser) *)
      ( "stmt: kinds",
        [ Alcotest.test_case "statement kinds" `Quick test_stmt_kinds ] );
      ( "stmt: errors",
        [ Alcotest.test_case "malformed bindings" `Quick test_stmt_errors ] );
      (* layer 3 — TODO(02c), the only group that genuinely needs parse_program *)
      ( "program: many statements",
        [ Alcotest.test_case "several statements" `Quick test_program_multi ] );
      ( "program: blank lines",
        [
          Alcotest.test_case "newlines and eof" `Quick test_program_blank_lines;
        ] );
      ( "program: errors",
        [ Alcotest.test_case "statement separators" `Quick test_program_errors ]
      );
      ( "program: recovery",
        [
          Alcotest.test_case "reports and keeps going" `Quick
            test_program_recovery;
        ] );
      ( "program: rejects",
        [
          Alcotest.test_case "malformed is reported" `Quick test_program_rejects;
        ] );
    ]
