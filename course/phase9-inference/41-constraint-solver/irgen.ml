...
let emit_llvm ?(should_emit_terminator = fun _ -> true) (sil_module : Sil.modul)
  ...
  let gen_func (func : Sil.func) =
    ...
    let type_of value = llvm_type (value_type value) in
      ...
      | Store (v, a) ->
          emit
            (Printf.sprintf "  store %s %s, ptr %s\n" (type_of v)
               (lookup_operand v) (lookup_operand a))
      | Binop (op, v1, v2) -> gen_binop value op v1 v2
      | Unop (Ast.Neg, v) when value_type v = Types.TDouble ->
          let t = fresh_temp () in
          bind_operand value t;
          emit
            (Printf.sprintf "  %s = fneg double 0, %s\n" t (lookup_operand v))
      | Unop (Ast.Neg, v) ->
          let t = fresh_temp () in
          bind_operand value t;
          emit (Printf.sprintf "  %s = sub i64 0, %s\n" t (lookup_operand v))
      | Func_ref name -> bind_operand value ("@" ^ name)
      | Apply (f, args) ->
          let t = fresh_temp () in
          bind_operand value t;
          ...
          let args =
            List.map
              (fun v -> Printf.sprintf "%s %s" (type_of v) (lookup_operand v))
              args
            |> String.concat ", "
          in
          if value_type value = Types.TVoid then
            emit (Printf.sprintf "  call void %s(%s)\n" (lookup_operand f) args)
          else
            emit
              (Printf.sprintf "  %s = call %s %s(%s)\n" t ty (lookup_operand f)
                 args)
      | Print v -> gen_print v
    ...
    let gen_term (terminator : Sil.term) =
      ...
      | Sil.Br blk -> emit (Printf.sprintf "  br label %%bb%d\n" blk)
      | Sil.Cond_br (v, then_blk, else_blk) ->
          emit
            (Printf.sprintf "  br i1 %s, label %%bb%d, label %%bb%d\n"
               (lookup_operand v) then_blk else_blk)
      | Sil.Return None when is_main -> emit "  ret i32 0\n"
      | Sil.Return None -> emit "  ret void\n"
      | Sil.Return (Some v) ->
          emit
