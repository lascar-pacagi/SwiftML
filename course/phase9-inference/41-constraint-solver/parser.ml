(* Parser — concept 07 (skeleton). Carries 05–06; you write the TODO(07) holes: the `return`
   statement, parse_params, and parse_func. Reference: solution/parser.ml. *)
...
and parse_stmt (parser : t) : Ast.stmt =
  ...
  | Token.Kw_return -> begin
      let keyword = advance parser in
      match peek_kind parser with
      | Token.(RBrace | Newline | Eof) -> Ast.Return (None, keyword.Token.span)
      | _ -> 
          let expression = parse_expr parser in
          Ast.Return (Some expression, keyword.Token.span)   
  end
...
let parse_params (parser : t) : Ast.param list =
  ...
  let rec param_list accumulator =
    match peek_kind parser with 
    | Token.Ident id1 -> 
        ignore (advance parser);
        let pname = 
          match peek_kind parser with
          | Token.Ident id2 -> ignore (advance parser); id2
          | _ -> id1
        in
        expect parser Token.Colon "':'" |> ignore;
        let ptype, _ = parse_ident_ty parser "a parameter type" in
        let accumulator = Ast.{ pname; ptype } :: accumulator in
        if peek_kind parser = Token.Comma then begin 
          ignore (advance parser); 
          (match peek_kind parser with
          | Token.Ident _ -> () 
          | _ ->
          ...
          param_list accumulator
        end else accumulator
    | Token.RParen -> accumulator
    | _ ->
        Diagnostics.error parser.diagnostics (peek parser).Token.span
          "expected a parameter name";
        accumulator
  in
  expect parser Token.LParen "'('" |> ignore;
  let params = param_list [] |> List.rev in 
  expect parser Token.RParen "')'" |> ignore;
  params

(* `func name ( params ) [ -> Type ] { body }` *)
let parse_func (parser : t) : Ast.func_decl =
  let func = advance parser in
  let fname, _ = parse_ident parser "a function name" in
  let params = parse_params parser in
  let ret =
    if peek_kind parser = Token.Arrow then
      (ignore (advance parser); Some (parse_ident_ty parser "a return type" |> fst))
    else
      None
  in
  let body = parse_block parser in
  Ast.{ fname; params; ret; body; fspan = func.Token.span }
  (* (* ignore parser; *)
  ignore parse_params; *)
