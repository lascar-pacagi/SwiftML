(* Lexer — concept 07 (skeleton). Carries 05–06; you add the '->' arrow (the TODO(07) hole in
   the '-' case). Reference: solution/lexer.ml. *)
...
let rec next (lexer : t) : Token.t =
  ...
  if at_end lexer then make_token lo lexer Token.Eof
    ...
    if is_digit c then (
      ...
      | '-' ->
          ignore (advance_char lexer);
          (* TODO(07): peek for '>' to emit Token.Arrow (the func return arrow); else Minus. *)
          if peek_char lexer = '>' then (
            ignore (advance_char lexer);
            make_token lo lexer Token.Arrow)
          else make_token lo lexer Token.Minus
      | '*' ->
          ignore (advance_char lexer);
