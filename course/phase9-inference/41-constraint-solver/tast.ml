(* The TYPE-CHECKED AST for concept 41 — a *contract* (given). Concept 07's, with one field
   added: a resolved call records WHICH overload was chosen, because in this concept a name no
   longer identifies a function on its own.
...
let rec dump_expr (x : expr) : string =
  ...
  | Fn_call (f, chosen, args) ->
      (* the overload's index goes in the dump: `f#1` is the second declaration of `f`, and
         seeing it is the only way to check that resolution picked what you meant *)
      Printf.sprintf "(%s#%d %s : %s)" f chosen
        (String.concat " " (List.map dump_expr args))
        t
