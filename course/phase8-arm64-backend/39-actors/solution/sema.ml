...
let check (prog : Ast.program) (diags : Diagnostics.sink) : unit =
  ...
  let rec infer (e : Ast.expr) : Types.ty =
    ...
    | Ast.Method_call (e0, m, args, span) -> (
        ...
        | Types.TClass cn -> (
            (* concept 39 — ACTOR ISOLATION: an actor serializes access to its state, so calling
               an actor's instance method from a nonisolated context (outside the actor, not under
               `await`) is a compile error — the access must `await` to hop onto the actor's
