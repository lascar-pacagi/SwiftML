...
let next (lx : t) : Token.t =
  ...
  if at_end lx then make lo lx Token.Eof
    ...
    if is_digit c then (
      ...
      | '!' ->
          ignore (bump lx);
          if peek_char lx = '=' then (ignore (bump lx); make lo lx Token.Ne) else make lo lx Token.Bang
      | '?' -> ignore (bump lx); make lo lx Token.Question (* optional type / coalescing — concept 13 *)
      | '@' -> ignore (bump lx); make lo lx Token.At (* attributes (`@escaping`) — concept 29 *)
      | '#' -> ignore (bump lx); make lo lx Token.Hash (* macros (`#line`, `#assert`) — concept 40 *)
      | '[' -> ignore (bump lx); make lo lx Token.LBracket
      | ']' -> ignore (bump lx); make lo lx Token.RBracket
      | '<' ->
          ignore (bump lx);
      ...
      | '&' ->
          ...
          else failwith (Printf.sprintf "lex error %d:%d: expected '&' after '&'" lx.line lx.col)
      | '|' ->
          ignore (bump lx);
