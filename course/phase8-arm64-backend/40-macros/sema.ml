...
let check (prog : Ast.program) (diags : Diagnostics.sink) : unit =
  ...
  (* concept 39: the set of ACTOR names, and whether we're directly under an `await` (where a hop
     onto an actor's executor is allowed) *)
  let actors : (string, unit) Hashtbl.t = Hashtbl.create 8 in
  List.iter
    (function Ast.IClass c when c.Ast.is_actor -> Hashtbl.replace actors c.Ast.cname () | _ -> ())
    prog.Ast.items;
  ...
  let rec infer (e : Ast.expr) : Types.ty =
    ...
    (* `fatalError()` — a builtin that traps; typed Void (concept 40, target of `#assert`) *)
    | Ast.Call ("fatalError", _, _) -> Types.TVoid
    ...
    | Ast.Method_call (e0, m, args, span) -> (
        ...
        | Types.TClass cn -> (
            (* ACTOR ISOLATION (concept 39 — GIVEN, carried forward): an actor serializes access to
               its state, so calling an actor's instance method from a nonisolated synchronous
               context is a compile error — the call must `await` to hop onto the actor's executor.
               Inside the actor's own methods (`current_class = Some cn`) access is synchronous. *)
    ...
    (* `await e` — concept 38: a suspension point. Transparent for typing (the awaited value's
       type is the expression's type); the `await` keyword marks where a task may suspend. *)
    | Ast.Await (e0, _) ->
        (* `await` permits a hop onto an actor's executor — its operand is checked in an
           await context, where cross-actor access is allowed (concept 39) *)
        let saved = !in_await in
        in_await := true;
        let t = infer e0 in
        in_await := saved;
        t
    ...
    | Ast.Coalesce (a, b, span) -> (
        ...
        | t ->
            ...
            ignore (infer b);
            t)
    (* ternary `c ? a : b` — the condition is Bool, the two arms must agree on a type, which is the
       result type (concept 08's value-diamond, generalized from `&&`/`||`) *)
    | Ast.Ternary (c, a, b, _) ->
      ...
      | Ast.IClass c ->
          ...
          | Some body ->
              check_func ~class_of:c.Ast.cname
                { Ast.fname = "deinit"; generics = []; params = []; throws = false; is_async = false; ret = None; body; fspan = c.Ast.cspan }
          | None -> ());
