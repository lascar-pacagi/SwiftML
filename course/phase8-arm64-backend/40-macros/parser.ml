...
let rec parse_type_name (p : t) (what : string) : string =
  (* `@escaping` (and any other attribute) is parsed and ACCEPTED — v0 does not track
     escaping-ness, so we are more permissive than swiftc: programs it rejects for capturing
     a non-escaping parameter compile here (documented divergence; the check is an exercise) *)
  while peek_kind p = Token.At do
    ignore (advance p);
    ignore (parse_ident p "an attribute name")
  done;
  (* an ARRAY type `[T]` (concept 31) — encoded canonically as "[T]" *)
  if peek_kind p = Token.LBracket then begin
    ignore (advance p);
    let el = parse_type_name p "an element type" in
    ignore (expect p Token.RBracket "']'");
    "[" ^ el ^ "]"
  end
  else
  (* a FUNCTION type `(T1, T2) -> R` (concept 29) — encoded canonically as "(T1,T2)->R" so
     the string-based written-type pipeline still works; resolvers split it back apart with
     Types.split_fn_written *)
  if peek_kind p = Token.LParen then begin
    ignore (advance p);
    let rec params acc =
      if peek_kind p = Token.RParen then (ignore (advance p); List.rev acc)
      else
        let t = parse_type_name p "a parameter type" in
        if peek_kind p = Token.Comma then (ignore (advance p); params (t :: acc))
        else (ignore (expect p Token.RParen "')'"); List.rev (t :: acc))
    in
    let ps = params [] in
    ignore (expect p Token.Arrow "'->'");
    let ret = parse_type_name p "a return type" in
    Printf.sprintf "(%s)->%s" (String.concat "," ps) ret
  end
  else
...
(* `( name: Type, … )` for closure literals — explicit types required (v0) *)
let parse_closure_params (p : t) : Ast.param list =
  if peek_kind p <> Token.LParen then []
    ...
    if peek_kind p = Token.RParen then (ignore (advance p); [])
      ...
      let rec loop acc =
        let pname =
          match peek_kind p with
          | Token.Ident n -> ignore (advance p); n
          | _ ->
              Diagnostics.error p.diags (peek p).Token.span "expected a parameter name";
              "_"
        in
...
let rec parse_expr_bp (p : t) (min_bp : int) : Ast.expr =
  let lhs =
    ...
    (* `[a, b, c]` — an array literal (concept 31) *)
    | Token.LBracket ->
        let lb = advance p in
        if peek_kind p = Token.RBracket then (let rb = advance p in Ast.Array_lit ([], span_between lb.Token.span rb.Token.span))
        else
          let rec elems acc =
            let e = parse_expr_bp p 0 in
            if peek_kind p = Token.Comma then (ignore (advance p); elems (e :: acc))
            else (let rb = expect p Token.RBracket "']'" in (List.rev (e :: acc), rb))
          in
          let es, rb = elems [] in
          Ast.Array_lit (es, span_between lb.Token.span rb.Token.span)
    (* `try` / `try?` / `try!` — a prefix on the throwing call expression (concept 30) *)
    | Token.Kw_try ->
        let t = advance p in
        let kind =
          match peek_kind p with
          | Token.Question -> ignore (advance p); Ast.TryOptional
          | Token.Bang -> ignore (advance p); Ast.TryForce
          | _ -> Ast.TryPlain
        in
        let e = parse_expr_bp p 0 in
        Ast.Try (kind, e, span_between t.Token.span (Ast.expr_span e))
    (* `#name` or `#name(args)` — a macro expansion (concept 40) *)
    | Token.Hash ->
        let h = advance p in
        let name, _ = parse_ident p "a macro name" in
        let args =
          if peek_kind p = Token.LParen then (
            ignore (advance p);
            if peek_kind p = Token.RParen then (ignore (advance p); [])
              ...
              let rec go acc =
                let e = parse_expr_bp p 0 in
                if peek_kind p = Token.Comma then (ignore (advance p); go (e :: acc))
                else (ignore (expect p Token.RParen "')'"); List.rev (e :: acc))
              in
              go [])
          else []
        in
        Ast.MacroExpr (name, args, h.Token.span)
    (* `await e` — a prefix marking a suspension point (concept 38) *)
    | Token.Kw_await ->
        let t = advance p in
        let e = parse_expr_bp p 0 in
        Ast.Await (e, span_between t.Token.span (Ast.expr_span e))
    (* `{ (x: Int) -> Int in expr }` — a closure literal (concept 29) *)
    | Token.LBrace ->
        let lb = advance p in
        ...
        let body = parse_expr_bp p 0 in
        let rb = expect p Token.RBrace "'}'" in
        Ast.Closure (params, ret, body, span_between lb.Token.span rb.Token.span)
    (* `super.init(args)` — the delegating initializer call in a subclass init (concept 25) *)
    | Token.Kw_super ->
        let t = advance p in
        ignore (expect p Token.Dot "'.'");
        (match peek_kind p with
        | Token.Kw_init ->
            ignore (advance p);
            ...
            let args = parse_call_args p in
            let rp = expect p Token.RParen "')'" in
            Ast.Method_call (Ast.Var ("super", t.Token.span), "init", args, span_between t.Token.span rp.Token.span)
        | _ ->
            Diagnostics.error p.diags (peek p).Token.span "only 'super.init(...)' is supported in this subset";
            Ast.Var ("super", t.Token.span))
    | Token.Minus ->
        let t = advance p in
  ...
  (* ternary `c ? a : b` is the LOWEST-precedence operator (below `||`), right-associative *)
  let ternary_bp = 2 in
  let rec loop lhs =
    ...
    if peek_kind p = Token.Question && peek_kind_at p 1 = Token.Question && coalesce_bp >= min_bp then (
      ...
      (* a single `?` in infix position is the ternary: parse the then-arm, the `:`, the else-arm
         (right-associative, so the else-arm is parsed at ternary_bp) *)
      ignore (advance p);
...
and parse_postfix (p : t) (e : Ast.expr) : Ast.expr =
  if peek_kind p = Token.LBracket then (
    ignore (advance p);
    let idx = parse_expr_bp p 0 in
    let rb = expect p Token.RBracket "']'" in
    parse_postfix p (Ast.Subscript (e, idx, span_between (Ast.expr_span e) rb.Token.span)))
  else
...
and parse_stmt (p : t) : Ast.stmt =
  match peek_kind p with
  (* `Task { … }` — spawn a concurrent task (concept 38). `Task` is a contextual identifier. *)
  | Token.Ident "Task" when peek_kind_at p 1 = Token.LBrace ->
      let kw = advance p in
      let body = parse_block p in
      Ast.Spawn (body, kw.Token.span)
  ...
  | Token.Kw_for ->
      ...
      if peek_kind p = Token.DotDotLt then (
        ignore (advance p);
        let hi = parse_expr p in
        let body = parse_block p in
        Ast.For { var; lo = first; hi; body; span = kw.Token.span })
      else
        (* `for x in <sequence>` — iterate an array (concept 31) *)
        let body = parse_block p in
        Ast.For_in { var; seq = first; body; span = kw.Token.span }
  ...
  | Token.Kw_throw ->
      let t = advance p in
      let e = parse_expr_bp p 0 in
      Ast.Throw (e, span_between t.Token.span (Ast.expr_span e))
  | Token.Kw_defer ->
      let t = advance p in
      let body = parse_block p in
      Ast.Defer (body, t.Token.span)
  | Token.Kw_do ->
      let t = advance p in
      let body = parse_block p in
      (* a `catch` may sit on the next line; peek PAST newlines, but only CONSUME them when a
         catch actually follows (else the statement separator after the do is left intact) *)
      let rec catches acc =
        let saved = p.pos in
        while peek_kind p = Token.Newline do ignore (advance p) done;
        if peek_kind p <> Token.Kw_catch then (p.pos <- saved; List.rev acc)
        else begin
          ignore (advance p (* catch *));
          (* `catch E.case` -> a value pattern; bare `catch` -> matches any *)
          let cpat =
            match peek_kind p with
            | Token.LBrace -> None
            | Token.Ident en ->
                ignore (advance p);
                ignore (expect p Token.Dot "'.'");
                let cs, _ = parse_ident p "a case name" in
                Some (en, cs)
            | _ ->
                Diagnostics.error p.diags (peek p).Token.span "expected a catch pattern or '{'";
                None
          in
          let cbody = parse_block p in
          catches ({ Ast.cpat; cbind = None; cbody } :: acc)
        end
      in
      Ast.Do { body; catches = catches []; span = t.Token.span }
  | Token.Ident name when peek_kind_at p 1 = Token.LBracket ->
      (* `a[i] = e` (subscript assign) or a bare `a[i]` expression statement (concept 31) *)
      ignore (advance p); ignore (advance p (* [ *));
      let index = parse_expr_bp p 0 in
      let rb = expect p Token.RBracket "']'" in
      if peek_kind p = Token.Eq then (
        ignore (advance p);
        let value = parse_expr p in
        Ast.Set_subscript { arr = name; index; value; span = span_between rb.Token.span (Ast.expr_span value) })
      else
        let e = parse_postfix p (Ast.Subscript (Ast.Var (name, rb.Token.span), index, rb.Token.span)) in
...
let parse_func (p : t) : Ast.func_decl =
  ...
  (* `async` then `throws` sit between the parameter list and the arrow (concept 38, 30) *)
  let is_async = if peek_kind p = Token.Kw_async then (ignore (advance p); true) else false in
  let throws = if peek_kind p = Token.Kw_throws then (ignore (advance p); true) else false in
  let ret =
  ...
  let body = parse_block p in
  { Ast.fname; generics = !generics; params; throws; is_async; ret; body; fspan = kw.Token.span }
...
let parse_class (p : t) : Ast.class_decl =
  ...
  in
  ignore (expect p Token.LBrace "'{'");
  let fields = ref [] and init = ref None and methods = ref [] and dei = ref None in
  let rec loop () =
    while peek_kind p = Token.Newline do ignore (advance p) done;
    match peek_kind p with
    ...
    | Token.Kw_init ->
        ...
        init := Some { Ast.fname = "init"; generics = []; params; throws = false; is_async = false; ret = None; body; fspan = ikw.Token.span };
        (match peek_kind p with Token.Newline -> ignore (advance p) | _ -> ());
        loop ()
    | Token.Kw_deinit ->
  ...
  in
  loop ();
  { Ast.cname; csuper; cfields = List.rev !fields; cinit = !init; cdeinit = !dei; cmethods = List.rev !methods;
    is_actor = (kw.Token.kind = Token.Kw_actor); cspan = kw.Token.span }
...
let parse_enum (p : t) : Ast.enum_decl =
  ...
  let conf = if peek_kind p = Token.Colon then (ignore (advance p); Some (fst (parse_ident p "a raw/protocol type"))) else None in
  let is_error = conf = Some "Error" in
  let eraw = match conf with Some "Error" -> None | other -> other in
  ignore (expect p Token.LBrace "'{'");
  let parse_payload () =
    if peek_kind p <> Token.LParen then []
    else (
  ...
  in
  let ecases = loop [] in
  { Ast.ename; ecases; eraw; is_error; espan = kw.Token.span }
