...
(* the loop stack, innermost first. `break` and `continue` read it directly — they need the scope
   depth out of the entry as well as a block id — but pushing and popping go through here, so the
   two targets are NAMED at the call site, and the depth is recorded for them. They are not the
   same block: `continue` on a `for` must reach the latch that steps the counter, not the header
   that tests it. *)
let enter_loop (b : builder) ~(continue_to : int) ~(break_to : int) : unit =
  b.loops <- (continue_to, break_to, List.length b.scopes) :: b.loops
...
let rec gen_expr (b : builder) (e : Ast.expr) : Sil.value =
  ...
  (* SHORT-CIRCUIT `&&` / `||` — concept 6. These are NOT bitwise: the right operand must only be
     evaluated when the left does not already decide the result, so its side effects (a trapping
     `a[i]` in `i < n && a[i] > 0`, a force-unwrap, a throwing call) never run on the short path.
     We lower to control flow: evaluate `l`, branch, evaluate `r` only on the deciding edge, and
     merge the result through a stack slot (mem2reg promotes it to a phi). *)
  | Ast.Binary ((Ast.And | Ast.Or) as op, l, r, _) ->
      ...
      (* `&&`: if lv is true, evaluate rhs (else short-circuit to false = lv).
         `||`: if lv is true, short-circuit to true = lv (else evaluate rhs). *)
      let t_tgt, f_tgt =
  ...
  | Ast.Call ("fatalError", _, _) ->
      ...
      emit b (Sil.Int_lit 0) Types.TInt
  (* macros are expanded before SILGen — a leftover one is a compiler bug *)
  | Ast.MacroExpr _ -> failwith "SILGen: unexpanded macro (should have been expanded before sema)"
  | Ast.Call (f, args, _) ->
  ...
  | Ast.Member (e0, fld, _) -> (
      ...
      | _ -> assert false)
  (* `await e` — concept 38: a suspension point; transparent at runtime (suspension happens at the
...
and build_array (b : builder) (et : Types.ty) (vs : Sil.value list) : Sil.value =
  ...
  List.iter
    (fun v ->
      let fr = emit b (Sil.Func_ref "rt.array_push") Types.TVoid in
      emit_void b (Sil.Apply (fr, [ arr; v ])))
    vs;
  arr
...
and gen_array_hof (b : builder) (src : Sil.value) (el : Types.ty) (m : string)
  ...
  let loop ~(body_fill : Sil.value -> unit) =
    ...
    emit_void b (Sil.Store (inc, iaddr));
    terminate b (Sil.Br (header.Sil.bid, []));
    switch_to b exit_b
  in
  match (m, args) with
...
and gen_stmt (b : builder) (s : Ast.stmt) : unit =
  ...
  | Ast.Let { name; annot; value; _ } ->
      ...
      | Types.TArray _, Ast.Var _ ->
          emit_void b (Sil.Apply (emit b (Sil.Func_ref "rt.array_retain") Types.TVoid, [ v ]))
      | _ -> ());
  ...
  | Ast.Set_subscript { arr; index; value; _ } ->
      ...
      emit_void b (Sil.Apply (fr, [ uq; iv; xv ]))
  (* `Task { … }` — concept 38: spawn a concurrent task. The body becomes a lifted void function
  ...
  | Ast.Spawn (body, _) ->
      ...
      emit_void b (Sil.Apply (fr, [ clo ]));
      (* the task conceptually takes ownership of the closure; v0 task bodies are capture-free, so
         the (empty) context can be released here — balances the owned closure for the verifier *)
      emit_void b (Sil.Destroy_value clo)
  ...
  | Ast.For_in { var; seq; body; _ } ->
      ...
      emit_void b (Sil.Store (xv, xaddr));
      enter_loop b ~continue_to:latch.Sil.bid ~break_to:exit_b.Sil.bid;
      gen_block b body;
      terminate b (Sil.Br (latch.Sil.bid, []));
      leave_loop b;
...
let lower (prog : Ast.program) : Sil.modul =
  ...
  let rec ty_of_name n =
    ...
    | None ->
    (* a written ARRAY type `[T]` — concept 31. (Was missing here, so a function returning/taking
       `[Int]` resolved its signature to TInt and miscompiled — review fix.) *)
  ...
  List.iter
    (function
      ...
      (* throwing METHODS (class/struct) need an error check at their call sites too — concept 30.
         Keyed by `method:<name>` so it can never collide with a bare function name; looked up by
         bare method name at the Method_call sites (dynamic dispatch can't change throws-ness). *)
      | Ast.IClass c ->
        ...
        | Ast.IClass c ->
            ...
            let dtor =
              lower_m ~epilogue:body_epilogue
                { Ast.fname = "deinit"; generics = []; params = []; throws = false; is_async = false; ret = None;
                  body = Option.value c.Ast.cdeinit ~default:[]; fspan = c.Ast.cspan }
            in
            let destroyer =
              lower_m ~epilogue:destroy_epilogue
                { Ast.fname = "destroy"; generics = []; params = []; throws = false; is_async = false; ret = None; body = []; fspan = c.Ast.cspan }
            in
            (dtor :: destroyer
  ...
  (* concept 38: after top-level code, drain the cooperative executor — run any spawned tasks to
     completion (resuming each at its yields) before the program exits *)
  let async_epilogue (b : builder) =
    let fr = emit b (Sil.Func_ref "rt_async_run") Types.TVoid in
    emit_void b (Sil.Apply (fr, []))
  in
  let main = lower_func ~epilogue:async_epilogue ~lifted ~throwing ~error_ord structs enums protos methods funcs gfuncs classes "main" [] Types.TVoid main_body in
  let struct_layouts =
