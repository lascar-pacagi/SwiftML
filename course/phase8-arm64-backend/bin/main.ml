(* The `swiftml8` CLI — Phase 8 (the from-scratch ARM64 backend). Backend A (LLVM) stays; Backend B
   (native ARM64) arrives:
     swiftml8 build <file.swift> [-O] [--native] [-o <out>]   (--native = Backend B: our ARM64)
     swiftml8 --emit-asm <file.swift>                         (ARM64 assembly text)
     swiftml8 --emit-sil|--sil-opt|--emit-llvm [-O] <file.swift>
     swiftml8 --emit-tokens|--emit-ast|--typecheck <file.swift> *)

let usage () =
  prerr_endline "usage: swiftml8 build <file.swift> [-O] [--native] [-o <out>]";
  prerr_endline "       swiftml8 --emit-asm <file.swift>";
  prerr_endline "       swiftml8 --emit-sil|--sil-opt|--emit-llvm [-O] <file.swift>";
  prerr_endline "       swiftml8 --emit-tokens|--emit-ast|--typecheck <file.swift>";
  exit 2
