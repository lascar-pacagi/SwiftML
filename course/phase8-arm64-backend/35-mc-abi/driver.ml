...
let emit_asm ?(regalloc = Regalloc.Graphcolor) (src : string) (diags : Diagnostics.sink) : string =
  let funcs, cstrings = Isel.select (lower_sil ~opt:false src diags) in
  Arm64.string_of_module (Regalloc.run_module regalloc funcs cstrings)
...
let compile_file ?(out = "a.out") ?(opt = false) ?(native = false)
    ?(regalloc = Regalloc.Graphcolor) ~(src_path : string) ~(emit : emit) () : unit =
  let src = read_file src_path in
  let diags = Diagnostics.create () in
  match emit with
  | Asm -> print_string (emit_asm ~regalloc src diags)
  | Tokens ->
  ...
  | Exe when native ->
      ...
      let asm = emit_asm ~regalloc src diags in
