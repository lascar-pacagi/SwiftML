...
let emit_asm ?(regalloc = Regalloc.Graphcolor) ?(peephole = true) ?(source_path = "") (src : string)
    (diags : Diagnostics.sink) : string =
  let funcs, cstrings = Isel.select (lower_sil ~opt:false src diags) in
  (* concept 37: name the source file so the `.loc` directives form a DWARF line table *)
  let m = Regalloc.run_module ~source:source_path regalloc funcs cstrings in
...
let run_clang_asm ~(asm_path : string) ~(out : string) : unit =
  (* clang drives the assembler (`as`) + linker (`ld`) on our .s, linking libc for printf. `-g`
     turns our `.file`/`.loc` directives into a real DWARF line table — concept 37.
     macOS keeps DWARF in the object file and leaves a "debug map" in the executable pointing to it,
     so we assemble to a kept `<out>.o` and then link it; `lldb` follows the map to step by source
     line. (A `clang -g file.s -o exe` would delete the temp .o and lose the DWARF.) *)
  let obj = out ^ ".o" in
  let asm = Printf.sprintf "clang -g -c %s -o %s" (Filename.quote asm_path) (Filename.quote obj) in
  let link = Printf.sprintf "clang -g %s -o %s" (Filename.quote obj) (Filename.quote out) in
  if Sys.command asm <> 0 then failwith (Printf.sprintf "clang (assembler) failed on %s" asm_path);
  if Sys.command link <> 0 then failwith (Printf.sprintf "clang (linker) failed on %s" obj)
...
let compile_file ?(out = "a.out") ?(opt = false) ?(native = false)
  ...
  | Exe when native ->
      ...
      let asm = emit_asm ~regalloc ~peephole ~source_path:src_path src diags in
