...
type instr =
  ...
  (* closures — concept 29. A function VALUE is thick: { code ptr, context ptr }. *)
  | Closure of string * value list
    (* build a closure: lifted-function name + the captured VALUES (copied into a fresh
       heap context, refcounted like an object with a no-op destructor — captures are PODs
       in v0). Result: the thick pair. *)
  | Thin_to_thick of string
    (* a plain named function as a value: { its thunk, null context } *)
  | Apply_value of value * value list
    (* indirect call of a thick function: extract code + context, call code(ctx, args) *)
  | Capture_get of value * int
    (* inside a LIFTED closure body: read capture #i from the context parameter *)
  (* classes — concept 25 *)
...
type func = {
  ...
  lines : (value, int) Hashtbl.t; (* the SOURCE LINE each value's instruction came from — concept 37 *)
}
...
type modul = {
  ...
  closures : (string * (string * Types.ty) list) list;
    (* concept 29: each lifted closure's CONTEXT layout — (lifted fn name, captures (name, ty)).
       IRGen sizes/indexes the heap context from this. *)
...
let string_of_instr (f : func) ((v, i) : value * instr) : string =
  ...
  | Closure (fn, caps) ->
      Printf.sprintf "%s = closure @%s (%s) %s" r fn
        (String.concat ", " (List.map (Printf.sprintf "%%%d") caps))
        (ty ())
  | Thin_to_thick fn -> Printf.sprintf "%s = thin_to_thick @%s %s" r fn (ty ())
  | Apply_value (fv, args) ->
      Printf.sprintf "%s = apply_value %%%d(%s) %s" r fv
        (String.concat ", " (List.map (Printf.sprintf "%%%d") args))
        (ty ())
  | Capture_get (ctx, i) -> Printf.sprintf "%s = capture_get %%%d, #%d %s" r ctx i (ty ())
...
let is_class_value (f : func) (v : value) : bool =
  match Hashtbl.find_opt f.val_ty v with
  | Some (Types.TClass _ | Types.TFunc _) (* function values are managed — concept 29 *) -> true
  | _ -> false
...
let def_is_owned (i : instr) : bool =
  ...
  | Closure _ | Thin_to_thick _ -> true (* a fresh function value owns its context — 29 *)
  | Apply _ | Apply_class _ | Apply_witness _ | Apply_value _ -> true (* results come back +1 *)
