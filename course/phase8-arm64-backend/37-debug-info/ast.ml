(* AST for concept 37 — a *contract*. It carries every node the concepts before it
...
(* the source LINE a statement begins on — for debug info (concept 37) *)
let stmt_line (s : stmt) : int =
  let sp =
    match s with
    | Let { span; _ } | Assign { span; _ } | Set_member { span; _ } | If { span; _ }
    | If_let { span; _ } | While { span; _ } | For { span; _ } | Switch { span; _ }
    | Set_subscript { span; _ } | For_in { span; _ } | Do { span; _ } -> span
    | Expr_stmt (_, span) | Return (_, span) | Break span | Continue span | Throw (_, span)
    | Defer (_, span) -> span
  in
  sp.Token.lo.Token.line
