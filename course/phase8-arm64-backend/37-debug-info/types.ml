(* The type lattice — a *contract*. Concepts 05–06: Int/Bool/Double/String; 07: `TVoid`;
   10/11/13: structs, enums, optionals. Concept 21 adds `TProto` — a PROTOCOL used as a type
   (Swift's existential, `any P`): a value of unknown concrete type that is known to conform
   to P. Function *types* stay implicit (calls check against a signature table) until Phase 7. *)

type ty =
  | TInt
  | TBool
  | TDouble
  | TString
  | TVoid (* () — a value-less result *)
  | TStruct of string (* a value type — concept 10; the name keys the struct registry *)
  | TEnum of string (* a sum type — concept 11; the name keys the enum registry *)
  | TOptional of ty (* `T?` — concept 13; really `enum { none; some(T) }` (the tag is 0/1) *)
  | TProto of string (* `any P` — concept 21; an existential: (value, witness table) *)
  | TFunc of ty list * ty
    (* `(Int, Bool) -> Int` — concept 29: a first-class FUNCTION type. A value is a THICK
       function: { code pointer, context pointer } — the context holds a closure's captures
       (heap-allocated, refcounted like an object; null for a plain named function). *)
  | TArray of ty
    (* `[Int]` / `Array<Int>` — concept 31: a VALUE type backed by a refcounted heap buffer
       (copy-on-write). The element type is monomorphic in v0 (Int). *)
  | TClass of string
    (* `class C` — concept 25: a REFERENCE type. A value is a pointer to a heap object whose
       header carries the vtable pointer (and, from concept 26, the reference count). *)
  | TVar of string * string
    (* `T` inside `func f<T: P>` — concept 22: a TYPE PARAMETER (name, constraint protocol).
       Statically it's an opaque-but-CONSISTENT type; at runtime (unspecialized) a T value
       travels in the same (buffer, witness-table) representation as `any P`. *)

(* the layout of a struct: its fields in declaration order (name, type). *)
type struct_layout = { sl_name : string; sl_fields : (string * ty) list }

(* the layout of an enum: its cases in declaration order, each with its payload types (the
   case index is its tag). el_raw is true for a `: Int` raw-value enum (implicit raws = index). *)
type enum_layout = { el_name : string; el_cases : (string * ty list) list; el_raw : bool }

(* the layout of a class — concept 25. cl_fields is the FULL list including inherited fields,
   in object layout order (superclass prefix first) — which is what makes an upcast free.
   cl_methods is the VTABLE: (name, params, ret) in slot order — inherited slots first, an
   override keeps its slot, new methods append. cl_impls names the implementing function per
   slot (e.g. "Dog.sound" in a slot Animal declared). *)
type class_layout = {
  cl_name : string;
  cl_super : string option;
  cl_fields : (string * ty) list;
  cl_methods : (string * ty list * ty) list;
  cl_impls : string list;
}

(* the layout of a protocol — concept 21: its requirements in declaration order. A requirement
   is a method signature (name, parameter types — self excluded — and return type); its index
   in this list is its WITNESS-TABLE SLOT. *)
type proto_layout = { pl_name : string; pl_reqs : (string * ty list * ty) list }

let rec string_of_ty : ty -> string = function
  | TInt -> "Int"
  | TBool -> "Bool"
  | TDouble -> "Double"
  | TString -> "String"
  | TVoid -> "()"
  | TStruct n -> n
  | TEnum n -> n
  | TOptional t -> string_of_ty t ^ "?"
  | TProto n -> "any " ^ n
  | TClass n -> n
  | TArray e -> "[" ^ string_of_ty e ^ "]"
  | TFunc (ps, r) ->
      Printf.sprintf "(%s) -> %s" (String.concat ", " (List.map string_of_ty ps)) (string_of_ty r)
  | TVar (n, _) -> n

let equal (a : ty) (b : ty) : bool = a = b

let is_numeric : ty -> bool = function
  | TInt | TDouble -> true
  | TBool | TString | TVoid | TStruct _ | TEnum _ | TOptional _ | TProto _ | TClass _ | TFunc _ | TArray _ | TVar _ -> false

(* enum helpers: a case's tag (index) and its payload types *)
let case_index (el : enum_layout) (c : string) : int option =
  let rec go i = function
    | (n, _) :: _ when n = c -> Some i
    | _ :: tl -> go (i + 1) tl
    | [] -> None
  in
  go 0 el.el_cases

let case_payload (el : enum_layout) (c : string) : ty list option =
  Option.map snd (List.find_opt (fun (n, _) -> n = c) el.el_cases)

let max_payload (el : enum_layout) : int =
  List.fold_left (fun m (_, tys) -> max m (List.length tys)) 0 el.el_cases

let has_payload (el : enum_layout) : bool = max_payload el > 0

(* field lookups on a layout *)
let field_index (sl : struct_layout) (f : string) : int option =
  let rec go i = function
    | (n, _) :: _ when n = f -> Some i
    | _ :: tl -> go (i + 1) tl
    | [] -> None
  in
  go 0 sl.sl_fields

let field_type (sl : struct_layout) (f : string) : ty option =
  List.assoc_opt f sl.sl_fields

(* method lookups on a class layout *)
let vslot (cl : class_layout) (m : string) : int option =
  let rec go i = function
    | (n, _, _) :: _ when n = m -> Some i
    | _ :: tl -> go (i + 1) tl
    | [] -> None
  in
  go 0 cl.cl_methods

let vsig (cl : class_layout) (m : string) : (ty list * ty) option =
  List.find_map (fun (n, ps, r) -> if n = m then Some (ps, r) else None) cl.cl_methods

let cfield_index (cl : class_layout) (f : string) : int option =
  let rec go i = function
    | (n, _) :: _ when n = f -> Some i
    | _ :: tl -> go (i + 1) tl
    | [] -> None
  in
  go 0 cl.cl_fields

let cfield_type (cl : class_layout) (f : string) : ty option = List.assoc_opt f cl.cl_fields

(* requirement lookups on a protocol layout *)
let req_index (pl : proto_layout) (m : string) : int option =
  let rec go i = function
    | (n, _, _) :: _ when n = m -> Some i
    | _ :: tl -> go (i + 1) tl
    | [] -> None
  in
  go 0 pl.pl_reqs

let req_sig (pl : proto_layout) (m : string) : (ty list * ty) option =
  List.find_map (fun (n, ps, r) -> if n = m then Some (ps, r) else None) pl.pl_reqs

(* the parser encodes a written function type canonically: "(Int,Bool)->Int" (no spaces;
   nested fn types keep their parens). Resolvers split it here and recurse, using their own
   name resolution for the leaves. *)
let split_fn_written (s : string) : (string list * string) option =
  let n = String.length s in
  if n = 0 || s.[0] <> '(' then None
  else begin
    (* find the matching ')' of the leading '(' *)
    let close = ref (-1) and depth = ref 0 in
    (try
       String.iteri
         (fun i c ->
           if c = '(' then incr depth
           else if c = ')' then begin
             decr depth;
             if !depth = 0 then begin close := i; raise Exit end
           end)
         s
     with Exit -> ());
    if !close < 0 || !close + 2 >= n || String.sub s (!close + 1) 2 <> "->" then None
    else begin
      let inner = String.sub s 1 (!close - 1) in
      let ret = String.sub s (!close + 3) (n - !close - 3) in
      (* split [inner] on top-level commas *)
      let parts = ref [] and start = ref 0 and depth = ref 0 in
      String.iteri
        (fun i c ->
          if c = '(' then incr depth
          else if c = ')' then decr depth
          else if c = ',' && !depth = 0 then begin
            parts := String.sub inner !start (i - !start) :: !parts;
            start := i + 1
          end)
        inner;
      let last = String.sub inner !start (String.length inner - !start) in
      let ps = if inner = "" then [] else List.rev (last :: !parts) in
      Some (ps, ret)
    end
  end

(* the parser encodes an array type `[T]` canonically as "[T]" (brackets kept) — concept 31 *)
let split_array_written (s : string) : string option =
  let n = String.length s in
  if n >= 2 && s.[0] = '[' && s.[n - 1] = ']' then Some (String.sub s 1 (n - 2)) else None

let of_name : string -> ty option = function
  | "Int" -> Some TInt
  | "Bool" -> Some TBool
  | "Double" -> Some TDouble
  | "String" -> Some TString
  | "Void" -> Some TVoid
  | _ -> None
