...
type builder = {
  ...
  lines : (Sil.value, int) Hashtbl.t;            (* debug info: each value's source line — concept 37 *)
  mutable cur_line : int;                        (* the line of the statement being lowered *)
}
...
let emit (b : builder) (instr : Sil.instr) (ty : Types.ty) : Sil.value =
  ...
  Hashtbl.replace b.lines v b.cur_line; (* stamp the value with the current source line — concept 37 *)
  v
...
and gen_stmt (b : builder) (s : Ast.stmt) : unit =
  b.cur_line <- Ast.stmt_line s; (* debug info: stamp subsequent values with this line — concept 37 *)
  match s with
...
and lower_func ?(generic = false) ?(init = false) ?(epilogue : (builder -> unit) option)
  ...
  let b =
    { next_val = 0; next_block = 1; cur = entry; blocks = [ entry ]; vars = Hashtbl.create 16; borrows = Hashtbl.create 4; val_ty; funcs; structs; enums; protos; methods; gfuncs; classes; ret; loops = [];
      ...
      lines = Hashtbl.create 64; cur_line = 0 }
  in
