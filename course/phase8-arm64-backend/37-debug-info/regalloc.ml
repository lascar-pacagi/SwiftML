...
let map_regs (g : Arm64.reg -> Arm64.reg) (i : Arm64.instr) : Arm64.instr =
  ...
  | (B _ | Bcond _ | Bl _ | Brk _ | Ret | Label _ | Comment _ | Loc _) as x -> x
...
let finalize (name : string) (nslots : int) (outgoing : int) (body : Arm64.instr list) (used_cs : int list) : Arm64.func =
  ...
  (* AAPCS64 frame, large-frame-safe (concept 35). PUSH fp/lr first (a small, in-range adjustment),
     set x29 = the saved-fp slot, THEN allocate the locals. So `stp`/`ldp` always use offset 0 even
     when the locals area is huge. x29 points 16 bytes below the incoming sp, so an incoming stack
     argument (8+j)-th sits at [x29, #16 + 8*j]. Locals: value slots [sp, #slot(v)] and the used
     callee-saved registers, all below the new sp. *)
...
let run_module ?(source = "") (strat : strategy) (funcs : Arm64.func list)
    (cstrings : (string * string) list) : Arm64.modul =
  { Arm64.funcs = List.map (run strat) funcs; cstrings; source }
