...
let () =
  ...
  let args = List.filter (fun a ->
      a <> "-O" && a <> "--native" && not (String.length a > 11 && String.sub a 0 11 = "--regalloc=")) args in
  match args with
  | _ :: "build" :: file :: rest ->
      let out =
        match rest with
        ...
        | _ -> usage ()
      in
      Driver.compile_file ~out ~opt ~native ~regalloc ~src_path:file ~emit:Driver.Exe ()
  | _ :: flag :: [ file ] when emit_of_flag flag <> None ->
      Driver.compile_file ~opt ~regalloc ~src_path:file ~emit:(Option.get (emit_of_flag flag)) ()
  | _ -> usage ()
