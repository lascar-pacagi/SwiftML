...
let lower_func (_m : Sil.modul) (f : Sil.func) : Arm64.func * (string * string) list =
  ...
  let sel_instr (vr : Sil.value) (i : Sil.instr) : unit =
    ...
    | Sil.Apply (callee, args) ->
        List.iteri (fun k a -> emit (Arm64.Mov (x k, Arm64.R (v a)))) args;
  ...
  (* incoming params arrive in x0..x7; move each into its virtual register *)
  List.iteri (fun k (pv, _) -> emit (Arm64.Mov (v pv, Arm64.R (x k)))) f.Sil.params;
  let blocks = List.sort (fun (a : Sil.block) b -> compare a.Sil.bid b.Sil.bid) f.Sil.blocks in
  List.iter
    (fun (b : Sil.block) ->
      emit (Arm64.Label (blabel b.Sil.bid));
      List.iter (fun (vv, i) -> sel_instr vv i) (List.rev b.Sil.instrs);
      sel_term b.Sil.term)
    blocks;
  let nslots = !next_tmp in
