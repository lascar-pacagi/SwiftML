(* irgen (given carry-forward) — concept 31 adds Array/String. *)
...
let rec llty : Types.ty -> string = function
  ...
  | Types.TString -> "ptr"
  | Types.TArray _ -> "ptr" (* a pointer to a refcounted heap buffer header — concept 31 *)
  | Types.TVoid -> "void"
...
let emit_llvm (m : Sil.modul) : string =
  ...
  let rec words : Types.ty -> int = function
    ...
    | Types.TArray _ -> 1 (* a pointer — concept 31 *)
    | Types.TVoid -> 0
  ...
  let gen_func (f : Sil.func) =
    ...
    let gen_instr (v, i) =
      ...
      | Sil.Apply_class (o, slot, args) ->
          ...
          in
          let rt = vty v in
          if rt = Types.TVoid then p (Printf.sprintf "  call void %s(ptr %s%s)\n" fn (lookup_operand o) argstr)
          else p (Printf.sprintf "  %s = call %s %s(ptr %s%s)\n" (lookup_operand v) (llty rt) fn (lookup_operand o) argstr)
      | Sil.Upcast (o, _) ->
      ...
      | Sil.Apply_witness (ex, slot, args) ->
          ...
          let argstr =
            String.concat ""
              (List.map (fun a -> Printf.sprintf ", %s %s" (llty (vty a)) (lookup_operand a)) args)
          in
          let rt = vty v in
          if rt = Types.TVoid then p (Printf.sprintf "  call void %s(ptr %s%s)\n" fn buf argstr)
          else p (Printf.sprintf "  %s = call %s %s(ptr %s%s)\n" (lookup_operand v) (llty rt) fn buf argstr)
    in
  ...
  let preamble =
    "; swiftml Phase-2 LLVM IR\n\
     ...
     declare void @rt_async_yield()\n\
     declare void @rt_async_run()\n\
     ...
     @.oob = private unnamed_addr constant [32 x i8] c\"Fatal error: Index out of range\\0A\"\n"
  in
  ...
  let arc_runtime =
    String.concat "\n"
      [ (* error handling — concept 30: a single global error register + its two intrinsics. The
        ...
        "" ]
  in
  (* the Array/String runtime — concept 31. An array is a pointer to a 32-byte header
       { i64 refcount; i64 count; i64 capacity; ptr data }
     (offsets 0/8/16/24). `data` is a separately-malloc'd buffer of `capacity` i64 elements.
     Value semantics come from COPY-ON-WRITE: a binding shares the buffer (retain → refcount++),
     and a mutation first `make_unique`s it (copies iff shared). Strings are libc C-strings. *)
  let stdlib_runtime =
    String.concat "\n"
      [ "";
        (* allocate an empty buffer, refcount 1 *)
        "define private ptr @rt.array_new() {";
        "  %h = call ptr @malloc(i64 32)";
        "  store i64 1, ptr %h";
        "  %cp = getelementptr i8, ptr %h, i64 8";
        "  store i64 0, ptr %cp";
        "  %kp = getelementptr i8, ptr %h, i64 16";
        "  store i64 0, ptr %kp";
        "  %dp = getelementptr i8, ptr %h, i64 24";
        "  store ptr null, ptr %dp";
        "  ret ptr %h";
        "}";
        "define private i64 @rt.array_count(ptr %h) {";
        "  %cp = getelementptr i8, ptr %h, i64 8";
        "  %c = load i64, ptr %cp";
        "  ret i64 %c";
        "}";
        (* push x; grow (double, min 4) when full *)
        "define private void @rt.array_push(ptr %h, i64 %x) {";
        "  %cp = getelementptr i8, ptr %h, i64 8";
        "  %c = load i64, ptr %cp";
        "  %kp = getelementptr i8, ptr %h, i64 16";
        "  %k = load i64, ptr %kp";
        "  %dp = getelementptr i8, ptr %h, i64 24";
        "  %full = icmp eq i64 %c, %k";
        "  br i1 %full, label %grow, label %store";
        "grow:";
        "  %dbl = mul i64 %k, 2";
        "  %z0 = icmp eq i64 %k, 0";
        "  %nk = select i1 %z0, i64 4, i64 %dbl";
        "  %nbytes = mul i64 %nk, 8";
        "  %nd = call ptr @malloc(i64 %nbytes)";
        "  %old = load ptr, ptr %dp";
        "  %hasold = icmp ne ptr %old, null";
        "  br i1 %hasold, label %cpy, label %after";
        "cpy:";
        "  %cbytes = mul i64 %c, 8";
        "  %m1 = call ptr @memcpy(ptr %nd, ptr %old, i64 %cbytes)";
        ...
        "store:";
        "  %d = load ptr, ptr %dp";
        "  %slot = getelementptr i64, ptr %d, i64 %c";
        "  store i64 %x, ptr %slot";
        "  %c1 = add i64 %c, 1";
        "  store i64 %c1, ptr %cp";
        "  ret void";
        "}";
        (* bounds-checked read; out-of-bounds traps (exit 133, like swiftc) *)
        "define private i64 @rt.array_get(ptr %h, i64 %i) {";
        "  %cp = getelementptr i8, ptr %h, i64 8";
        "  %c = load i64, ptr %cp";
        "  %neg = icmp slt i64 %i, 0";
        "  %big = icmp sge i64 %i, %c";
        "  %oob = or i1 %neg, %big";
        "  br i1 %oob, label %trap, label %ok";
        "trap:";
        "  call i64 @write(i32 2, ptr @.oob, i64 32)";
        "  call void @llvm.trap()";
        "  unreachable";
        "ok:";
        "  %dp = getelementptr i8, ptr %h, i64 24";
        "  %d = load ptr, ptr %dp";
        "  %slot = getelementptr i64, ptr %d, i64 %i";
        "  %x = load i64, ptr %slot";
        "  ret i64 %x";
        "}";
        "define private void @rt.array_set(ptr %h, i64 %i, i64 %x) {";
        "  %cp = getelementptr i8, ptr %h, i64 8";
        "  %c = load i64, ptr %cp";
        "  %neg = icmp slt i64 %i, 0";
        "  %big = icmp sge i64 %i, %c";
        ...
        "  ret void";
        "}";
        (* retain: a new sharer of the buffer — refcount++ *)
        "define private void @rt.array_retain(ptr %h) {";
        "  %r = load i64, ptr %h";
        "  %n = add i64 %r, 1";
        "  store i64 %n, ptr %h";
        "  ret void";
        "}";
        (* the copy-on-write CHECK: if the buffer is shared (refcount > 1), release our share and
           return a fresh independent copy; otherwise it's already unique — return it as-is. *)
        ...
        "  %r1 = sub i64 %r, 1";
        "  store i64 %r1, ptr %h";
        "  %nh = call ptr @malloc(i64 32)";
        "  store i64 1, ptr %nh";
        "  %cp = getelementptr i8, ptr %h, i64 8";
        "  %c = load i64, ptr %cp";
        "  %ncp = getelementptr i8, ptr %nh, i64 8";
        "  store i64 %c, ptr %ncp";
        "  %nkp = getelementptr i8, ptr %nh, i64 16";
        "  store i64 %c, ptr %nkp";
        "  %ndp = getelementptr i8, ptr %nh, i64 24";
        "  %bytes = mul i64 %c, 8";
        "  %z = icmp eq i64 %c, 0";
        "  br i1 %z, label %empty, label %dup";
        "dup:";
        "  %nd = call ptr @malloc(i64 %bytes)";
        "  %dp = getelementptr i8, ptr %h, i64 24";
        "  %d = load ptr, ptr %dp";
        "  %m = call ptr @memcpy(ptr %nd, ptr %d, i64 %bytes)";
        "  store ptr %nd, ptr %ndp";
        "  ret ptr %nh";
        "empty:";
        "  store ptr null, ptr %ndp";
        "  ret ptr %nh";
        "}";
        (* String: a libc C-string. count = byte length; concat = malloc + copy both halves. *)
        "define private i64 @rt.str_count(ptr %s) {";
        "  %n = call i64 @strlen(ptr %s)";
        "  ret i64 %n";
        "}";
        "define private ptr @rt.str_concat(ptr %a, ptr %b) {";
        "  %la = call i64 @strlen(ptr %a)";
        "  %lb = call i64 @strlen(ptr %b)";
        "  %sum = add i64 %la, %lb";
        "  %sz = add i64 %sum, 1";
        "  %m = call ptr @malloc(i64 %sz)";
        "  %c1 = call ptr @memcpy(ptr %m, ptr %a, i64 %la)";
        "  %tail = getelementptr i8, ptr %m, i64 %la";
        "  %lb1 = add i64 %lb, 1";
        ...
        "" ]
  in
  preamble ^ arc_runtime ^ stdlib_runtime
  ^ (if type_defs = [] then "" else String.concat "\n" type_defs ^ "\n")
  (* concept 38: declared AFTER the type defs, since it takes a %thickfn ({code,ctx}) by value *)
  ^ "declare void @rt_async_spawn(%thickfn)\n"
