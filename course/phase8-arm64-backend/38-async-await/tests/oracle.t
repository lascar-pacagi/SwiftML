...
must be byte-identical. The eighteen programs are SEQUENTIAL async: `async` functions, `await`
at top level and inside functions, `Task.yield()` in loops and recursion, and async results
flowing through everything the earlier phases built — structs, enums, optionals, `throws`/`do`,
arrays and the HOF trio, a class mutated across a yield, a ternary, strings.

Sequential async is the part `swiftc` can arbitrate. Spawned tasks are NOT here: Swift's default
executor is concurrent and its interleaving is nondeterministic, while ours is a serial FIFO — so
comparing the two would be comparing a documented semantics against an undocumented one. The
interleaving is checked instead in `silgen-spawn.t`, against our own stated round-robin rule.
