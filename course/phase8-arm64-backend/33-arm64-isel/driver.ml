...
(* Backend B (concept 33): lower the RAW (memory-based) SIL to ARM64 assembly. v0 isel doesn't
   handle the optimized SSA (block-arg) form, so the native path always uses -Onone SIL. *)
let emit_asm (src : string) (diags : Diagnostics.sink) : string =
  Arm64.string_of_module (Isel.select (lower_sil ~opt:false src diags))
...
let compile_file ?(out = "a.out") ?(opt = false) ?(native = false) ~(src_path : string)
    ~(emit : emit) () : unit =
  let src = read_file src_path in
  let diags = Diagnostics.create () in
  match emit with
  | Asm -> print_string (emit_asm src diags)
  | Tokens ->
      let toks = Lexer.tokenize (Lexer.create src diags) in
      bail_on_errors diags;
