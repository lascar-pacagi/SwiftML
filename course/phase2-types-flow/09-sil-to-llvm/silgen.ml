...
type builder = {
  ...
  mutable current_block : Sil.block; (* THE CURSOR: `emit` appends here, and `switch_to` moves it *)
  mutable blocks : Sil.block list; (* every block made, newest first; `lower` reverses at the end *)
  variables : (string, Sil.value) Hashtbl.t; (* variable name -> the ADDRESS of its alloc_stack slot *)
  value_types : (Sil.value, Types.ty) Hashtbl.t; (* the type of every value emitted; read with `value_type` *)
  functions : (string, Types.ty list * Types.ty) Hashtbl.t; (* signatures, so a call knows its result *)
...
let emit (builder : builder) (instruction : Sil.instr) (ty : Types.ty) : Sil.value =
...
let terminate (builder : builder) (t : Sil.term) = if builder.current_block.Sil.term = Sil.Unreachable then builder.current_block.Sil.term <- t
...
let emit_void (builder : builder) (instruction : Sil.instr) : unit = ignore (emit builder instruction Types.TVoid)
...
let leave_loop (builder : builder) : unit = builder.loops <- List.tl builder.loops
...
let break_target (builder : builder) : int option =
  match builder.loops with (_, exit_target) :: _ -> Some exit_target | [] -> None

let continue_target (builder : builder) : int option =
  match builder.loops with (continue_target_id, _) :: _ -> Some continue_target_id | [] -> None
...
let restore_variables (builder : builder) (saved_variables : (string, Sil.value) Hashtbl.t) : unit =
  ...
  Hashtbl.iter (fun k value -> Hashtbl.replace builder.variables k value) saved_variables
...
let rec gen_expression (builder : builder) (expression : Ast.expr) : Sil.value =
  ...
  (* `e as T`: generate the operand AT the written type (see gen_expression_as). The `None` arm cannot
  ...
  | Ast.Var (name, _) ->
      ...
      emit builder (Sil.Load addr) (value_type builder addr) (* the slot's element type *)
  ...
  | Ast.Binary ((Ast.And | Ast.Or) as op, left_expression, right_expression, _) ->
      ...
      let slot = emit builder (Sil.Alloc_stack (if op = Ast.And then "$and" else "$or")) Types.TBool in
      ...
      let then_target, else_target =
        if op = Ast.And then (right_block.Sil.bid, merge_block.Sil.bid) else (merge_block.Sil.bid, right_block.Sil.bid)
      in
  ...
  | Ast.Binary (op, left_expression, right_expression, _) ->
      let left_value = gen_expression builder left_expression and right_value = gen_expression builder right_expression in
      let operand = if value_type builder left_value = Types.TDouble || value_type builder right_value = Types.TDouble then Types.TDouble else value_type builder left_value in
      emit builder (Sil.Binop (op, left_value, right_value)) (result_type op operand)
  | Ast.Call (function_name, arguments, _) ->
      ...
      if Hashtbl.mem builder.functions function_name then (
        ...
        let function_reference = emit builder (Sil.Func_ref function_name) ret in
...
and gen_expression_as (builder : builder) (expression : Ast.expr) (expected : Types.ty) : Sil.value =
  match (expression, expected) with
  | Ast.Int_lit (n, _), Types.TDouble -> emit builder (Sil.Float_lit (float_of_int n)) Types.TDouble
  ...
  | Ast.Binary (((Ast.Add | Ast.Sub | Ast.Mul | Ast.Div) as op), left_expression, right_expression, _), Types.TDouble ->
      let left_value = gen_expression_as builder left_expression Types.TDouble and right_value = gen_expression_as builder right_expression Types.TDouble in
...
and gen_statement (builder : builder) (s : Ast.stmt) : unit =
  ...
  | Ast.Let { name; annot; value; _ } ->
      ...
      let value =
        match annot with
        | Some n -> ( match Types.of_name n with Some t -> gen_expression_as builder value t | None -> gen_expression builder value)
  ...
  | Ast.For { var; lo; hi; body; _ } ->
      ...
      let condition_value = emit builder (Sil.Binop (Ast.Lt, current_value, upper_value)) Types.TBool in
      terminate builder (Sil.Cond_br (condition_value, body_block.Sil.bid, exit_block.Sil.bid));
      switch_to builder body_block;
  ...
  | Ast.Continue _ -> ( match continue_target builder with Some condition_value -> terminate builder (Sil.Br condition_value) | None -> ())

(* --- lowering a function: params get slots; then the body --- *)
let lower_func functions (name : string) (params : (string * Types.ty) list) (ret : Types.ty)
    (body : Ast.stmt list) : Sil.func =
  let value_types = Hashtbl.create 16 in
  let entry = { Sil.bid = 0; instrs = []; term = Sil.Unreachable } in
  let builder =
    { next_value = 0; next_block = 1; current_block = entry; blocks = [ entry ]; variables = Hashtbl.create 16; value_types; functions; loops = [] }
  in
  ...
  gen_block builder body;
  terminate builder (if ret = Types.TVoid then Sil.Return None else Sil.Unreachable);
  { Sil.fname = name; params = sil_parameters; ret; blocks = builder.blocks; val_ty = value_types }
...
let lower (program : Ast.program) : Sil.modul =
  ...
  let return_type_of function_decl = match function_decl.Ast.ret with None -> Types.TVoid | Some n -> type_of_name n in
  ...
  let function_definitions =
    List.filter_map
      (function
        | Ast.IFunc function_decl ->
            let params = List.map (fun (parameter : Ast.param) -> (parameter.Ast.pname, type_of_name parameter.Ast.ptype)) function_decl.Ast.params in
            Some (lower_func functions function_decl.Ast.fname params (return_type_of function_decl) function_decl.Ast.body)
        | Ast.IStmt _ -> None)
      program.Ast.items
  in
  let main_statements = List.filter_map (function Ast.IStmt s -> Some s | Ast.IFunc _ -> None) program.Ast.items in
