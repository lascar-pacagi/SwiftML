(* FROZEN SOLUTION, WITH §6 EXERCISE 2 APPLIED (the `...` operator), — concept 06 lexer. Phase-1 scanning + the Phase-2 lexemes:
...
let rec next (lexer : t) : Token.t =
  ...
  if at_end lexer then make_token lo lexer Token.Eof
    ...
    if is_digit c then (
      ...
      | '{' ->
          ignore (advance_char lexer);
          make_token lo lexer Token.LBrace
      | '}' ->
          ignore (advance_char lexer);
          make_token lo lexer Token.RBrace
      | ',' ->
          ignore (advance_char lexer);
      ...
      | '&' ->
          ...
          if peek_char lexer = '&' then (
            ignore (advance_char lexer);
            make_token lo lexer Token.AmpAmp)
          else (
            report_error lexer lo "expected '&' after '&'";
            next lexer)
      | '|' ->
          ...
          if peek_char lexer = '|' then (
            ignore (advance_char lexer);
            make_token lo lexer Token.PipePipe)
          else (
            report_error lexer lo "expected '|' after '|'";
            next lexer)
      | '.' ->
          ...
          if peek_char lexer = '.' && peek_next_char lexer = '<' then (
            ignore (advance_char lexer);
            ignore (advance_char lexer);
            make_token lo lexer Token.DotDotLt)
          (* EX2: maximal munch again — the third `.` has to be checked before the
             two-character form is accepted, or `1...3` lexes as `..` then `.3`. *)
          else if peek_char lexer = '.' && peek_next_char lexer = '.' then (
            ignore (advance_char lexer);
            ignore (advance_char lexer);
            make_token lo lexer Token.DotDotDot)
          else (
            report_error lexer lo "unexpected character '.'";
            next lexer)
      | '\n' ->
          ignore (advance_char lexer);
