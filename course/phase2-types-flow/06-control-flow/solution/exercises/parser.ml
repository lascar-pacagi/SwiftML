(* FROZEN SOLUTION, WITH §6 EXERCISES 1 and 2 APPLIED, — concept 06 parser. Phase-1 recursive-descent + Pratt, plus the new
...
and parse_stmt (parser : t) : Ast.stmt =
  ...
  | Token.Kw_for ->
      ...
      (* EX2: either range operator, and the parser records WHICH. `expect` cannot do
         this job — it takes one kind — so the choice is made here and the message
         names both, which is what a reader who typed neither needs to see. *)
      let closed =
        match peek_kind parser with
        | Token.DotDotDot ->
            ignore (advance parser);
      ...
      let body = parse_block parser in
      Ast.For { var; lo; hi; closed; body; span = keyword.Token.span }
  (* EX1: `repeat { body } while cond`. The body is parsed BEFORE the condition, which
     is the only place in this parser where that happens — and it is exactly what the
     source says, so no lookahead is needed to tell it from `while`. *)
  | Token.Kw_repeat ->
      let keyword = advance parser in
      let body = parse_block parser in
      ignore (expect parser Token.Kw_while "'while'");
      let cond = parse_expr parser in
      Ast.Repeat { body; cond; span = keyword.Token.span }
  | Token.Kw_break ->
      let t = advance parser in
