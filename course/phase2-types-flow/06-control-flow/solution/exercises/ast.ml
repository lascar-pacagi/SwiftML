(* AST for concept 06 — a *contract*, WITH §6's EXERCISES APPLIED (1 and 2). Concept-05 nodes + control flow: the logical
...
type stmt =
  ...
  | For of {
      ...
      lo : expr;
      hi : expr;
      (* EX2: `..<` is half-open, `...` closed. The flag has to live HERE rather than
         being decided in the parser, because `1 ..< 4` and `1 ... 3` mean the same
         loop but are not the same program — §6's test reads the dump to check the
         tree remembers which was written. *)
      closed : bool;
      body : stmt list;
      span : Token.span;
    }
    (* `for var in lo ..< hi { body }` / `lo ... hi` — an Int range *)
  | Break of Token.span
  | Continue of Token.span
  (* EX1: `repeat { body } while cond` — Swift's do-while. The body runs BEFORE the
     condition is ever evaluated, which is the whole difference from `While` and the
     reason it is its own node rather than sugar for one. *)
  | Repeat of { body : stmt list; cond : expr; span : Token.span }
...
let rec dump_stmt = function
  ...
  | For { var; lo; hi; closed; body; _ } ->
      (* the half-open form dumps exactly as it did before the exercise, so every
         golden written against `..<` still holds; only `...` says so *)
      Printf.sprintf "(for %s %s %s%s %s)" var (dump_expr lo) (dump_expr hi)
        (if closed then " closed" else "")
        (dump_block body)
  | Break _ -> "break"
  | Continue _ -> "continue"
  | Repeat { body; cond; _ } ->
      Printf.sprintf "(repeat %s %s)" (dump_block body) (dump_expr cond)
