...
(* WITH §6 EXERCISE 1 APPLIED: the checked form of `repeat`. `For` is unchanged —
   `closed` is a fact about the SOURCE, and by here the loop has been checked either
   way, so the Tast has no use for it. *)
