...
`while cond { body }`:
  ...
  $ printf 'var n = 0\nwhile n < 3 {\n  n = n + 1\n}\n' > w.swift
  $ timeout 5 ./lab.exe --emit-ast w.swift
  (var n 0)
  (while (< n 3) ((= n (+ n 1))))
...
`for i in 0 ..< n { body }` dumps as `(for i lo hi body)`; the bounds are expressions:
  ...
  $ printf 'let n = 3\nfor i in 0 ..< n + 1 {\n  print(i)\n}\n' > f.swift
  $ timeout 5 ./lab.exe --emit-ast f.swift
  (let n 3)
  (for i 0 (+ n 1) ((print i)))
...
An `if` inside a loop body needs `parse_stmt` to dispatch on `if` as well as on the loop keywords:
  ...
  $ printf 'while true {\n  if true {\n    print(1)\n  }\n  print(2)\n}\n' > nest2.swift
  $ timeout 5 ./lab.exe --emit-ast nest2.swift
  (while true ((if true ((print 1))) (print 2)))
