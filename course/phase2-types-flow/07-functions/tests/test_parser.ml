...
let tokens (src : string) : Token.kind list =
  ...
  Lexer.tokenize (Lexer.create src d)
  |> List.map (fun (t : Token.t) -> t.Token.kind)
...
let params_of (src : string) : string * string list =
  let d = Diagnostics.create () in
  let ps =
    Parser.parse_params (Parser.create (Lexer.tokenize (Lexer.create src d)) d)
  in
...
let check_params what expected src =
  ...
  Alcotest.(check (list string))
    (Printf.sprintf "%S is accepted silently" src)
    [] msgs
...
let param_error src msg =
  ...
  Alcotest.(check (option string))
    (Printf.sprintf "%S first error" src)
    (Some msg) (List.nth_opt msgs 0)

(* ---- lexer: the arrow ------------------------------------------------------------------ *)

let test_arrow () =
  let kinds src =
    List.filter (fun k -> k <> Token.Newline && k <> Token.Eof) (tokens src)
  in
  let has src k =
    Alcotest.(check bool)
      (Printf.sprintf "%S lexes %s" src (Token.string_of_kind k))
      true
  ...
  has "a->b" Token.Arrow;
  Alcotest.(check int)
    "`a->-b`: one arrow, one minus" 2
    (List.length
       (List.filter
          (fun k -> k = Token.Arrow || k = Token.Minus)
          (kinds "a->-b")));
  (* a `-` not followed by `>` is still minus, and `- >` is two tokens *)
  Alcotest.(check bool)
    "`a - b` has no arrow" false
    (List.mem Token.Arrow (kinds "a - b"));
  Alcotest.(check bool)
    "`- >` has no arrow" false
    (List.mem Token.Arrow (kinds "a - > b"));
  Alcotest.(check bool)
    "`- >` is minus then greater" true
    (List.mem Token.Minus (kinds "a - > b")
    && List.mem Token.Gt (kinds "a - > b"))

(* ---- parser: return -------------------------------------------------------------------- *)

let test_return () =
  check "return value" "(return 5)" "return 5";
  check "return expression" "(return (+ (fib (- n 1)) 2))"
    "return fib(n - 1) + 2";
  check "bare return, newline" "(return)\n(print 1)" "return\nprint(1)";
  check "bare return, eof" "(return)" "return";
  check "bare return before }" "(if c ((return)))" "if c { return }";
  check "return in a block" "(if c ((print 1) (return x)))"
    "if c {\n  print(1)\n  return x\n}"
...
let test_params_shape () =
  ...
  check_params "three, in order" "(a:Int b:Bool c:String)"
    "(_ a: Int, _ b: Bool, _ c: String)"
...
let test_params_trailing_comma () =
  param_error "(a: Int,)" "expected a parameter name"

let test_params_underscore () =
  check_params "`_` is a name" "(_:Int)" "(_: Int)"
...
let test_params_leading_comma () =
  param_error "(, _ a: Int)" "expected a parameter name"

let test_params_double_comma () =
  param_error "(_ a: Int,, _ b: Int)" "expected a parameter name"

let test_params_missing_comma () =
  param_error "(_ a: Int _ b: Int)" "expected ')'"
...
let test_params_keyword_label () =
  param_error "(let a: Int)" "expected a parameter name"
...
let test_items () =
  ...
  check "func then stmt" "(func f () -> Int ((return 1)))\n(print (f ))"
    "func f() -> Int { return 1 }\nprint(f())";
  check "stmt, func, stmt, func"
    "(print (g ))\n\
     (func g () -> Int ((return 1)))\n\
     (let x (g ))\n\
     (func h () ())"
    "print(g())\nfunc g() -> Int { return 1 }\nlet x = g()\nfunc h() { }";
  check "blank line between funcs"
...
let () =
  Alcotest.run "parser-funcs"
    [
      ...
      ( "return",
        [ Alcotest.test_case "with and without a value" `Quick test_return ] );
      ( "params",
        [
          Alcotest.test_case "empty, one, three in order" `Quick
            test_params_shape;
          Alcotest.test_case "labels dropped: _, named, none" `Quick
            test_params_labels;
          Alcotest.test_case "bad: no '(' to open it" `Quick
            test_params_no_lparen;
          Alcotest.test_case "bad: `(a)`, name or ':' missing" `Quick
            test_params_label_no_name;
          Alcotest.test_case "bad: no type after ':'" `Quick test_params_no_type;
          Alcotest.test_case "bad: list never closed" `Quick
            test_params_unclosed;
          Alcotest.test_case "bad: trailing comma" `Quick
            test_params_trailing_comma;
          Alcotest.test_case "`_` is an ordinary name" `Quick
            test_params_underscore;
          Alcotest.test_case "all three label forms" `Quick test_params_forms;
          Alcotest.test_case "bad: leading comma" `Quick
            test_params_leading_comma;
          Alcotest.test_case "bad: two commas in a row" `Quick
            test_params_double_comma;
          Alcotest.test_case "bad: no comma between two" `Quick
            test_params_missing_comma;
          Alcotest.test_case "bad: no ':' name/type" `Quick
            test_params_missing_colon;
          Alcotest.test_case "bad: end of input, no ')'" `Quick test_params_eof;
          Alcotest.test_case "bad: keyword as a label" `Quick
            test_params_keyword_label;
        ] );
      ( "func",
        [
      ...
      ( "items",
        [ Alcotest.test_case "top-level items in order" `Quick test_items ] );
    ]
