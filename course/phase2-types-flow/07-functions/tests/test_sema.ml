...
let error_count src n =
  Alcotest.(check int)
    (Printf.sprintf "%S => %d error(s)" src n)
    n
...
let test_forward_reference () =
  accepted "print(g())\nfunc g() -> Int { return 1 }";
  accepted
    "func twice(_ n: Int) -> Int { return double(n) }\n\
...
let test_return_void () =
  has_error "func f() { return 1 }"
    "unexpected non-void return value in void function";
  has_error "func f() -> Int { return }"
...
let test_ret_if () =
  ...
  returns "`if`/`else`, only then returns" false
    [ if_ e [ ret ] (Some [ noise ]) ];
  returns "`if`/`else`, only else returns" false
    [ if_ e [ noise ] (Some [ ret ]) ]
...
let () =
  Alcotest.run "sema-funcs"
    [
      ...
      ( "passes",
        [
          Alcotest.test_case "statements only; top level" `Quick
            test_statements_only;
      ...
      ( "check_func",
        [
          Alcotest.test_case "params in scope; shadowing" `Quick
            test_params_in_scope;
          ...
          Alcotest.test_case "unknown param/return type" `Quick
            test_unknown_types;
          Alcotest.test_case "missing return" `Quick test_missing_return;
          Alcotest.test_case "definite return accepted" `Quick
            test_definite_return;
        ] );
      ( "return",
        [
