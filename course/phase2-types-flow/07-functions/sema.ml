(* Sema — concept 07 (skeleton): concepts 05–06 + functions. Concepts 05–06 are filled in;
   ...
   the two passes). Reference: solution/sema.ml.
   ...
   As in 05, the checker PRODUCES a `Tast.program` rather than returning a verdict, so every hole
   below returns a typed node. Functions make that concrete: a `Tast.func_decl` carries RESOLVED
   parameter and return types, so the written type names never travel past this file. And
   `Ast.Call` — which could be print, a declared function, or an unknown name — becomes whichever
   it actually was; choosing is part of TODO(07)'s call hole. PLAN.md §0.1. *)
...
let rec stmt_returns (s : Ast.stmt) : bool =
  ignore s;
  (* TODO(07): does this STATEMENT definitely return? Flow-sensitive but tiny: four cases decide
     it, and one of them is a trap. §2 "Missing return: your first real flow analysis". *)
  failwith "TODO(07): does this statement definitely return?"

and block_returns (statements : Ast.stmt list) : bool =
  ignore statements;
  ignore stmt_returns;
  (* TODO(07): ... and this BLOCK? *)
...
let check (program : Ast.program) (diagnostics : Diagnostics.sink) : Tast.program option =
  ...
  and infer_call f args span : Tast.expr =
    ...
    | Some (ptypes, ret) ->
        ignore (ptypes, ret, span, some_arg);
        (* TODO(07): a call — arity, then each argument against its parameter type; the result
           is the callee's return type, and the node is `Tast.Fn_call`, which records that this
           call resolved to a declared function rather than to print. The tests pin swiftc's
           wording. §3. *)
  ...
  let rec check_stmt (s : Ast.stmt) : Tast.stmt =
    ...
    | Ast.Continue span ->
        ...
        Tast.Continue span
    | Ast.Return (eo, span) ->
        ignore (eo, span, current_return_type);
  ...
  let check_func (f : Ast.func_decl) : Tast.func_decl =
    ignore (f, resolve_ty, block_returns);
    (* TODO(07): one function — parameters bound immutably in a scope of their own (a function
       ...
       non-Void function must return on every path (`block_returns`). Return the `Tast.func_decl`,
       whose params and return type are RESOLVED — the written names do not survive. §3. *)
    failwith "TODO(07): check a function body"
  in

  ignore (check_func, resolve_silent, functions);
  (* TODO(07): the two passes — collect every signature first (which is what makes calls,
     recursion and forward references resolve), then check the bodies in order, building the
     `Tast.item` list. A duplicate name is an error. §2 explains why one pass cannot work.
     Finish by returning `None` if anything was reported, else `Some { Tast.items }`. *)
