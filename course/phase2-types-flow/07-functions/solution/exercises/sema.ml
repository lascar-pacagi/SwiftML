(* FROZEN SOLUTION — concept 07 sema: concepts 05–06 + functions.
   ...
   New vs 06:
     ...
     - functions are self-contained (params + the function table only — no top-level
     capture)
     - print and Void functions yield () (Types.TVoid)

   As in 05, the checker PRODUCES a `Tast.program` rather than returning a verdict.
   Functions
   make that concrete: a `Tast.func_decl` carries RESOLVED parameter and return types, so
   the
