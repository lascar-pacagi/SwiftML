(* FROZEN SOLUTION — concept 07 parser (carries 05–06; adds functions and
   return). Phase-1 recursive-descent + Pratt, plus the new
   ...
   WITH §6 EXERCISE 1 APPLIED: argument labels, at the declaration and the call. *)
...
(* GIVEN — a lookahead that may have to un-read what it read. `mark`
   remembers where the cursor
...
let rec parse_expr_bp (parser : t) (minimum_binding_power : int) : Ast.expr =
  let left =
    ...
    | Token.Float f ->
        let t = advance parser in
        Ast.Double_lit (f, t.Token.span)
    | Token.String s ->
        let t = advance parser in
        Ast.String_lit (s, t.Token.span)
    | Token.Kw_true ->
        let t = advance parser in
        Ast.Bool_lit (true, t.Token.span)
    | Token.Kw_false ->
        let t = advance parser in
        Ast.Bool_lit (false, t.Token.span)
    | Token.Ident name ->
        let t = advance parser in
...
and parse_call_args (parser : t) : (string option * Ast.expr) list =
  if peek_kind parser = Token.RParen then []
  else
    let rec loop accumulator =
      (* EX1: `ident :` is a LABEL, anything else is the start of an expression. One token
         of lookahead settles it, and the lookahead is needed: `f(a: 1)` labels its
         argument while `f(a)` passes the variable `a`, and the two differ only in what
         follows the identifier. *)
      let label =
        match (peek_kind parser, peek_kind_at parser 1) with
      ...
      if peek_kind parser = Token.Comma then (
        ignore (advance parser);
        loop ((label, expression) :: accumulator))
      else List.rev ((label, expression) :: accumulator)
    in
    loop []
...
and parse_stmt (parser : t) : Ast.stmt =
  ...
  | Token.Kw_break ->
      let t = advance parser in
      Ast.Break t.Token.span
  | Token.Kw_continue ->
      let t = advance parser in
      Ast.Continue t.Token.span
  | Token.Kw_return -> (
      let keyword = advance parser in
      match peek_kind parser with
...
let parse_params (parser : t) : Ast.param list =
  ...
  if peek_kind parser = Token.RParen then (
    ...
    let rec loop accumulator =
      ...
      (* EX1: the three forms, and now the LABEL is kept rather than skipped.
           `name: T`    -> label and name are both `name`
           `label n: T` -> the caller writes `label:`, the body sees `n`
           `_ n: T`     -> no label at all, so the call is positional *)
      let plabel, pname =
        if peek_kind parser = Token.Colon then (Some first, first)
        else
          let inner, _ = parse_ident parser "a parameter name" in
          ((if first = "_" then None else Some first), inner)
      in
      ignore (expect parser Token.Colon "':'");
      let ptype, _ = parse_ident parser "a parameter type" in
      let accumulator = { Ast.plabel; pname; ptype } :: accumulator in
      if peek_kind parser = Token.Comma then (
        ignore (advance parser);
        loop accumulator)
      else (
...
let parse_func (parser : t) : Ast.func_decl =
  let keyword =
    advance parser
    (* func *)
  in
  let fname, _ = parse_ident parser "a function name" in
  let params = parse_params parser in
  let ret =
    if peek_kind parser = Token.Arrow then (
      ignore (advance parser);
      let t, _ = parse_ident parser "a return type" in
      Some t)
    else None
  in
  let body = parse_block parser in
  { Ast.fname; params; ret; body; fspan = keyword.Token.span }
