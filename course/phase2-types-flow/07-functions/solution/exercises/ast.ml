(* AST for concept 07 — a *contract*. Concept-06 nodes + functions: a parameter, a
   ...
   WITH §6 EXERCISE 1 APPLIED: argument labels, on the declaration and at the call. *)
...
type expr =
  ...
  (* EX1: each argument carries the label the CALLER wrote, or None for a positional one.
     Sema checks it against the parameter's; it does not survive into the Tast, because by
     then the call is resolved and a label is a fact about the source. *)
  | Call of string * (string option * expr) list * Token.span
...
type stmt =
  ...
  | Return of
      expr option * Token.span (* NEW in this concept: `return` or `return e` *)

(* NEW in this concept: functions *)
type param = {
  (* EX1: the ARGUMENT LABEL, which is what a caller writes, as opposed to [pname], which
     only the body sees. `func f(a b: Int)` has label "a" and name "b"; `func f(a: Int)`
     has both as "a"; `func f(_ a: Int)` has no label at all, which is None. *)
  plabel : string option;
  pname : string;
...
let rec dump_expr = function
  ...
  | Call (function_name, arguments, _) ->
      (* and likewise the call: the labels are carried, not printed. See dump_param. *)
      Printf.sprintf "(%s %s)" function_name
        (String.concat " " (List.map (fun (_, a) -> dump_expr a) arguments))
...
(* EX1 deliberately does NOT print the label. The label is now in the AST and sema checks
   it, but the dump is what `tests/parser-params.t` and `test_parser.ml` assert, and those
   are testing that a list PARSES — a question the exercise does not change. Printing it
   would turn a dozen passing cases red for no reason anyone could act on. The course
   standard calls this staying exercise-neutral; here it costs nothing, because the
   exercise's own tests read diagnostics rather than dumps. *)
