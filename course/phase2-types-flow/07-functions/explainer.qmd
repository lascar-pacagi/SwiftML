...
3. **`sema.ml`** — five holes: **call typing** in `infer_call` (look the name up in `funcs`; check
   ...
   **The definite-return analysis stands alone** — start there if you like. It is a pure function
   ...
   The other four all run inside `check`, so they do have an order. Start with the **two passes**.
   They are `check`'s entry point: until they exist, no program is checked at all. Then
   **`check_func`**. It calls `block_returns` on every function that declares a return type, so
   write the analysis before this one — otherwise every such function dies there. Then **`return`**,
   then the **call**. §4 says which file each turns green.
