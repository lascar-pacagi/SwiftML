...
3. **`sema.ml`** — five holes. The scope helpers, `loop_depth`, `current_return_type` and
   ...
   **Start with the definite-return analysis.** It is a pure function of the AST — no
   environment, no diagnostics, nothing else in the file — so it sits at module level and two
   tests reach it without going through `check`. `sema-definite-return.t` asks `--emit-returns`
   about each function in a program, and `test_sema.ml`'s *definite return* group calls
   `Sema.block_returns` on statements it builds by hand, with no parser at all. Both go green
   on that one hole while every other TODO in the file still raises.

   The other four run inside `check`, so they have an order:

   1. **the two passes** — `check`'s entry point, so nothing is checked until they exist;
   2. **`check_func`** — which calls `block_returns`, so write that first or every function
      with a `-> T` crashes here;
   3. **`return`**;
   4. **the call**.
