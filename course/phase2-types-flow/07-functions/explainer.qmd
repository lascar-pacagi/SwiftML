...
- `parser-params.t` — `--emit-params`: parameter lists on their own — empty, one, three, the
  `_` label dropped, a named label dropped, and the first error of five malformed lists. Green
  with `parse_params` alone: the mode parses one list and stops, so the source of each case is
  the list itself and this file never waits for `parse_func`.
