...
other analyses, and it benefits from earlier passes. In `while 1 < 2 { … }` the loop's test
block has two edges: into the body, and out to the code after the loop, which falls off the end
of the function. Constant folding turns `1 < 2` into `true` and deletes that second edge. The
code after the loop is now unreachable, so the missing-return check passes — without the check
itself knowing anything about `1 < 2`.
