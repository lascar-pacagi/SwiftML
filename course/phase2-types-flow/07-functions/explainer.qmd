...
other analyses, and it lets passes divide the work. swiftc accepts
`var ok = false; ok = true; if ok { return 1 }` with no `else`, because a constant-propagation
pass runs first and deletes the branch that could skip the `return`. You could do the same
tracking inside an AST checker, but then the checker owns it; on the graph, the check just reads
what the earlier pass left.
