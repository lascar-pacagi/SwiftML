...
3. **`sema.ml`** — five holes: **call typing** in `infer_call` (look the name up in `funcs`; check
   ...
   **The definite-return analysis stands alone** — start there if you like. It is a pure function
   ...
   The rest do have an order, and `check` forces it: fill the **two passes first**, because they
   are its entry point and until they exist no sema *program* can be checked — every sema cram
   file reads `TODO` whatever else you have written. Then `check_func`, which is where the
   analysis gets used: it asks `block_returns` about every non-`Void` function, so leaving that
   hole raising while you write this one puts red across all five sema files. Then `return`, then
   the call; §4 lists which file each turns green.
