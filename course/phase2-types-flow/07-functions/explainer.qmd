...
3. **`sema.ml`** — five holes: **call typing** in `infer_call` (look the name up in `funcs`; check
   ...
   analysis** `stmt_returns` / `block_returns` (§2 works out all four cases: a `return` returns; an
   `if` returns only with an `else` *and* both blocks returning; a loop never does; a block returns
   if **any** statement does — the two are mutually recursive because an `if` holds blocks);
   ...
   **The definite-return analysis stands alone** — start there if you like. It is a pure function
   ...
   level, and `test_sema.ml`'s *definite return* group calls `Sema.block_returns` on statements it
   builds by hand, with no parser and no `check` in the way. Those five cases go green on that one
   hole while every other TODO in `sema.ml` still raises.
