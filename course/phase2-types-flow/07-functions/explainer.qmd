...
other analyses, and it benefits from earlier passes. Take a function whose whole body is
`if 1 < 2 { return 1 }`, with no `else`. Your rule rejects it: an `if` without `else` does not
definitely return. In the graph, the test block has two edges: to `return 1`, and to the end of
the function, which returns nothing. Constant folding turns `1 < 2` into `true` and deletes the
second edge, so no path reaches the end without returning. swiftc accepts it — and the check
never had to know anything about `1 < 2`. With `n < 2` instead, nothing folds, the edge stays,
and swiftc rejects it like you do.
