...
@fig-twopass runs one program both ways. Read the red half first: nothing in it is a mistake
the programmer made. `fib(10)` is a perfectly good call and so is `fib(n-1)` — what defeats
them is that a walk building the table as it goes is never ahead of itself, so both arrive
before the entry that answers them exists.

![The same program checked top to bottom, where both calls fail, and checked in two passes, where the same two calls are answered by a table that was already finished when they were asked.](figs/twopass.png){#fig-twopass}
