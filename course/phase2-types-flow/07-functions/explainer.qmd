...
to carry real weight. `Ast.Call (f, args)` is three different things depending on `f` — `print`, a
function declared somewhere in this file, or a name that means nothing — and *only the checker can
tell which*. It records the answer as a different node: `Tast.Print` or `Tast.Fn_call`. And a
`Tast.func_decl` carries **resolved** parameter and return types, so the written type names
(`"Int"`, the string) stop at this file and never travel further.
