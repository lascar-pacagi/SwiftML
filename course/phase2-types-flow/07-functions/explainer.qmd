...
Which is the honest reason your version is the weaker one. An AST recursion can only see the
shapes the grammar gives it, so `while true { return 1 }` looks like a loop that might run zero
times — the one thing a CFG would already have folded away.
