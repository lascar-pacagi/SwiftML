...
control flow into shapes no tree can hold. Inline a function that returns early into the middle
of a loop, and its `return` becomes a jump from inside the loop body to the code after the call
— no nesting of `if` and `while` says that without inventing flag variables. On the graph it is
one more edge. Phase 4's passes all work this way: SSA construction (16) needs the graph's
