...
— so `let x = 1` at top level followed by `print(x)` in a body is "cannot find 'x' in scope" here,
where swiftc would accept it. That is the one place in this concept where we reject what swiftc
accepts; the oracle corpus stays off it.)
