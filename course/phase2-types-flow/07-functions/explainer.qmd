...
3. **`sema.ml`** — five holes: **call typing** in `infer_call` (look the name up in `funcs`; check
   ...
   **`check_func`** (fresh `env`, parameters bound immutable, set `current_ret`, check the body,
   restore both, then the "missing return" rule, which is `block_returns` on the body); and the
   **two passes** (fill `funcs` with signatures — a name seen twice is a redeclaration — then check
   each item in order). The scope helpers, `loop_depth`, `current_ret` and `resolve_*` are given.
