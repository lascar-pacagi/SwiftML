...
*The rest of this rule's story — where swiftc runs the check, and why a graph answers it better
than a tree — is for the curious. You do not need it to write `stmt_returns`; if you would rather
keep going, skip to [the note on argument labels](#sec-labels).*
...
case for `switch` in concept 12, and for `throw` and `do` in concept 30.

But the check is a side benefit. The graph exists because everything after sema needs it. The
machine has no `if` or `while`, only jumps to labels, so code generation has to produce the graph
anyway; concept 08 builds it and concept 09 prints it as branches. And optimisations rewrite
control flow into shapes a tree holds badly. Take inlining — pasting a function's body into its
caller, so the call disappears. Here is a function and a call to it:
...
Inlined, there is no function any more, so `return k` cannot stay a `return`: it must leave the
loop *and* skip the `print("not found")`. `break` does only the first. Written as source, the
result needs a variable whose only job is to carry "we left early" past the loop:
...
var found = false
while k < 10 {
  if k * k == 49 { r = k; found = true; break }
  k = k + 1
}
if !found { print("not found"); r = -1 }
print(r)
```

In the graph none of that is needed: the `if` just gets an edge straight to the block that does
`print(r)`, and `found` never exists. Phase 4's passes all work this way: SSA construction (16)
needs the graph's dominators, and deleting a dead branch (17) is deleting an edge.

### A note on argument labels (a Swift feature we simplify) {#sec-labels}
