...
3. **`sema.ml`** — five holes: **call typing** in `infer_call` (look the name up in `functions`; check
   ...
   **two passes** (fill `functions` with signatures — a name seen twice is a redeclaration — then check
   each item in order). The scope helpers, `loop_depth`, `current_return_type` and `resolve_*` are given.
...
and infer_call f args span : Types.ty =
  ...
  | None ->
      if f = "print" then (
        ...
        Types.TVoid)
      else (report_error span (Printf.sprintf "cannot find '%s' in scope" f); Types.TInt)

(* return: only in a function; value checked against the return type *)
| Ast.Return (eo, span) -> (match !current_return_type with
    | None -> report_error span "return invalid outside of a func"
    | Some rt -> (match eo with
        | Some e -> if rt = Types.TVoid then report_error span "unexpected non-void return value in void function"
                    else check_expr e rt
        | None -> if rt <> Types.TVoid then report_error span "non-void function should return a value"))

(* does a block definitely return on every path? — TODO(07) in the skeleton *)
...
let check_func (f : Ast.func_decl) : unit =
  ...
  environment := []; current_return_type := Some ret;                 (* fresh scope; record the return type *)
  List.iter
    (fun (pr : Ast.param) ->
      bind pr.Ast.pname (resolve_ty f.Ast.fspan pr.Ast.ptype, false))
    f.Ast.params;
  List.iter check_stmt f.Ast.body;
  environment := saved_env; current_return_type := saved_ret;
...
List.iter (function
  ...
  | Ast.IStmt _ -> ()) program.Ast.items;
List.iter (function Ast.IFunc f -> check_func f | Ast.IStmt s -> check_stmt s) program.Ast.items
```
