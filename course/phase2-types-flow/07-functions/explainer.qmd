...
control flow into shapes a tree holds badly. Take inlining — copying a function's body into its
caller:
...
func find(_ x: Int) -> Int {
  var k = 0
  while k < 10 {
    if k * k == x { return k }   // once inlined: leave the loop AND skip `return -1`
    k = k + 1
  }
  return -1
}
...
Once the body is pasted into the caller there is no function to return from, so `return k` has
to become something else. `break` leaves the loop but then runs `return -1`'s replacement too.
Written as source, you would need an extra variable — `var found = false`, set before the
`break`, tested after the loop. In the graph it is one edge, from the `if` straight to the code
after the call. Phase 4's passes all work this way: SSA construction (16) needs the graph's
dominators, and deleting a dead branch (17) is deleting an edge.
