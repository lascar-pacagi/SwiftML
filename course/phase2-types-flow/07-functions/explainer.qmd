...
3. **`sema.ml`** — five holes: **call typing** in `infer_call` (look the name up in `funcs`; check
   ...
   **two passes** (fill `funcs` with signatures — a name seen twice is a redeclaration — then check
   ...
   sema test can run — every sema file reads `TODO`. Then **`check_func` and the analysis
   together**: `check_func` asks `block_returns` about *every* non-`Void` function, so while the
   analysis is still a `failwith` any program that declares one dies there — the red you see will
   be spread across all five sema files, not confined to the missing-return cases. Once it
   returns, those five cases in `sema-func.t` are what grade it: a body that never returns, an
   `if` with no `else`, a `return` inside a loop, an `else if` chain, and a `return` with
   statements after it. Then `return`, then the call; §4 lists which file each turns green.
