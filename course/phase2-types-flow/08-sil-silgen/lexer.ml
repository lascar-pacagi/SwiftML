...
let rec next (lexer : t) : Token.t =
  ...
  if at_end lexer then make_token lo lexer Token.Eof
    ...
    if is_digit c then (
      ...
      | '-' ->
          ignore (advance_char lexer);
          if peek_char lexer = '>' then (ignore (advance_char lexer); make_token lo lexer Token.Arrow)
      ...
      | '{' -> ignore (advance_char lexer); make_token lo lexer Token.LBrace
      | '}' -> ignore (advance_char lexer); make_token lo lexer Token.RBrace
      | ',' -> ignore (advance_char lexer); make_token lo lexer Token.Comma
      | '&' ->
          ignore (advance_char lexer);
          if peek_char lexer = '&' then (ignore (advance_char lexer); make_token lo lexer Token.AmpAmp)
          else (report_error lexer lo "expected '&' after '&'"; next lexer)
      | '|' ->
          ignore (advance_char lexer);
          if peek_char lexer = '|' then (ignore (advance_char lexer); make_token lo lexer Token.PipePipe)
          else (report_error lexer lo "expected '|' after '|'"; next lexer)
      | '.' ->
          ignore (advance_char lexer);
          ...
          else (report_error lexer lo "unexpected character '.'"; next lexer)
