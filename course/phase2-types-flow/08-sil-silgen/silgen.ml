...
let rec gen_expression (builder : builder) (expression : Ast.expr) : Sil.value =
  ...
  | Ast.Var (name, _) ->
      ignore name;
      (* TODO(08a): reading a variable is a LOAD from its slot. `address_of builder name` is where that
...
and gen_statement (builder : builder) (s : Ast.stmt) : unit =
  ...
  | Ast.Continue _ ->
      (* TODO(08c): branch to the innermost loop's continue target — `continue_target builder`, which is
