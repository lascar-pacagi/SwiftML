...
(* The front end's result is the TYPE-CHECKED tree, not the parsed one: `Sema.check` resolves
   names and records types, and everything downstream consumes what it returns. `None` means the
   program had errors and has no typed form. *)
