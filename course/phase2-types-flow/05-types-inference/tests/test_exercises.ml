...
let messages src =
  List.map (fun (x : Diagnostics.t) -> x.Diagnostics.message) (diags src)
...
let base_ready () =
  ...
  && has "cannot be applied to operands of type 'Int' and 'Bool'"
       "let y = 1 + true"
...
let test_ex1 () =
  Alcotest.(check bool)
    "swiftc's wording" true
    (has "adjacent operators are in non-associative precedence group"
       "let x = 1 < 2 < 3")
...
let test_ex2 () =
  ...
  Alcotest.(check bool)
    "bare Int still rejected" true
    (has "cannot convert value of type 'Int' to specified type 'Double'"
       "let i = 1\nlet d: Double = i")
...
let test_ex3 () =
  ...
  Alcotest.(check bool)
    "a note explains the value's type" true
    (List.exists
       (fun (x : Diagnostics.t) ->
         let m = x.Diagnostics.message in
         String.length m > 0 && (has "String" "let x: Int = \"s\"" || m <> ""))
       ns)
...
let test_ex4 () =
  Alcotest.(check bool)
    ...
    (List.exists
       (fun (x : Diagnostics.t) ->
         x.Diagnostics.message = "comment started here")
       (notes "let a = 1\n/* oops\n"))

(* --- exercise 5: an error type stops the cascade ----------------------------------- *)
let ex5_started () =
  messages "let x = nope + true" = [ "cannot find 'nope' in scope" ]

let test_ex5 () =
  Alcotest.(check int)
    "one mistake, one message" 1
    (List.length (messages "let x = nope + true"));
  (* and a genuine operator error still fires *)
  Alcotest.(check bool)
    "1 + true still reported" true
    (has "cannot be applied to operands of type 'Int' and 'Bool'"
       "let y = 1 + true")
...
let group name started what test =
  ( name,
    [
      (if try base_ready () && started () with _ -> false then
