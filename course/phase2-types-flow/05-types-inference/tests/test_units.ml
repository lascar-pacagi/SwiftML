...
let test_unify () =
  ...
  Alcotest.(check (option ty))
    ...
    (u (int_ 1) Types.TInt (int_ 2) Types.TInt);
  Alcotest.(check (option ty))
    "String/String" (Some Types.TString)
    (u
       (Ast.String_lit ("a", sp))
       Types.TString
       (Ast.String_lit ("b", sp))
...
let test_check_expr () =
  ...
  | l ->
      Alcotest.failf "expected exactly one diagnostic, got %d" (List.length l)
...
let test_check_stmt () =
  ...
  | [ x ] ->
      Alcotest.(check string)
        "wording" "cannot assign to value: 'x' is a 'let' constant"
        x.Diagnostics.message
  | l ->
      Alcotest.failf "expected exactly one diagnostic, got %d" (List.length l)
...
let () =
  Alcotest.run "sema-units"
    [
      ( "05a",
        [
          Alcotest.test_case "is_int_literal: only literal trees" `Quick
            test_is_int_literal;
        ] );
      ( "05b",
        [ Alcotest.test_case "unify: one side may flex" `Quick test_unify ] );
      ( "05c",
        [
          Alcotest.test_case "infer: types synthesized" `Quick test_infer;
          Alcotest.test_case "infer: unknown name reports" `Quick
            test_unknown_name;
          Alcotest.test_case "infer: `-true` reports" `Quick test_unary_reports;
          Alcotest.test_case "infer: `1 + true` reports" `Quick
            test_binop_reports;
          Alcotest.test_case "infer: print infers its arg" `Quick
            test_print_infers_arg;
          Alcotest.test_case "infer: unknown function reports" `Quick
            test_unknown_function;
        ] );
      ( "05d",
        [ Alcotest.test_case "check_expr: pushes down" `Quick test_check_expr ]
      );
      ( "05e",
        [
          Alcotest.test_case "check_stmt: binds and rejects" `Quick
            test_check_stmt;
        ] );
    ]
