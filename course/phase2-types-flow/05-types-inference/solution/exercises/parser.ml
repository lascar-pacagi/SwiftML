(* FROZEN SOLUTION, WITH §6 EXERCISE 1 APPLIED (non-associative comparisons), — concept 05 parser. Phase-1 recursive-descent + Pratt, plus the new
...
(* EX1: the one place the comparison level is named, so the rule below cannot drift
   from the table. *)
let comparison_bp = 5
  ...
  let rec loop left =
    ...
    if peek_kind parser = Token.Kw_as && cast_bp >= minimum_binding_power then (
      ...
      | Some bp when bp >= minimum_binding_power ->
          ...
          (* EX1: comparisons are NON-ASSOCIATIVE in Swift, so `a < b < c` is a syntax
             error, not `(a < b) < c` typed and then rejected. `bp + 1` already stops the
             right operand absorbing a second comparison; what is left is to say so when
             one is sitting there, rather than folding it into the left and letting sema
             complain about comparing a Bool. Reported at the OFFENDING second operator. *)
          if is_comparison op then (
            match infix_bp (peek_kind parser) with
            | Some bp' when bp' = comparison_bp ->
                (* At the FIRST of the two operators, which is where swiftc points:
                   `let c = 1 < 2 == true` reports at column 11, the `<`, not at the
                   `==` that made it adjacent. Checked with the oracle. *)
                Diagnostics.error parser.diagnostics op_tok.Token.span
