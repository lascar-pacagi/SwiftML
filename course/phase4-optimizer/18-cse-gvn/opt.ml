(* The SIL optimizer — concept 18 (skeleton). Builds on 15–17. The new pass is GVN (global value
   numbering / CSE): keep one copy of each computation, reusing it where its definition dominates.
   You fill the TODO(18) holes: value_key + the dominance-scoped numbering walk. Reference: solution/opt.ml. *)
...
(* ---- the default `-O` pipeline. mem2reg first (so the rest see through memory), then
   fold/simplify/GVN/clean — repeated once so a folded branch exposes more folding. ---- *)
