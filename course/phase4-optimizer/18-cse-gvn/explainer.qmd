...
- `value_key canon instr` — `TODO(18)`: return `Some key` for each **pure** instruction, building the
  key from its operation and its operands' `canon` value-numbers (use the given `_v`). Cover the
...
```ocaml
let value_key (canon : Sil.value -> Sil.value) : Sil.instr -> string option =
  ...
  | Sil.Struct fs -> Some (Printf.sprintf "struct:%s" (String.concat "," (List.map v fs)))
  | Sil.Struct_extract (a, i) -> Some (Printf.sprintf "sext:%s:%d" (v a) i)
  | Sil.Enum (t, p) -> Some (Printf.sprintf "enum:%d:%s" t (String.concat "," (List.map v p)))
