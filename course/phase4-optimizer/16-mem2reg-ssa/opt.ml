(* The SIL optimizer — concept 16 (skeleton). Builds on the pass manager (15). The new pass is
   mem2reg: promote alloc_stack/load/store to SSA values threaded through basic-block arguments.
   The analyses (dominators, dominance frontiers, liveness) + phi placement are given; you write the
   RENAMING (the TODO(16) hole). Reference: solution/opt.ml.
...
(* ---- the default `-O` pipeline (mem2reg first, so folding/DCE see through memory) ---- *)
