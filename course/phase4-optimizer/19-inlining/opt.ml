(* The SIL optimizer — concept 19 (skeleton). Builds on 15–18. Adds INLINING: replace a call with the
   callee's body (v0: single-block leaf callees only). You fill the TODO(19) hole — the inline transform
   (renumber the callee's values, bind params to args, splice, return->result). Reference: solution/opt.ml. *)
