...
let check (prog : Ast.program) (diags : Diagnostics.sink) : Tast.program option =
  ...
  let rec infer (e : Ast.expr) : Tast.expr =
    ...
    | Ast.Nil span ->
        ...
        mk Tast.Nil (Types.TOptional Types.TInt) span
    | Ast.Force_unwrap (e0, span) -> (
        ...
        | t ->
            ...
            mk (Tast.Force_unwrap n) t span)
    | Ast.Coalesce (a, bexp, span) -> (
        let na = infer a in
        ...
        | t ->
            err span
              (Printf.sprintf "left operand of '??' must be optional, found '%s'"
                 (Types.string_of_ty t));
            mk (Tast.Coalesce (na, infer bexp)) t span)
    (* ternary `c ? a : b` — condition is Bool, both arms agree on a type = the result type *)
    | Ast.Ternary (c, a, bexp, span) ->
        let nc = check_expr c Types.TBool in
        let na = infer a in
        mk (Tast.Ternary (nc, na, check_expr bexp na.Tast.ty)) na.Tast.ty span
        ...
        | t ->
            err span
              (Printf.sprintf "value of type '%s' cannot be compared with 'nil'"
                 (Types.string_of_ty t));
            mk (Tast.Is_nil (n, op = Ast.Eq)) Types.TBool span)
    | _ -> infer_binary_base op l r span
  and infer_binary_base op l r span : Tast.expr =
  ...
  (* checking mode: an optional expected type accepts `nil`, an already-optional value, or — by
     implicit wrapping — a value of the wrapped type (`5 : Int?` becomes `.some(5)`). That wrap is
     a DECISION, and `Tast.Inject_optional` is where it is written down: swiftc inserts an
  ...
  and check_expr (e : Ast.expr) (expected : Types.ty) : Tast.expr =
    ...
    | Ast.Nil _, _ ->
        ...
        mk Tast.Nil expected (Ast.expr_span e)
    | _, Types.TOptional t ->
        let n = infer e in
        if Types.equal n.Tast.ty expected then n (* already a T? *)
        else
          (* wrap T -> T?, and RECORD it *)
          mk (Tast.Inject_optional (check_expr e t)) expected (Ast.expr_span e)
    | _ -> check_expr_base e expected
  and check_expr_base (e : Ast.expr) (expected : Types.ty) : Tast.expr =
    match e with
    | Ast.Int_lit (n, span) ->
  ...
  let rec check_stmt (s : Ast.stmt) : Tast.stmt =
    ...
    | Ast.If_let { name; opt; then_blk; else_blk; span } -> (
        (* `if let x = opt`: opt must be optional; x is the unwrapped type, bound in the
           then-block. The resolved statement carries that unwrapped TYPE. *)
        let n = infer opt in
        match n.Tast.ty with
        | Types.TOptional t ->
            let tb = ref [] in
        ...
        | t ->
            err span
              (Printf.sprintf
                 ...
                 (Types.string_of_ty t));
            Tast.If_let
              { name; ty = Types.TInt; opt = n; then_blk = check_block then_blk;
                else_blk = Option.map check_block else_blk; span })
