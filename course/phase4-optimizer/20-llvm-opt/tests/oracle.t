...
force-unwrap of nil whose exit 133 must survive `-O`. Four of the twenty-four are about the
passes themselves: repeated subexpressions beside calls that must NOT be merged, two same-shaped
struct types plus two same-shaped enum types (the once-shipped GVN bug), a pair of leaves
inlined at several sites with asymmetric arguments, and mutual recursion beside a struct-returning
leaf — one that must stay, one that must go. Three more pin the front-end fixes this concept
carries: an optional stored property written with `nil` and then with a
