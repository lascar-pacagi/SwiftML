(* The TYPE-CHECKED AST for concept 13 — a *contract* (given).
...
and expr_kind =
  ...
  | Nil (* the `nil` literal, at the optional type the context gave it *)
  | Inject_optional of expr
      (* THE CONVERSION, made explicit. Swift wraps a `T` where a `T?` is expected, invisibly in
         the source; the checker decides it and this node RECORDS it, so the lowering does not
         have to notice. This is swiftc's `InjectIntoOptionalExpr` (Expr.h), inserted by CSApply
         and merely lowered by SILGen — the same division of labour. *)
  | Is_nil of expr * bool
      (* RESOLVED: `opt == nil` / `opt != nil` (the bool is true for `==`). The source compares
         against a literal; what it MEANS is a tag test, and saying so here keeps SILGen from
         having to spot a `nil` in the operand shape for itself. *)
  | Force_unwrap of expr (* `e!` — traps if nil *)
  | Coalesce of expr * expr (* `a ?? b` *)
...
type stmt =
  ...
  | If_let of {
      name : string;
      ty : Types.ty; (* the WRAPPED type the name is bound at — resolved *)
      opt : expr;
      then_blk : stmt list;
...
let rec dump_expr (x : expr) : string =
  ...
  | Nil -> Printf.sprintf "(nil : %s)" t
  | Inject_optional e0 -> Printf.sprintf "(some %s : %s)" (dump_expr e0) t
  | Is_nil (e0, eq) -> Printf.sprintf "(%s-nil %s : %s)" (if eq then "is" else "not") (dump_expr e0) t
  | Force_unwrap e0 -> Printf.sprintf "(! %s : %s)" (dump_expr e0) t
  | Coalesce (a, bb) -> Printf.sprintf "(?? %s %s : %s)" (dump_expr a) (dump_expr bb) t
  | Ternary (c, a, bb) ->
      Printf.sprintf "(?: %s %s %s : %s)" (dump_expr c) (dump_expr a) (dump_expr bb) t
...
let rec dump_stmt = function
  ...
  | If_let { name; ty; opt; then_blk; else_blk; _ } ->
      Printf.sprintf "(if-let %s:%s %s %s%s)" name (Types.string_of_ty ty) (dump_expr opt)
        (dump_block then_blk)
        (match else_blk with None -> "" | Some b -> " " ^ dump_block b)
