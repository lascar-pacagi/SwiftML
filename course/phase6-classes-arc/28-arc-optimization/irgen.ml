(* IRGen (given) — concept 26's runtime + the ownership instruction lowerings (concept 27). *)
...
let emit_llvm (m : Sil.modul) : string =
  ...
  let gen_func (f : Sil.func) =
    ...
    let gen_instr (v, i) =
      ...
      | Sil.Copy_value o ->
          (* a copy IS a retain; the new SIL value is the same pointer *)
      ...
      | Sil.Destroy_value o -> p (Printf.sprintf "  call void @rt.release(ptr %s)\n" (lookup_operand o))
      | Sil.Load_take a ->
  ...
  let class_defs =
    List.map
      ...
      m.Sil.classes
  in
  let type_defs = struct_defs @ enum_defs @ proto_defs @ class_defs in
  ...
  ^ Buffer.contents globals ^ "\n" ^ Buffer.contents vt_buf ^ Buffer.contents wt_buf ^ Buffer.contents out
