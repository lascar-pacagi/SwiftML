(* FROZEN SOLUTION — concept 28 optimizer: copy propagation + WMO class devirt (M6). *)
...
let has_side_effect : Sil.instr -> bool = function
  ...
  | Sil.Copy_value _ | Sil.Destroy_value _ (* refcount traffic IS behavior: deinit timing — 26/27 *) -> true
  | _ -> false
...
let value_key (canon : Sil.value -> Sil.value) (rty : Types.ty) : Sil.instr -> string option =
  ...
  | Sil.Alloc_ref _ | Sil.Apply_class _ | Sil.Copy_value _ | Sil.Destroy_value _
...
let inlinable (f : Sil.func) : bool =
  ...
  && (match f.Sil.blocks with
     | [ b ] ->
         not
           (List.exists
              (fun (_, i) -> match i with Sil.Apply _ | Sil.Apply_witness _ | Sil.Apply_class _ -> true | _ -> false)
              b.Sil.instrs)
     | _ -> false)
...
let consumed_ops (f : Sil.func) (i : Sil.instr) : Sil.value list =
  let is_class v = match Hashtbl.find_opt f.Sil.val_ty v with Some (Types.TClass _) -> true | _ -> false in
  match i with
