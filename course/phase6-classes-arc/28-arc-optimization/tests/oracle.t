...
check what a program computes, it checks the exact moment every object dies. It is the phase's
accumulated corpus — the twelve lifetime scenarios and then some — because THAT is the soundness
gate for an ARC optimizer: a pass that changes when an object dies is a bug, not an
optimization, and here the same programs run at `-O` with copy propagation and whole-module
devirtualization both firing. Four more are this concept's own: the adversarial KEEP case (a
load-sourced copy whose slot is overwritten inside the bracket), three chained copies called
twice, a devirtualizable class beside an overridden hierarchy, and a borrow-copy inside a hot
loop.
