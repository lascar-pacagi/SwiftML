(* FROZEN SOLUTION — concept 26 IRGen: the refcount runtime, destructor in vtable slot 0. *)
...
let emit_llvm (m : Sil.modul) : string =
  ...
  let gen_func (f : Sil.func) =
    ...
    let gen_instr (v, i) =
      ...
      (* ARC — concept 26 *)
      | Sil.Retain o -> p (Printf.sprintf "  call void @rt.retain(ptr %s)\n" (lookup_operand o))
      | Sil.Release o -> p (Printf.sprintf "  call void @rt.release(ptr %s)\n" (lookup_operand o))
      (* classes — concept 25 *)
      | Sil.Alloc_ref cn ->
          let cl = clay cn in
