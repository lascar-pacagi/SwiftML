...
swiftc and only type-checked by us (this is concept 25's loop, kept because every program here
has an initializer and half of them have two):
...
check what a program computes, it checks the exact moment every object dies. The 18 programs are
the twelve lifetime scenarios and then some — scope exit in reverse declaration order, a shared
object released once, reassignment evaluating the new object before releasing the old, statement
temporaries dying at the end of their statement, arguments borrowed, a return as a +1 transfer,
loop iterations, `break` and `continue`, field replacement, an inheritance chain, cross-level
fields, the two-chain deallocation order, a `deinit` reading a field its own class mutated, a
dispatch through an overridden method, and two boxes sharing one object.
