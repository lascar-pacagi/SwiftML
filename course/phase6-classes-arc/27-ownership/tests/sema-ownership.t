...
It is green from the first run: the hole is in `sil.ml`, and `--typecheck` stops long before
any SIL exists.
...
The class rules inherited from concepts 25 and 26 are unchanged, and the corpus depends on
them: the override diagnostics, `let` properties, and the refusal to `print` or `==` an
aggregate.
