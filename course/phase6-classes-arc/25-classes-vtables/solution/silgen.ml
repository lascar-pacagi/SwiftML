(* FROZEN SOLUTION — concept 25 SILGen: reference semantics, constructors, vtable dispatch. *)
...
let lower_func ?(generic = false) structs enums protos methods funcs gfuncs classes (name : string)
    ...
    List.concat_map
      (function
        ...
        | Ast.IClass c ->
            ...
            in
            (* Swift's IMPLICIT `super.init()`: an initializer that never calls it gets the call
               appended when the superclass's initializer takes no arguments (sema rejected the
               other case). swiftc inserts it at the end of the initializer's phase 1. *)
            let with_implicit_super (init : Ast.func_decl) : Ast.func_decl =
              let rec calls_super (st : Ast.stmt) =
                ...
                | Ast.Switch { cases; default; _ } ->
                    List.exists (fun (_, blk) -> any blk) cases || opt default
                | _ -> false
              ...
              if c.Ast.csuper = None || any init.Ast.body || not zero_arg_super then init
                ...
                { init with
                  Ast.body =
                    init.Ast.body
                    @ [ Ast.Expr_stmt (Ast.Method_call (Ast.Var ("super", sp), "init", [], sp), sp) ] }
            in
            (match c.Ast.cinit with Some init -> [ lower_m (with_implicit_super init) ] | None -> [])
