(* IRGen for concept 25 (skeleton) — heap objects + vtables. Object layout, Alloc_ref (the
   header), and field addressing are given; you emit the DISPATCH (TODO(25c)). *)
...
let emit_llvm (m : Sil.modul) : string =
  ...
  List.iter
    (fun (cl : Types.class_layout) ->
      let entries = List.map (fun fn -> "ptr @" ^ fn) cl.Types.cl_impls in
      if entries = [] then
        Buffer.add_string vt_buf (Printf.sprintf "@vtbl.%s = private unnamed_addr constant [1 x ptr] [ptr null]\n" cl.Types.cl_name)
      else
