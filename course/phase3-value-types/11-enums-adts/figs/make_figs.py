...
def enum_value(ax, x, y, title, tag, cells):
    ...
    cw = 1.15
    w = cw * n + 0.2
    ax.text(x, y + 0.95, title, ha="center", fontsize=12,
            family="monospace", color=TEXT, fontweight="bold")
    x0 = x - w / 2 + 0.1
    # tag cell
    ax.add_patch(Rectangle((x0, y - 0.4), cw, 0.8, facecolor=TAG, edgecolor=EDGE, linewidth=1.2))
    ax.text(x0 + cw / 2, y, f"tag={tag}", ha="center", va="center",
            fontsize=11, family="monospace", color=TEXT)
    for i, (txt, live) in enumerate(cells):
        ...
        ax.text(cx + cw / 2, y, txt, ha="center", va="center", fontsize=11,
                family="monospace",
...
def make_tagged_union():
    ...
    ax.text(6, 4.5, "enum Shape { case circle(Int);  case rect(Int, Int);  case dot }",
            ha="center", fontsize=13, family="monospace", color=TEXT)
