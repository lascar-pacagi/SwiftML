...
An associated-value list cannot be empty in this subset.
  ...
  $ python3 timeout.py 0.5 ./lab.exe --emit-ast bad-payload-type.swift 2>&1 | head -1
...
An associated-value list requires its closing parenthesis.
  ...
  $ python3 timeout.py 0.5 ./lab.exe --emit-ast bad-payload-close.swift 2>&1 | head -1
  1:25: error: expected ')'

Two case declarations require a newline or semicolon between them.

  $ printf 'enum E { case a case b }\n' > bad-separator.swift
  $ python3 timeout.py 0.5 ./lab.exe --emit-ast bad-separator.swift 2>&1 | head -1
