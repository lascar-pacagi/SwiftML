(* Concept 11 runtime comparison: cycle through a four-case enum and read its tag.
...
let source =
  "phase3-value-types/11-enums-adts/bench/programs/enum-cycle.swift"
...
let make_temp_directory () =
  let path = Filename.temp_file "swiftml-bench11-" "" in
  Sys.remove path;
