...
let check (program : Ast.program) (diagnostics : Diagnostics.sink) : unit =
  ...
  let rec is_int_literal = function
    ...
    | Ast.Binary
        ((Ast.Add | Ast.Sub | Ast.Mul | Ast.Div | Ast.Mod), left, right, _) ->
  ...
  let rec infer_expression (expression : Ast.expr) : Types.ty =
    ...
    | Ast.Member (operand_expression, field_name, span) -> (
        ...
        | base_type ->
            report_error span
              (Printf.sprintf "value of type '%s' has no member '%s'"
                 (Types.string_of_ty base_type)
                 field_name);
            Types.TInt)
  and infer_binary operator left_expression right_expression span : Types.ty =
    let left_type = infer_expression left_expression
    and right_type = infer_expression right_expression in
    ...
    | Ast.Add -> (
        match
          common_operand_type left_expression left_type right_expression
            right_type
        with
            ...
            if expected_count <> actual_count then
              ...
              List.iter2
                (fun argument parameter_type ->
                  check_expr argument parameter_type)
                argument_expressions parameter_types;
            return_type
        | None ->
            if function_name = "print" then (
              ...
              | [ printed_expression ] -> (
                  ...
                  | unsupported_type ->
                      report_error
                        (Ast.expr_span printed_expression)
                        (Printf.sprintf
                           "cannot print a value of type '%s' (only Int, \
                            Double, Bool and String)"
                           (Types.string_of_ty unsupported_type)))
              | _ ->
                  report_error span "print(_:) expects exactly one argument";
                  List.iter
                    (fun argument -> ignore (infer_expression argument))
                    argument_expressions);
              Types.TVoid)
            else (
              report_error span
                (Printf.sprintf "cannot find '%s' in scope" function_name);
              List.iter
                (fun argument -> ignore (infer_expression argument))
                argument_expressions;
              Types.TInt))
  ...
  and infer_initializer struct_name (layout : Types.struct_layout)
      (arguments : Ast.arg list) span : Types.ty =
    let fields = layout.Types.sl_fields in
    if List.length arguments <> List.length fields then
      report_error span
        (Printf.sprintf "'%s' initializer expects %d argument(s) but %d given"
           struct_name (List.length fields) (List.length arguments))
    else
      List.iter2
        (fun (label, value) (field_name, field_type) ->
          ...
          | Some supplied_label when supplied_label <> field_name ->
              report_error (Ast.expr_span value)
                (Printf.sprintf
                   "incorrect argument label in call (have '%s:', expected \
                    '%s:')"
                   supplied_label field_name)
          | None ->
              report_error (Ast.expr_span value)
                (Printf.sprintf "missing argument label '%s:' in call"
                   field_name)
          | _ -> ());
  ...
  and check_expr (expression : Ast.expr) (expected : Types.ty) : unit =
    ...
    | Ast.Binary (Ast.Mod, left_expression, right_expression, _)
      when expected = Types.TInt ->
        check_expr left_expression Types.TInt;
        check_expr right_expression Types.TInt
    | Ast.Unary (Ast.Neg, operand_expression, _) when Types.is_numeric expected
      ->
        check_expr operand_expression expected
    | _ ->
        ...
        if not (Types.equal actual_type expected) then
          report_error (Ast.expr_span expression)
            (Printf.sprintf
               ...
               (Types.string_of_ty actual_type)
               (Types.string_of_ty expected))
  in
  ...
  let rec check_statement (statement : Ast.stmt) : unit =
    ...
    | Ast.Let { name; is_var; annot; value; span } ->
        let binding_type =
          ...
          | Some type_name -> (
              ...
              | Some resolved_type ->
                  check_expr value resolved_type;
                  resolved_type
              | None ->
                  report_error span
                    (Printf.sprintf "cannot find type '%s' in scope" type_name);
    ...
    | Ast.Set_member { obj = object_name; field = field_name; value; span } -> (
        ...
        | None ->
            report_error span
              (Printf.sprintf "cannot find '%s' in scope" object_name);
            ignore (infer_expression value)
        | Some (Types.TStruct struct_name, is_var) -> (
            match
              Option.bind (Hashtbl.find_opt struct_layouts struct_name)
                (fun layout -> Types.field_type layout field_name)
            with
            | Some field_type ->
                ...
                if not is_var then
                  report_error span
                    (Printf.sprintf
                       ...
                       object_name)
                else if Hashtbl.mem immutable_fields (struct_name, field_name)
                then
                  report_error span
                    (Printf.sprintf
                       "cannot assign to property: '%s' is a 'let' constant"
    ...
    | Ast.If
        { cond = condition; then_blk = then_block; else_blk = else_block; _ } ->
    ...
    | Ast.While { cond = condition; body; _ } ->
        ...
        decr loop_depth
    | Ast.For
        { var = loop_variable; lo = lower_bound; hi = upper_bound; body; _ } ->
  ...
  and check_block (statements : Ast.stmt list) : unit =
    within_scope (fun () -> List.iter check_statement statements)
  in
  ...
  let check_function (function_decl : Ast.func_decl) : unit =
    let return_type =
      ...
      | Some written_return_type ->
          resolve_type function_decl.Ast.fspan written_return_type
    in
    let saved_environment = !environment
    and saved_return_type = !current_return_type in
    environment := [];
    current_return_type := Some return_type;
    List.iter
      (fun (parameter : Ast.param) ->
        bind_name parameter.Ast.pname
          (resolve_type function_decl.Ast.fspan parameter.Ast.ptype, false))
      function_decl.Ast.params;
    List.iter check_statement function_decl.Ast.body;
    environment := saved_environment;
    current_return_type := saved_return_type;
    if return_type <> Types.TVoid && not (block_returns function_decl.Ast.body)
    then
      report_error function_decl.Ast.fspan
        (Printf.sprintf "missing return in %s expected to return '%s'"
           "global function"
