(* ANSWER KEY — concept 10 IRGen. Carries Phase-2 IRGen and lowers the three struct
   instructions to LLVM aggregate operations.
...
let emit_llvm (sil_module : Sil.modul) : string =
  ...
  let gen_function (function_definition : Sil.func) =
    ...
    let gen_instruction (value, instruction) =
      ...
      (* structs — concept 10 *)
      | Sil.Struct fields ->
          (* Build the aggregate value with a chain of insertvalue from [undef]. *)
          let struct_type = llvm_type (value_type value) in
          let aggregate_operand = ref "undef" in
          List.iteri
            (fun field_index field_value ->
              let result_operand = fresh_temp () in
              emit
                (Printf.sprintf "  %s = insertvalue %s %s, %s %s, %d\n"
                   result_operand struct_type !aggregate_operand
                   (llvm_type (value_type field_value)) (lookup_operand field_value)
                   field_index);
              aggregate_operand := result_operand)
            fields;
      ...
      | Sil.Struct_extract (aggregate, field_index) ->
          ...
          emit
            (Printf.sprintf "  %s = extractvalue %s %s, %d\n" result_operand
               (llvm_type (value_type aggregate)) (lookup_operand aggregate) field_index);
          bind_operand value result_operand
      | Sil.Struct_element_addr (address, field_index) ->
          let result_operand = fresh_temp () in
          emit
            (Printf.sprintf "  %s = getelementptr %s, ptr %s, i32 0, i32 %d\n"
               result_operand (llvm_type (value_type address)) (lookup_operand address)
               field_index);
          bind_operand value result_operand
    in
