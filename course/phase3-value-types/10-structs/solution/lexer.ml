...
type t = {
  ...
  diagnostics : Diagnostics.sink;
      (* errors are REPORTED, not raised — see [report_error] below *)
}
...
let make_token (start_position : Token.pos) (lexer : t) (kind : Token.kind) :
    Token.t =
  {
    Token.kind;
    span = { Token.lo = start_position; hi = current_position lexer };
  }
...
let report_error (lexer : t) (start_position : Token.pos) (message : string) :
    unit =
  Diagnostics.error lexer.diagnostics
    { Token.lo = start_position; hi = current_position lexer }
    message
...
let rec next (lexer : t) : Token.t =
  ...
  let rec skip_trivia () =
    if at_end lexer then ()
      ...
      if character = ' ' || character = '\t' || character = '\r' then (
        ...
        if !depth > 0 then
          report_error lexer (current_position lexer)
  ...
  if at_end lexer then make_token start_position lexer Token.Eof
    ...
    if is_digit character then (
      ...
      if peek_char lexer = '.' && is_digit (peek_next_char lexer) then (
        ...
        make_token start_position lexer
          (Token.Float
             (float_of_string
                (String.sub lexer.source start_offset (lexer.pos - start_offset)))))
      else
        make_token start_position lexer
          (Token.Int
             (int_of_string
      ...
      make_token start_position lexer (Token.String (scan_string lexer))
      ...
      | '+' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.Plus
      | '-' ->
          ignore (advance_char lexer);
          if peek_char lexer = '>' then (
            ignore (advance_char lexer);
            make_token start_position lexer Token.Arrow)
          else make_token start_position lexer Token.Minus
      | '*' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.Star
      | '/' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.Slash
      | '%' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.Percent
      | '=' ->
          ignore (advance_char lexer);
          if peek_char lexer = '=' then (
            ignore (advance_char lexer);
            make_token start_position lexer Token.EqEq)
          else make_token start_position lexer Token.Eq
      | '!' ->
          ignore (advance_char lexer);
          if peek_char lexer = '=' then (
            ignore (advance_char lexer);
            make_token start_position lexer Token.Ne)
          else (
            report_error lexer start_position "expected '=' after '!'";
            next lexer)
      | '<' ->
          ignore (advance_char lexer);
          if peek_char lexer = '=' then (
            ignore (advance_char lexer);
            make_token start_position lexer Token.Le)
          else make_token start_position lexer Token.Lt
      | '>' ->
          ignore (advance_char lexer);
          if peek_char lexer = '=' then (
            ignore (advance_char lexer);
            make_token start_position lexer Token.Ge)
          else make_token start_position lexer Token.Gt
      | ':' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.Colon
      | '(' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.LParen
      | ')' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.RParen
      | '{' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.LBrace
      | '}' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.RBrace
      | ',' ->
          ignore (advance_char lexer);
          make_token start_position lexer Token.Comma
      | '&' ->
          ignore (advance_char lexer);
          if peek_char lexer = '&' then (
            ignore (advance_char lexer);
            make_token start_position lexer Token.AmpAmp)
          else (
            report_error lexer start_position "expected '&' after '&'";
            next lexer)
      | '|' ->
          ...
          if peek_char lexer = '|' then (
            ignore (advance_char lexer);
            make_token start_position lexer Token.PipePipe)
          else (
            report_error lexer start_position "expected '|' after '|'";
            next lexer)
      | '.' ->
          ...
          if peek_char lexer = '.' && peek_next_char lexer = '<' then (
            ...
            make_token start_position lexer
              Token.Dot (* member access — concept 10 *)
      | '\n' ->
          ignore (advance_char lexer);
      ...
      | ';' ->
          ignore (advance_char lexer);
          make_token start_position lexer
            Token.Newline (* `;` is a statement separator in Swift *)
      | _ ->
...
let tokenize (lexer : t) : Token.t list =
  let rec loop reversed_tokens =
    ...
    | Token.Eof -> List.rev (token :: reversed_tokens)
    | _ -> loop (token :: reversed_tokens)
  in
  loop []
