(* FROZEN SOLUTION — concept 10 SILGen (Phase-2 + structs): lower the (checked) AST to raw, memory-based SIL.
...
let lower_func structs funcs (name : string) (params : (string * Types.ty) list) (ret : Types.ty)
    (body : Ast.stmt list) : Sil.func =
  let val_ty = Hashtbl.create 16 in
  let entry = { Sil.bid = 0; instrs = []; term = Sil.Unreachable } in
  let b =
    { next_val = 0; next_block = 1; cur = entry; blocks = [ entry ]; vars = Hashtbl.create 16; val_ty; funcs; structs; loops = [] }
  in
...
let lower (prog : Ast.program) : Sil.modul =
  ...
  List.iter
    ...
    prog.Ast.items;
  let ty_of_name n =
    match Types.of_name n with Some t -> t | None -> if Hashtbl.mem structs n then Types.TStruct n else Types.TInt
  in
  List.iter
    (function
      | Ast.IStruct s ->
