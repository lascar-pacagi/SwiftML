(* FROZEN SOLUTION — concept 10 SILGen: lower the TYPE-CHECKED tree (`Tast`) — Phase-2 +
...
let rec gen_expr (builder : builder) (expression : Tast.expr) : Sil.value =
  ...
  (* read a field out of a struct VALUE with struct_extract. The INDEX came from Sema, so there
     is no layout to look up and no name to match — and no way to disagree about either. *)
  | Tast.Field (operand_expression, index, _) ->
      let struct_value = gen_expr builder operand_expression in
      emit builder (Sil.Struct_extract (struct_value, index)) ty
...
and gen_stmt (builder : builder) (statement : Tast.stmt) : unit =
  ...
  | Tast.Set_member { obj = object_name; field = index; value; _ } ->
      (* `p.x = expression`: take the field's ADDRESS in p's slot, then store — this is what
         makes a struct a value type, since p has its own slot distinct from any copy. The index
         is Sema's; SILGen looks nothing up. *)
      let stored = gen_expr builder value in
      let object_address = Hashtbl.find builder.variables object_name in
      let field_address =
        emit builder (Sil.Struct_element_addr (object_address, index)) (value_type builder stored)
      in
      emit_void builder (Sil.Store (stored, field_address))
