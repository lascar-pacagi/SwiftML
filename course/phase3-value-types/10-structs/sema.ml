(* Sema — concept 10 (skeleton): concepts 05–07 plus structs. The earlier concepts are filled
   in; you write the TODO(10) holes. Reference: solution/sema.ml.

   Concepts 05–07 plus structs.
...
let check (program : Ast.program) (diagnostics : Diagnostics.sink) : Tast.program option =
  ...
  let rec check_statement (statement : Ast.stmt) : Tast.stmt =
    ...
    | Ast.Set_member { obj; field; value; span } ->
        ...
        (* TODO(10f): resolve the binding and field, reject a [let] binding or [let] field, and
           check the assigned value against the field type (§2). As in 10e, the `Tast` statement
           records the field's INDEX. *)
  ...
  (* TODO(10d): PASS 0 — register struct NAMES first (so a field can reference another struct),
     then fill in the layouts. After this, any written type name resolves and the struct types
     are known to passes 1 and 2. A duplicate name is an error. §2. *)
  ignore (immutable_fields, resolve_type);
