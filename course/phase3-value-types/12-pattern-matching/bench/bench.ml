(* Concept 12 runtime comparison: two eight-case switches per iteration.

   The runner checks output before timing. Timings are the best of five executions
   after one warm-up. Run from course/:

     make bench C=phase3-value-types/12-pattern-matching *)

let source =
  "phase3-value-types/12-pattern-matching/bench/programs/switch-dispatch.swift"
...
let run_quiet program arguments =
  ...
  let process_id =
    Unix.create_process program arguments Unix.stdin null null
  in
...
let make_temp_directory () =
  let path = Filename.temp_file "swiftml-bench12-" "" in
  Sys.remove path;
...
let () =
  ...
  Fun.protect
    ...
    (fun () ->
      Printf.printf
        "Switch benchmark -- 40,000,000 dispatches through an eight-case switch\n%!";
      ...
      in
      let swiftml_o2_time =
        time_and_report "swiftml + clang -O2" swiftml_o2
      in
      let swiftc_onone_time =
        time_and_report "swiftc -Onone" swiftc_onone
      in
