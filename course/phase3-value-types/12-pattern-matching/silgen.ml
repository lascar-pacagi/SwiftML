(* SILGen — concept 12 (skeleton). Reference: solution/silgen.ml. Original note: lowers the TYPE-CHECKED tree (`Tast`). Sema resolved
...
and gen_stmt (b : builder) (s : Tast.stmt) : unit =
  ...
  | Tast.Switch { subject; cases; default; _ } ->
      (* TODO(12f): lower the switch to a DISPATCH CHAIN — evaluate the subject once, compare its
         ...
         Note what Sema hands you: each pattern's TAG and each binding's TYPE, so the chain
         cannot select a different case than the exhaustiveness check reasoned about. *)
