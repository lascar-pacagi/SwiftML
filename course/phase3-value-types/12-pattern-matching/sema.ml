        ...
        let cs =
          List.map
            (fun (pat, body) ->
              ...
              (* TODO(12d): type one switch arm. An `Ast.PEnumCase` names a case of the subject's
                 enum: check it exists, check the number of bindings matches the payload, and bind
                 each `Ast.Bind` name to that payload slot's type — immutably — for the arm's body
                 only (`in_scope` gives you a scope that is popped again). Record the case name in
                 `covered`, which TODO(12e) needs. An `Ast.PInt` cannot match an enum.
                 The `Tast` pattern you return carries the case's TAG and each binding's TYPE, so
                 the dispatch lowering has nothing left to resolve. §2's table has the wording. §3. *)
