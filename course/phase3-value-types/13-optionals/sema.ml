  ...
  and infer_binary_base op l r span : Types.ty =
    ...
    | Ast.Eq | Ast.Ne -> (
        ...
        | Some (Types.TEnum en) when Types.has_payload (Hashtbl.find enums en) ->
            ...
            Types.TBool
        (* structs and optionals would need an Equatable conformance too, and the back end has no
           aggregate compare — `opt == nil` is the one optional comparison, handled above *)
