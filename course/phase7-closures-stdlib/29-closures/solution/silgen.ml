(* FROZEN SOLUTION — concept 29 SILGen: closure lifting, captures, thin->thick, indirect calls. *)
  ...
  | Ast.Closure (params, retname, body, _) ->
      ...
      let caps =
        List.fold_left
          ...
          [] fvs
      in
      (* the captured VALUES, evaluated now (by-value capture) *)
      ...
      incr b.clo_count;
      let rty =
        match retname with Some n -> resolve_name b n | None -> Types.TVoid (* fixed below *)
      in
      (* infer the body type when unannotated: lower a throwaway? No — sema typed it; we
         re-derive from the lifted function's Return. Lower with the annotated/inferred type:
         v0 requires the lowering to know it — use the annotation, else lower the body once
         in THIS builder context? Simplest sound: lower the lifted function with ret = the
         annotation if present; otherwise patch after lowering from the body value's type. *)
...
let lower (prog : Ast.program) : Sil.modul =
    ...
    List.concat_map
      (function
        | Ast.IFunc f when f.Ast.generics <> [] ->
            ...
            [ lower_func ~generic:true ~lifted structs enums protos methods funcs gfuncs classes f.Ast.fname params (erase ret) f.Ast.body ]
        | Ast.IFunc f ->
            let params = List.map (fun (pr : Ast.param) -> (pr.Ast.pname, ty_of_name pr.Ast.ptype)) f.Ast.params in
            [ lower_func ~lifted structs enums protos methods funcs gfuncs classes f.Ast.fname params (ret_of f) f.Ast.body ]
        | Ast.IClass c ->
            ...
            let lower_m ?(init = false) ?epilogue (m : Ast.func_decl) =
              ...
              in
              lower_func ~init ?epilogue ~lifted structs enums protos methods funcs gfuncs classes
        ...
        | Ast.IStruct st ->
            ...
            List.map
              (fun (m : Ast.func_decl) ->
                ...
                in
                lower_func ~lifted structs enums protos methods funcs gfuncs classes
  ...
  let main = lower_func ~lifted structs enums protos methods funcs gfuncs classes "main" [] Types.TVoid main_body in
  let struct_layouts =
