  ...
  let rec infer (e : Ast.expr) : Types.ty =
    ...
    | Ast.Method_call (e0, m, args, span) -> (
        ...
        | Types.TArray el -> (
            match (m, exprs) with
            | "append", [ _ ] ->
                (* `append` MUTATES the array. A `let` binding to an array is a constant value,
                   not a constant reference to a mutable buffer — swiftc refuses this, and so
                   must we, or `let a = [1,2]; a.append(3)` silently grows a constant. The
                   subscript-set path already checked this; the method call did not. *)
                (match e0 with
                | Ast.Var (nm, _) -> (
                    match lookup nm with
                    | Some (_, false) ->
                        err span
                          (Printf.sprintf
                             "cannot use mutating member on immutable value: '%s' is a 'let' constant" nm)
                    | _ -> ())
                | _ -> ());
                check_args "method 'append'" [ el ];
                Types.TVoid
            ...
            | "reduce", [ init; f ] -> (
                ...
                | tf ->
                    ...
                    r)
            (* a KNOWN member called with the wrong number of arguments: say so, rather than
               claiming the array has no such member and then cascading on the '()' result *)
            | ("append" | "map" | "filter"), _ ->
                err span (Printf.sprintf "method '%s' expects 1 argument(s) but %d given" m (List.length exprs));
                List.iter (fun a -> ignore (infer a)) exprs;
                if m = "append" then Types.TVoid else Types.TArray el
            | "reduce", _ ->
                err span (Printf.sprintf "method 'reduce' expects 2 argument(s) but %d given" (List.length exprs));
                List.iter (fun a -> ignore (infer a)) exprs;
                Types.TInt
            | _ ->
