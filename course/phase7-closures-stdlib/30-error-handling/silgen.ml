(* SILGen for concept 29 (skeleton) — closures. FV analysis, indirect calls, and the
   thin->thick path are given; you implement the LIFTING (TODO(29a)). *)
...
and emit_error_check (b : builder) : unit =
  (* TODO(30a): after a throwing call, branch on the error register — nonzero takes the error
     edge, zero continues with the rest of the statement. Errors travel in that one register
     precisely so this is a branch and not unwinding (§2). The error edge needs a block of its
     own: `goto_handler` (given, just above) emits the jump to the current handler along with
     the defers and releases that jump crosses. Leave the builder positioned on the CONTINUE
     block, since the caller carries on emitting there. *)
  ignore (rt_error_get, goto_handler);
