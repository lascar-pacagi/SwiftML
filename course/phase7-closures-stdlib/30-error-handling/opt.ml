(* The SIL optimizer (given) — Phases 4-7. Concept 30 (error handling) is PURE DESUGARING:
   throws/try/do-catch lower to ordinary branches + an error-out parameter; no new SIL. *)
...
let operands : Sil.instr -> Sil.value list = function
  ...
  | Sil.Init_existential (p, _, _) -> [ p ]
  | Sil.Apply_witness (ex, _, args) -> ex :: args
  | Sil.Open_existential (ex, _) -> [ ex ]
  | Sil.Same_witness (ex, _) -> [ ex ]
  | Sil.Alloc_ref _ -> []
  | Sil.Copy_value o -> [ o ]
  | Sil.Destroy_value o -> [ o ]
  | Sil.Load_take a -> [ a ]
  | Sil.Ref_element_addr (o, _) -> [ o ]
  | Sil.Apply_class (o, _, args) -> o :: args
  | Sil.Upcast (o, _) -> [ o ]
  | Sil.Closure (_, caps) -> caps
  | Sil.Thin_to_thick _ -> []
  | Sil.Apply_value (fv, args) -> fv :: args
  | Sil.Capture_get (ctx, _) -> [ ctx ]
...
(* an instruction we must keep even if its result is unused (it does something observable).
   `Apply_witness`/`Apply_class` are calls; `Alloc_ref` is an allocation whose lifetime concept
   26 (ARC) will manage — dropping one would change deinit behavior, so it stays. *)
let has_side_effect : Sil.instr -> bool = function
  | Sil.Store _ | Sil.Apply _ | Sil.Print _ | Sil.Apply_witness _ | Sil.Apply_class _ | Sil.Alloc_ref _
  | Sil.Copy_value _ | Sil.Destroy_value _ (* refcount traffic IS behavior: deinit timing — 26/27 *)
  | Sil.Closure _ (* an allocation, like Alloc_ref — concept 29 *)
  | Sil.Apply_value _ (* an indirect CALL — concept 29 *) -> true
  | _ -> false
...
let map_instr (g : Sil.value -> Sil.value) : Sil.instr -> Sil.instr = function
  ...
  | Sil.Init_existential (p, sn, pn) -> Sil.Init_existential (g p, sn, pn)
  | Sil.Apply_witness (ex, slot, args) -> Sil.Apply_witness (g ex, slot, List.map g args)
  | Sil.Open_existential (ex, sn) -> Sil.Open_existential (g ex, sn)
  | Sil.Same_witness (ex, sn) -> Sil.Same_witness (g ex, sn)
  | Sil.Ref_element_addr (o, i) -> Sil.Ref_element_addr (g o, i)
  | Sil.Copy_value o -> Sil.Copy_value (g o)
  | Sil.Destroy_value o -> Sil.Destroy_value (g o)
  | Sil.Load_take a -> Sil.Load_take (g a)
  | Sil.Apply_class (o, slot, args) -> Sil.Apply_class (g o, slot, List.map g args)
  | Sil.Upcast (o, cn) -> Sil.Upcast (g o, cn)
  | Sil.Closure (fn, caps) -> Sil.Closure (fn, List.map g caps)
  | Sil.Apply_value (fv, args) -> Sil.Apply_value (g fv, List.map g args)
  | Sil.Capture_get (ctx, i) -> Sil.Capture_get (g ctx, i)
  | other -> other
...
let mem2reg (f : Sil.func) : Sil.func =
  ...
  let value_uses : Sil.instr -> Sil.value list = function
    | Sil.Load _ | Sil.Load_take _ -> [] (* the address is not a value use *)
    ...
    List.iter
      (fun (b : Sil.block) ->
        ...
        List.iter
          (fun (_, i) ->
            match i with
            | (Sil.Load a | Sil.Load_take a) when is_promo a -> if not (ISet.mem a !k) then g := ISet.add a !g
    ...
    let rec rename n (cur : Sil.value IMap.t) =
      ...
      List.iter
        (fun (v, i) ->
          ...
          | (Sil.Load a | Sil.Load_take a) when is_promo a ->
              Hashtbl.replace replace v (IMap.find a !cur); Hashtbl.replace removed v ()
          | _ -> ())
        (List.rev b.Sil.instrs);
...
let value_key (canon : Sil.value -> Sil.value) (rty : Types.ty) : Sil.instr -> string option =
  ...
  (* wrapping the same value for the same conformance is the same existential — CSE-able (21) *)
  | Sil.Init_existential (p, sn, pn) -> Some (Printf.sprintf "exist:%s:%s:%s" (v p) sn pn)
  (* opening is pure too: same existential, same concrete type -> same payload (22) *)
  | Sil.Open_existential (ex, sn) -> Some (Printf.sprintf "open:%s:%s" (v ex) sn)
  (* so is the identity test (23) *)
  | Sil.Same_witness (ex, sn) -> Some (Printf.sprintf "samew:%s:%s" (v ex) sn)
  (* field addresses and upcasts are pure address arithmetic (25); loads through them are NOT *)
  | Sil.Ref_element_addr (o, i) -> Some (Printf.sprintf "rea:%s:%d" (v o) i)
  | Sil.Upcast (o, cn) -> Some (Printf.sprintf "upcast:%s:%s" (v o) cn)
  (* thin->thick of the same function is the same value; captures are immutable so a
     capture_get of the same (ctx, i) is too — both CSE-able (29) *)
  | Sil.Thin_to_thick fn -> Some (Printf.sprintf "t2t:%s" fn)
  | Sil.Capture_get (ctx, i) -> Some (Printf.sprintf "capget:%s:%d" (v ctx) i)
  | Sil.Alloc_ref _ | Sil.Apply_class _ | Sil.Copy_value _ | Sil.Destroy_value _ | Sil.Closure _
  | Sil.Apply_value _
  | Sil.Load_take _ (* a take MOVES out of memory: not a pure value *) -> None
  | Sil.Load _ | Sil.Store _ | Sil.Apply _ | Sil.Print _ | Sil.Alloc_stack _ | Sil.Struct_element_addr _
  | Sil.Apply_witness _ (* dynamic dispatch = a call: never CSE'd (21) *) -> None
     ...
     | [ b ] ->
         not
           (List.exists
              (fun (_, i) ->
                match i with
                | Sil.Apply _ | Sil.Apply_witness _ | Sil.Apply_class _ | Sil.Apply_value _ -> true
                | _ -> false)
              b.Sil.instrs)
     | _ -> false)
...
let inline_module (m : Sil.modul) : Sil.modul =
  ...
  (* drop functions that are no longer CALLED — count only `apply`s (a dead func_ref left over from
     inlining doesn't count), so a fully-inlined callee is removed. A function named in a WITNESS
     TABLE is reachable through dynamic dispatch and must always be kept (concept 21). *)
  let called = Hashtbl.create 16 in
  List.iter (fun (_, _, fns) -> List.iter (fun fn -> Hashtbl.replace called fn ()) fns) m.Sil.wtables;
  List.iter (fun (cl : Types.class_layout) -> List.iter (fun fn -> Hashtbl.replace called fn ()) cl.Types.cl_impls) m.Sil.classes;
  List.iter (fun (cl : Types.class_layout) -> Hashtbl.replace called (cl.Types.cl_name ^ ".init") ()) m.Sil.classes;
  (* the synthesized DESTRUCTOR sits in vtable slot 0 — reachable from the runtime's release,
     invisible to any Apply scan (concept 26) *)
  List.iter (fun (cl : Types.class_layout) -> Hashtbl.replace called (cl.Types.cl_name ^ ".deinit") ()) m.Sil.classes;
  List.iter (fun (cl : Types.class_layout) -> Hashtbl.replace called (cl.Types.cl_name ^ ".destroy") ()) m.Sil.classes;
  (* lifted closures are reached via Closure instrs; thin_to_thick targets via their thunks (29) *)
  List.iter
    (fun (f : Sil.func) ->
      List.iter
        (fun (b : Sil.block) ->
          List.iter
            (fun (_, i) ->
              match i with
              | Sil.Closure (fn, _) | Sil.Thin_to_thick fn -> Hashtbl.replace called fn ()
              | _ -> ())
            b.Sil.instrs)
        f.Sil.blocks)
    m.Sil.funcs;
...
(* ================= concept 24: devirtualization + generic specialization ================= *)

(* DEVIRTUALIZATION — a peephole over SSA def-chains. When the optimizer can PROVE which
   concrete type an existential holds, the dynamic machinery folds away:
     apply_witness on a known wrap  ->  a DIRECT call to the witness implementation
     open_existential of a known wrap (or of an already-concrete value)  ->  the payload itself
     same_witness of a known wrap  ->  a constant Bool
   Provable = the value's SSA definition is an `init_existential` (so run AFTER mem2reg), or —
   after specialization retypes a clone — the value is simply a concrete struct already. *)
let devirt_module (m : Sil.modul) : Sil.modul =
  let impl_of pn sn slot =
    List.find_map (fun (p, s, fns) -> if p = pn && s = sn then List.nth_opt fns slot else None) m.Sil.wtables
  in
  let run_func (f : Sil.func) =
    let def : (Sil.value, Sil.instr) Hashtbl.t = Hashtbl.create 64 in
    List.iter (fun (b : Sil.block) -> List.iter (fun (v, i) -> Hashtbl.replace def v i) b.Sil.instrs) f.Sil.blocks;
    let vty v = Hashtbl.find_opt f.Sil.val_ty v in
    let next = ref (1 + List.fold_left (fun mx (b : Sil.block) -> List.fold_left (fun mx (v, _) -> max mx v) (List.fold_left (fun mx (v, _) -> max mx v) mx b.Sil.args) b.Sil.instrs) (List.fold_left (fun mx (v, _) -> max mx v) 0 f.Sil.params) f.Sil.blocks) in
    let fresh ty = let v = !next in incr next; Hashtbl.replace f.Sil.val_ty v ty; v in
    let alias : (Sil.value, Sil.value) Hashtbl.t = Hashtbl.create 16 in
    let wrap_of ex =
      (* the concrete (payload, struct, proto) if [ex] provably holds it. Two proofs:
         its SSA def is an init_existential (the wrap is right there), or — inside a
         specialized clone — the value is ALREADY a concrete struct; then the protocol comes
         from the type's conformances, bailing if it conforms to more than one (the slot
         number would be ambiguous). *)
      match Hashtbl.find_opt def ex with
      | Some (Sil.Init_existential (pv, sn, pn)) -> Some (Some pv, sn, pn)
      | _ -> (
          match vty ex with
          | Some (Types.TStruct sn) -> (
              match List.filter (fun (_, s, _) -> s = sn) m.Sil.wtables with
              | [ (pn, _, _) ] -> Some (None, sn, pn)
              | _ -> None)
          | _ -> None)
    in
    List.iter
          ...
          List.concat_map
            (fun (v, i) ->
              ...
              | Sil.Apply_class (o, slot, args) -> (
                  (* WHOLE-MODULE class devirtualization (concept 28): if the receiver's
                     static class has NO subclasses anywhere in the module, the dynamic type
                     can only be that class — the vtable lookup has one possible answer, so
                     call it directly (then the inliner can eat it). This is swiftc's
                     effectively-final/WMO devirt; `final` gets it without the analysis. *)
                  match Hashtbl.find_opt f.Sil.val_ty o with
                  | Some (Types.TClass cn)
                    when not
                           (List.exists
                              (fun (cl : Types.class_layout) -> cl.Types.cl_super = Some cn)
                              m.Sil.classes) -> (
                      match List.find_opt (fun (cl : Types.class_layout) -> cl.Types.cl_name = cn) m.Sil.classes with
                      | Some cl when slot < List.length cl.Types.cl_impls ->
                          let impl = List.nth cl.Types.cl_impls slot in
                          let fr = fresh (Option.value (vty v) ~default:Types.TVoid) in
                          [ (fr, Sil.Func_ref impl); (v, Sil.Apply (fr, o :: args)) ]
                      | _ -> [ (v, i) ])
                  | _ -> [ (v, i) ])
            ...
            b.Sil.instrs)
      f.Sil.blocks;
    (* chase aliases through every operand *)
    let rec res v = match Hashtbl.find_opt alias v with Some v' when v' <> v -> res v' | _ -> v in
    List.iter
      (fun (b : Sil.block) ->
        b.Sil.instrs <- List.map (fun (v, i) -> (v, map_instr res i)) b.Sil.instrs;
        b.Sil.term <- map_term res b.Sil.term)
      f.Sil.blocks;
    f
  in
  { m with Sil.funcs = List.map run_func m.Sil.funcs }

(* GENERIC SPECIALIZATION — clone a generic function per concrete type argument. The clone
   g$S retypes every erased value (TProto of T's constraint) to the concrete struct; the
   devirtualizer then turns its witness dispatches into direct calls, the inliner eats those,
   and the Phase-4 pipeline erases what's left. Sites whose T can't be proven stay on the
   erased original — both worlds coexist, exactly like swiftc. *)
let specialize_module (m : Sil.modul) : Sil.modul =
  let funcs = ref m.Sil.funcs in
  let find_fn n = List.find_opt (fun (f : Sil.func) -> f.Sil.fname = n) !funcs in
  (* can g be soundly retyped for constraint c? not if it builds its OWN existentials of c *)
  let retypable (g : Sil.func) (c : string) : bool =
    not
      (List.exists
         (fun (b : Sil.block) ->
           List.exists (fun (_, i) -> match i with Sil.Init_existential (_, _, pn) -> pn = c | _ -> false) b.Sil.instrs)
         g.Sil.blocks)
  in
  let clone_specialized (g : Sil.func) (sn : string) : Sil.func =
    let subst = function Types.TProto _ -> Types.TStruct sn | t -> t in
    let val_ty = Hashtbl.copy g.Sil.val_ty in
    Hashtbl.iter (fun v t -> Hashtbl.replace val_ty v (subst t)) (Hashtbl.copy val_ty);
    let blocks =
      List.map
        (fun (b : Sil.block) ->
          { Sil.bid = b.Sil.bid;
            args = List.map (fun (v, t) -> (v, subst t)) b.Sil.args;
            instrs = b.Sil.instrs;
            term = b.Sil.term })
        g.Sil.blocks
    in
    { Sil.fname = g.Sil.fname ^ "$" ^ sn;
      params = List.map (fun (v, t) -> (v, subst t)) g.Sil.params;
      ret = subst g.Sil.ret;
      generic = false;
      blocks;
      val_ty }
            ...
            b.Sil.instrs <-
              List.map
                (fun (v, i) ->
                  match i with
                  | Sil.Apply (fr, args) -> (
                      match Hashtbl.find_opt def fr with
                      | Some (Sil.Func_ref g) -> (
                          match find_fn g with
                          | Some gf when gf.Sil.generic -> (
                              (* every erased (TProto) position must resolve to the SAME concrete type *)
                              let resolve arg pty =
                                match pty with
                                | Types.TProto c -> (
                                    ...
                                    | _ -> (
                                        ...
                                        | _ -> Some None (* erased position, unprovable: bail *)))
                                | _ -> None (* concrete position: untouched *)
                              in
                              let infos = List.map2 (fun a (_, pt) -> resolve a pt) args gf.Sil.params in
                              let sns =
                                List.filter_map (function Some (Some (_, sn, c)) -> Some (sn, c) | _ -> None) infos
                              in
                              let bail = List.exists (function Some None -> true | _ -> false) infos in
                              match sns with
                              | (sn, c) :: _
                                when (not bail)
                                     && List.for_all (fun (s, _) -> s = sn) sns
                                     && retypable gf c ->
                                  let spec_name = g ^ "$" ^ sn in
                                  if find_fn spec_name = None then begin
                                    funcs := !funcs @ [ clone_specialized gf sn ];
                                    changed := true
                                  end;
                                  (* unwrap erased args; retype the result if it was erased *)
                                  let args' =
                                    List.map2
                                      (fun a info -> match info with Some (Some (pv, _, _)) -> pv | _ -> a)
                                      args infos
                                  in
                                  Hashtbl.replace def fr (Sil.Func_ref spec_name);
                                  (match Hashtbl.find_opt f.Sil.val_ty v with
                                  | Some (Types.TProto _) -> Hashtbl.replace f.Sil.val_ty v (Types.TStruct sn)
                                  | _ -> ());
                                  ignore (Hashtbl.find_opt f.Sil.val_ty fr);
                                  (v, Sil.Apply (fr, args'))
                              | _ -> (v, i))
                          | _ -> (v, i))
                      | _ -> (v, i))
                  | _ -> (v, i))
                b.Sil.instrs;
            (* commit the retargeted func_refs *)
            b.Sil.instrs <-
              List.map
                (fun (v, i) ->
                  match (i, Hashtbl.find_opt def v) with
                  | Sil.Func_ref _, Some (Sil.Func_ref n) -> (v, Sil.Func_ref n)
                  | _ -> (v, i))
                b.Sil.instrs)
          f.Sil.blocks)
      !funcs
  done;
  { m with Sil.funcs = !funcs }

(* ================= concept 28: ARC OPTIMIZATION — copy propagation ================= *)

(* the class-typed operands an instruction CONSUMES (mirrors the ownership verifier) *)
let consumed_ops (f : Sil.func) (i : Sil.instr) : Sil.value list =
  let is_class v =
    match Hashtbl.find_opt f.Sil.val_ty v with
    | Some (Types.TClass _ | Types.TFunc _) (* function values are managed too — 29 *) -> true
    | _ -> false
  in
  match i with
  | Sil.Destroy_value v -> [ v ]
  | Sil.Store (v, _) when is_class v -> [ v ]
  | _ -> []

(* A `copy_value`/`destroy_value` pair is REDUNDANT when the copy's whole lifetime sits inside
   its SOURCE's lifetime: every use of the copy could just use the source, the count never
   touches zero inside the bracket, and no deinit observation moves. Deleting the pair removes
   two refcount operations — the flagship ARC rewrite (swiftc: OSSA CopyPropagation).

   v0 soundness conditions, checked per copy `%c = copy_value %src` (all same-block; the
   cross-block generalization needs liveness — exercise):
     - %c's only CONSUMING use is a `destroy_value %c` later in the same block, and every
       other use of %c sits between the copy and that destroy;
     - the source outlives the bracket: %src is a function PARAMETER (guaranteed for the whole
       call), or an OWNED value whose own consuming use comes strictly AFTER %c's destroy in
       the same block. (A copy whose source is a memory LOAD is NOT removable: the slot may be
       overwritten inside the bracket, dropping the object's only other +1 — keeping it alive
       was the copy's whole job. The probes pin this case.)
   The rewrite: uses of %c become uses of %src; the copy and the destroy disappear. Iterated
   to a fixpoint (copies of copies). Soundness gate: the deinit-parity suite at -O — the
   ownership verifier guards RAW SIL (after mem2reg the slots are gone and ownership is
   threaded through phis, exactly like swiftc, whose OSSA is lowered before late passes). *)
let copy_propagation (f : Sil.func) : Sil.func =
  let changed = ref true in
  while !changed do
    changed := false;
    let params = List.map fst f.Sil.params in
    let uses : (Sil.value, (int * int * bool) list) Hashtbl.t = Hashtbl.create 64 in
    let note v b i c = Hashtbl.replace uses v ((b, i, c) :: Option.value (Hashtbl.find_opt uses v) ~default:[]) in
    List.iter
      (fun (b : Sil.block) ->
        List.iteri
          (fun idx (_, i) ->
            let consumed = consumed_ops f i in
            List.iter (fun u -> note u b.Sil.bid idx (List.mem u consumed)) (operands i))
          (List.rev b.Sil.instrs);
        List.iter (fun u -> note u b.Sil.bid max_int true) (term_operands b.Sil.term))
      f.Sil.blocks;
    let consuming_uses v =
      match Hashtbl.find_opt uses v with
      | Some us -> List.filter (fun (_, _, c) -> c) us
      | None -> []
    in
    List.iter
      (fun (b : Sil.block) ->
        let arr = Array.of_list (List.rev b.Sil.instrs) in
        Array.iter
          (fun (c, i) ->
            match i with
            | Sil.Copy_value src -> (
                ...
                | true, [ (_, d_idx, _) ] when d_idx <> max_int -> (
                    ...
                    if is_destroy && uses_inside && src_outlives && not !changed then begin
                      (* ONE rewrite per sweep: deleting instructions shifts every index and
                         stales the use map — recompute from scratch before the next one
                         (handoff-style chained copies corrupted a stale-index rewrite) *)
                      b.Sil.instrs <-
                        List.filter
                          (fun (v, i2) ->
                            (not (v = c))
...
let default_pipeline =
  [
    ...
    { name = "copy-propagation"; run = copy_propagation }; (* ARC optimization — concept 28 *)
...
(* `-O` (concept 24): inline -> SSA -> SPECIALIZE -> DEVIRTUALIZE -> inline the now-direct
   calls -> the scalar pipeline. Specialization creates direct-call work; the second inline
   round is where the abstraction cost actually dies. *)
let scalar_pipeline =
  List.filter (fun p -> p.name <> "mem2reg") default_pipeline

let optimize ?(log = false) (m : Sil.modul) : Sil.modul =
  let m = inline_module m in
  let m = run_pipeline ~log [ { name = "mem2reg"; run = mem2reg } ] m in
  let m = specialize_module m in
  let m = devirt_module m in
  let m = inline_module m in
  run_pipeline ~log scalar_pipeline m
