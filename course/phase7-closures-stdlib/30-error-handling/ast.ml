(* AST for concept 30 — a *contract*. It carries every node the concepts before it
...
type func_decl = {
  ...
  params : param list;
  throws : bool; (* NEW in this concept: declared `throws` *)
...
type enum_decl = {
  ...
  eraw : string option;
  is_error : bool; (* NEW in this concept: declared `: Error` — an error type *)
  espan : Token.span;
}
