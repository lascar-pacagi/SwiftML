...
First the front end. On the 22 programs of `oracle-typecheck.txt` — eleven well-formed,
eleven the generic rules must refuse — `swiftc -typecheck` and `./lab.exe --typecheck` must
...
stdout + exit code must be identical to `swiftc -Onone`'s. The 18 programs are the generic
story: a `-> T` result used concretely (the open), two conformers through one erased function,
a `where` clause, a generic calling a generic, a generic call in a loop and in a `while`, a
recursive generic, a multi-requirement constraint, a generic beside a plain `any P` parameter,
a generic result inside an optional, a three-field conformer, a `let` property beside a `var`
one, and a `Double` field times an integer literal. Only stdout and the exit code are compared.
A program either compiler refuses to build is reported as such, never counted as agreement; an
unstarted TODO stops the loop so the file reads TODO. (No `print` of an aggregate, no `==` on
two structs, no existential passed where `T` is expected, no ÷0 — see sema-subset.t and §2.)
