...
This concept adds three rules to that boundary, all checked inside the generic function's BODY
(no call is involved, so they are green before `infer_generic_call` exists). A value of type
parameter `T` exposes its CONSTRAINT's requirements and nothing else — `w()` is A's, not P's:
...
type is not known here, and `P` does not require `==`:
...
swiftc's route, and concept 23's — so we refuse the declaration outright, in our own words:

  $ cat > tbare.swift <<'PROG'
