(* SILGen for concept 22 (skeleton) — generic functions. The erased ONE-COPY lowering of the
   generic function itself is given; you implement the CALL-SITE lowering (TODO(22-silgen)). *)
...
let lower_func structs enums protos methods funcs gfuncs (name : string)
  ...
  let b =
    { next_val = 0; next_block = 1; cur = entry; blocks = [ entry ]; vars = Hashtbl.create 16; val_ty; funcs; structs; enums; protos; methods; gfuncs; ret; loops = [] }
  in
...
let lower (prog : Ast.program) : Sil.modul =
    ...
    List.concat_map
      (function
        | Ast.IFunc f when f.Ast.generics <> [] ->
            ...
            [ lower_func structs enums protos methods funcs gfuncs f.Ast.fname params (erase ret) f.Ast.body ]
        | Ast.IFunc f ->
            let params = List.map (fun (pr : Ast.param) -> (pr.Ast.pname, ty_of_name pr.Ast.ptype)) f.Ast.params in
            [ lower_func structs enums protos methods funcs gfuncs f.Ast.fname params (ret_of f) f.Ast.body ]
        | Ast.IStruct st ->
            ...
            List.map
              (fun (m : Ast.func_decl) ->
                ...
                in
                lower_func structs enums protos methods funcs gfuncs
  ...
  let main = lower_func structs enums protos methods funcs gfuncs "main" [] Types.TVoid main_body in
  let struct_layouts =
