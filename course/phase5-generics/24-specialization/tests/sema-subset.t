...
The generic rules of concept 22 are carried in unchanged, and they are given here too. A value
of type parameter `T` exposes its CONSTRAINT's requirements and nothing else — `w()` is A's,
not P's:
...
which we do not have, so we refuse the declaration outright, in our own words. That also means
the specializer never meets an unconstrained parameter:
