(* SILGen (given) — concept 23's full lowering: protocols, generics, casts. *)
...
let lower_func ?(generic = false) structs enums protos methods funcs gfuncs (name : string)
    ...
    List.concat_map
      (function
        | Ast.IFunc f when f.Ast.generics <> [] ->
            ...
            [ lower_func ~generic:true structs enums protos methods funcs gfuncs f.Ast.fname params (erase ret) f.Ast.body ]
        | Ast.IFunc f ->
