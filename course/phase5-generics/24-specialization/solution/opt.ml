(* FROZEN SOLUTION — concept 24 optimizer: devirtualization + generic specialization (M5). *)
...
(* an instruction we must keep even if its result is unused (it does something observable).
   `Apply_witness` is a call — through a witness table, but a call (concept 21). *)
let has_side_effect : Sil.instr -> bool = function
  | Sil.Store _ | Sil.Apply _ | Sil.Print _ | Sil.Apply_witness _ -> true
  | _ -> false
...
let inlinable (f : Sil.func) : bool =
  ...
  && (match f.Sil.blocks with
     | [ b ] ->
         not
           (List.exists
              (fun (_, i) -> match i with Sil.Apply _ | Sil.Apply_witness _ -> true | _ -> false)
              b.Sil.instrs)
     | _ -> false)
