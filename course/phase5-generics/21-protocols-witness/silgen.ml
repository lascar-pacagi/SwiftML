(* SILGen for concept 21 (skeleton) — protocols & witness tables. Methods, implicit self,
   and the builder are given; you implement the existential wrap, method dispatch, and the
   witness-table construction (the TODO(21a/b/c) holes).
...
let rec gen_expr (b : builder) (e : Ast.expr) : Sil.value =
  ...
  | Ast.Var (x, _) -> (
      ...
      | None ->
          (* inside a method, a bare name is a stored property: `r` = `self.r` (concept 21) *)
...
let lower_func structs enums protos methods funcs (name : string) (params : (string * Types.ty) list)
  ...
  let b =
    { next_val = 0; next_block = 1; cur = entry; blocks = [ entry ]; vars = Hashtbl.create 16; val_ty; funcs; structs; enums; protos; methods; ret; loops = [] }
  in
...
let lower (prog : Ast.program) : Sil.modul =
    ...
    List.concat_map
      (function
        | Ast.IFunc f ->
            ...
            [ lower_func structs enums protos methods funcs f.Ast.fname params (ret_of f) f.Ast.body ]
        | Ast.IStruct st ->
            ...
            List.map
              (fun (m : Ast.func_decl) ->
                ...
                in
                lower_func structs enums protos methods funcs
  ...
  let main = lower_func structs enums protos methods funcs "main" [] Types.TVoid main_body in
  let struct_layouts =
  ...
  in
  { Sil.funcs = funcdefs @ [ main ]; structs = struct_layouts; enums = enum_layouts; protos = proto_layouts; wtables }
