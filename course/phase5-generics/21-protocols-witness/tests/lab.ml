(* The concept lab CLI — linked against THIS concept's library (see tests/dune). Same surface as `swiftml3`, plus `-O` (run the
   SIL optimizer) and `--sil-opt` (print the optimized SIL):
     lab build <file.swift> [-O] [-o <out>]
     lab --emit-sil|--sil-opt|--emit-llvm [-O] <file.swift>
     lab --emit-tokens|--emit-ast|--typecheck <file.swift> *)

let usage () =
  prerr_endline "usage: lab build <file.swift> [-O] [-o <out>]";
