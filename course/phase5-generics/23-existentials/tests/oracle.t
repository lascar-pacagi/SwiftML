...
First the front end. On the 16 programs of `typecheck-corpus.txt` — eight well-formed,
eight the container and cast rules must refuse — `swiftc -typecheck` and `./lab.exe
--typecheck` must reach the same VERDICT (it cannot judge wording, only accept-or-reject). Running programs can
...
stdout + exit code must be identical to `swiftc -Onone`'s. The 15 programs are the container
story: five-word conformers that must be BOXED, carried through functions, `var` reassignment,
struct fields, optionals, generics and loops; two-, three- and four-word ones that must NOT be
(the boundary is at three); `as?` answering by the value's dynamic type; a cast in a loop over a
reassigned `var`, where the answer changes from iteration to iteration; a successful `as!`; a
`let` property beside a `var` one; and a `Double` field times an integer literal. Only stdout
and the exit code are compared. A program either compiler refuses to build is reported as such,
never counted as agreement; an unstarted TODO stops the loop so the file reads TODO. (No `print`
of an aggregate, no `==` on two structs, no optional chaining, no ÷0 — see sema-subset.t. The
FAILING `as!` is not here either: its exit 134 is checked in run-existentials.t, where the
message swiftc prints — module-qualified, with addresses — can be left out of the comparison.)
