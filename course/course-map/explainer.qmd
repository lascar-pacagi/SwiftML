...
is pedagogical. **SSA** — static single assignment — is the form in which every register is written
exactly once in the program text, so each value has exactly one defining instruction and "where did
this come from?" costs a lookup instead of a backwards search; virtually every optimization assumes
it. Handing it to you would waste the lesson, so SIL starts in memory and you **build SSA yourself**
in `phase4-optimizer/16-mem2reg-ssa`, watching a loop turn into block-argument (φ) form.
...
├── tests/          # one cram .t per TODO hole + alcotest units + oracle.t = the spec
...
Inside `tests/` the layout is deliberate: **one cram file per hole**, named for it, so `make lab`
reports a finished hole by name and an untouched one as `TODO` rather than a wall of diffs; every
case is a sentence claiming what its input should do; and every concept carries an **`oracle.t`**
that re-asks `swiftc` about a corpus on each run — accept/reject for a front-end concept,
build-and-run with byte-identical output once there is a back end. The other goldens record what
was true when they were written; the oracle asks again.
